...
function initCalTables(e, a) {
    ...
    $("#chk-cultivation-mode").off("change").on("change", function() {
        ...
        if (isGodMode) {
            ...
            // 手机版时，把神级方案选择框和按钮移动到顶部
            if (isMobile) {
                $("#recipe-god-query-wrapper").appendTo("#mobile-mode-switch-container");
                $("#recipe-god-query-action-wrapper").appendTo("#mobile-mode-switch-container");
            }
        } else {
            // 修炼查询模式
            ...
            // 手机版时，把修炼查询选择框和按钮移动到顶部
            if (isMobile) {
                $("#unultimated-chef-wrapper").appendTo("#mobile-mode-switch-container");
                $("#unultimated-chef-action-wrapper").appendTo("#mobile-mode-switch-container");
            }
        }
...
function loadHistoricalCalRule(e, historicalRule) {
    ...
    for (var s = 0; s < calCustomRule.rules.length; s++) {
        ...
        if (c.Satiety) {
            ...
            $("#pane-cal-custom .cal-custom-item:first").prepend($("#banquet-auto-calc"));
        }
...
function loadCalRule(e) {
    ...
    if (calCustomRule.isGuestRate) {
        ...
        // 手机版时，把修炼/神级方案开关移动到顶部
        if (isMobile) {
            $("#cultivation-mode-switch-wrapper").appendTo("#mobile-mode-switch-container");
            $("#mobile-mode-switch-container").show();
        } else {
            // 桌面版时，确保开关在原位置
