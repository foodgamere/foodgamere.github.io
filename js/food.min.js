...
function initCalCustomTable(e) {
    ...
    $(".equip-box").click(function() {
        ...
        if (isUnultimatedMode && typeof sortEquipsForCultivation === 'function') {
            ...
            // 如果是"全部"分类（categoryName为null），保持原始选项不过滤
        }
        
        // 先解绑旧的事件，避免重复绑定
        $equipSelect.off("changed.bs.select").off("change").off("hidden.bs.select");
...
function _updateBanquetTierSelector() {
    ...
    // 单客人模式只用第一个贵客
    var levels = BanquetOptimizer.BANQUET_LEVELS;
    var isDual = levels[levelIndex] && levels[levelIndex].dual;
    var key = BanquetOptimizer.findTierScoreKey(title1, isDual ? title2 : null);
