...
function getUpdateData(e) {
    ...
    for (var o in e.chefs) {
        ...
        if (selectedEquip) {
            ...
            e.chefs[o].equip = e.chefs[o].originalEquip;
            e.chefs[o].equipId = e.chefs[o].originalEquipId;
            e.chefs[o].equipDisp = e.chefs[o].originalEquipDisp;
        }
        var equipToUse = e.chefs[o].equip;
        
        // 只有当选择了应用遗玉时，才覆盖厨师自身的遗玉设置
        if (selectedAmbers.length > 0) {
            e.chefs[o].disk.ambers = JSON.parse(JSON.stringify(e.chefs[o].originalDiskAmbers));
        ...
        // 处理满级心法盘
        if (maxDiskLevel) {
            e.chefs[o].disk.level = e.chefs[o].disk.maxLevel;
            e.chefs[o].diskDisp = GetDiskDisp(e.chefs[o].disk);
        } else {
            e.chefs[o].disk.level = e.chefs[o].originalDiskLevel;
            e.chefs[o].diskDisp = GetDiskDisp(e.chefs[o].disk);
        }
...
function initChefShow() {
    ...
    // 采集选项只控制第12-15列（肉、面、菜、鱼），不控制第16列（采集类型）
    for (t = 12; t <= 15; t++)
        changeTableHeaderClass(e, t, i);
    // 采集类型列（第16列）的显示由采集组合下拉框控制，在updateChefMaterialComboColumn中处理
