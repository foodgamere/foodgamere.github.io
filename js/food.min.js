...
function showAlert(message, title) {
    ...
    // 移除已有的弹窗
    $('.common-alert-overlay').remove();
    
    // 创建弹窗HTML
    var html = '<div class="common-alert-overlay">';
    html += '<div class="common-alert-box">';
    html += '<div class="common-alert-header">';
    html += '<span class="common-alert-title">' + title + '</span>';
    html += '<span class="common-alert-close">&times;</span>';
    html += '</div>';
    html += '<div class="common-alert-body">' + message + '</div>';
    html += '</div>';
    html += '</div>';
    ...
    // 点击关闭按钮关闭弹窗
    $alert.find('.common-alert-close').on('click', function() {
 ...
 * @param {jQuery} matSelect - 下拉框的 jQuery 对象
 */
function showExcludeSchemesModal(storageKey, matSelect) {
    // 读取保存的方案
    var schemes = JSON.parse(localStorage.getItem(storageKey) || "[]");
    
    var $modal = $('#exclude-schemes-modal');
    var $modalBody = $('#exclude-schemes-modal-body');
    
    // 保存当前的 matSelect 和 storageKey 到 modal 的 data 中
    $modal.data('current-matSelect', matSelect);
    $modal.data('current-storageKey', storageKey);
    ...
    if (schemes.length === 0) {
        // 使用空状态模板
        var emptyTemplate = document.getElementById('exclude-schemes-empty-template');
        var emptyContent = emptyTemplate.content.cloneNode(true);
        $modalBody.append(emptyContent);
    } else {
        // 使用方案项模板
        var itemTemplate = document.getElementById('exclude-scheme-item-template');
        
        for (var i = 0; i < schemes.length; i++) {
            var scheme = schemes[i];
            
            // 克隆模板
            var itemContent = itemTemplate.content.cloneNode(true);
            var $item = $(itemContent).find('.scheme-item');
            
            // 设置数据属性
            $item.attr('data-index', i);
            
            // 填充内容
            $item.find('.scheme-name').text(scheme.name);
            $item.find('.scheme-materials').text((scheme.names || []).join('、'));
            
            // 设置按钮的数据属性
            $item.find('.btn-apply-scheme').attr('data-index', i);
            $item.find('.btn-delete-scheme').attr('data-index', i);
            
            // 添加到容器
            $modalBody.append(itemContent);
        }
    }
    
    // 只在第一次时绑定事件（避免重复绑定）
    if (!$modal.data('events-bound')) {
        $modal.data('events-bound', true);
        
        // 绑定应用按钮事件（使用事件委托）
        $modal.on('click', '.btn-apply-scheme', function() {
            var index = $(this).data('index');
            // 从 modal 的 data 中获取当前的 storageKey 和 matSelect
            var currentStorageKey = $modal.data('current-storageKey');
            var currentMatSelect = $modal.data('current-matSelect');
            var currentSchemes = JSON.parse(localStorage.getItem(currentStorageKey) || "[]");
            var scheme = currentSchemes[index];
            if (scheme) {
                currentMatSelect.selectpicker('val', scheme.values);
                setExcludeMaterialQuantityMap(currentMatSelect, scheme.quantities || {});
                currentMatSelect.selectpicker('refresh');
                
                // 手动触发change事件以更新表格
                currentMatSelect.trigger('change');
                currentMatSelect.trigger('changed.bs.select');
                
                $modal.modal('hide');
                showAlert('方案"' + scheme.name + '"已应用', '成功');
            }
        });
        
        // 绑定删除按钮事件（使用事件委托）
        $modal.on('click', '.btn-delete-scheme', function() {
            var index = $(this).data('index');
            // 从 modal 的 data 中获取当前的 storageKey 和 matSelect
            var currentStorageKey = $modal.data('current-storageKey');
            var currentMatSelect = $modal.data('current-matSelect');
            ...
            bootbox.confirm({
                size: "small",
                message: '<div class="text-center">确定要删除方案"' + currentSchemes[index].name + '"吗？</div>',
                buttons: {
                    confirm: {
                        label: '删除',
                ...
                callback: function(result) {
                    if (result) {
                        ...
                        localStorage.setItem(currentStorageKey, JSON.stringify(currentSchemes));
                        
                        showAlert('方案已删除', '成功');
                        
                        // 只刷新内容，不重新绑定事件
                        showExcludeSchemesModal(currentStorageKey, currentMatSelect);
                    }
                }
            });
        });
...
function ensureExcludeMaterialLimitStyles() {
    if ($("#exclude-material-limit-styles").length) return;
    $("head").append("<style id='exclude-material-limit-styles'>"
        + ".exclude-material-option-row{display:flex;align-items:center;justify-content:space-between;gap:4px;width:100%;}"
        + ".exclude-material-option-name{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}"
        + ".exclude-material-limit-box{display:inline-flex;align-items:center;gap:1px;flex:none;position:relative;z-index:4;}"
        + ".exclude-material-limit-prefix{font-size:12px;color:#666;white-space:nowrap;}"
        + ".exclude-material-limit-input{width:40px;min-width:40px;height:22px;padding:1px 3px;border:1px solid #ccc;border-radius:4px;text-align:center;background:#fff;position:relative;z-index:5;}"
        + ".bootstrap-select.exclude-material-select-picker,.bootstrap-select.exclude-material-select-picker>.dropdown-toggle{width:112px!important;}"
        + ".bootstrap-select.exclude-material-select-picker .dropdown-menu{min-width:180px!important;}"
        + ".bootstrap-select .dropdown-menu li.exclude-material-limit-item{position:relative;}"
        + ".bootstrap-select .dropdown-menu li.exclude-material-limit-item>a{display:block;padding-right:72px;}"
        + ".bootstrap-select .dropdown-menu li.exclude-material-limit-item>a>span.text{display:block;width:100%;min-width:0;}"
        + ".bootstrap-select.show-tick .dropdown-menu li.exclude-material-limit-item a span.text{margin-right:0!important;}"
        + ".bootstrap-select .dropdown-menu li.exclude-material-limit-item .exclude-material-limit-overlay{position:absolute;right:34px;top:50%;transform:translateY(-50%);z-index:4;}"
        + ".bootstrap-select.show-tick .dropdown-menu li.exclude-material-limit-item a span.check-mark{right:8px;top:50%;margin-top:0;transform:translateY(-50%);z-index:1;}"
        + ".bootstrap-select .dropdown-menu li a .exclude-material-limit-input{pointer-events:auto;}"
        + "</style>")
}
function getExcludeMaterialQuantityMap(matSelect) {
    var $select = matSelect && matSelect.jquery ? matSelect : $(matSelect)
      , stored = $select.data("excludeMaterialQuantityMap");
    if (stored)
        return $.extend({}, stored);
    stored = {};
    $select.find("option").each(function() {
        var value = $(this).val()
          , limit = $(this).attr("data-exclude-limit");
        if (null != limit && "" !== $.trim(limit)) {
            var parsed = parseInt(limit, 10);
            isNaN(parsed) || parsed < 0 || (stored[String(value)] = String(parsed))
        }
    });
    return $select.data("excludeMaterialQuantityMap", stored),
    $.extend({}, stored)
}
function setExcludeMaterialQuantityMap(matSelect, quantityMap) {
    var $select = matSelect && matSelect.jquery ? matSelect : $(matSelect)
      , cleaned = {};
    quantityMap = quantityMap || {};
    for (var key in quantityMap)
        if (quantityMap.hasOwnProperty(key)) {
            var value = quantityMap[key];
            if (null != value) {
                var text = $.trim(String(value));
                if ("" !== text) {
                    var parsed = parseInt(text, 10);
                    isNaN(parsed) || parsed < 0 || (cleaned[String(key)] = String(parsed))
                }
            }
        }
    $select.data("excludeMaterialQuantityMap", cleaned),
    $select.find("option").each(function() {
        var value = String($(this).val());
        cleaned.hasOwnProperty(value) ? $(this).attr("data-exclude-limit", cleaned[value]) : $(this).removeAttr("data-exclude-limit")
    }),
    syncExcludeMaterialLimitInputs($select)
}
function clearExcludeMaterialQuantityMap(matSelect) {
    setExcludeMaterialQuantityMap(matSelect, {})
}
function getExcludeMaterialRules(matSelect) {
    var $select = matSelect && matSelect.jquery ? matSelect : $(matSelect)
      , selectedValues = $select.val() || []
      , quantityMap = getExcludeMaterialQuantityMap($select)
      , rules = [];
    for (var i = 0; i < selectedValues.length; i++) {
        var materialId = String(selectedValues[i])
          , quantity = null;
        quantityMap.hasOwnProperty(materialId) && (quantity = parseInt(quantityMap[materialId], 10),
...
function isRecipeExcludedByMaterialRules(recipeMaterials, materialRules) {
    if (!recipeMaterials || 0 === recipeMaterials.length || !materialRules || 0 === materialRules.length)
        return !1;
    for (var ruleMap = {}, i = 0; i < materialRules.length; i++) {
        var rule = materialRules[i];
        rule && null != rule.materialId && (ruleMap[String(rule.materialId)] = null == rule.quantity || "" === rule.quantity ? null : Number(rule.quantity))
    }
    for (var j = 0; j < recipeMaterials.length; j++) {
        var recipeMaterial = recipeMaterials[j]
          , materialId = String(recipeMaterial.material);
        if (ruleMap.hasOwnProperty(materialId)) {
            var limit = ruleMap[materialId];
            if (null == limit)
                return !0;
            if ((Number(recipeMaterial.quantity) || 0) > limit)
                return !0
        }
    }
    return !1
}
function getExcludeMaterialSelectedNames(matSelect) {
    var $select = matSelect && matSelect.jquery ? matSelect : $(matSelect)
      , quantityMap = getExcludeMaterialQuantityMap($select)
      , names = [];
    return $select.find("option:selected").each(function() {
        var value = String($(this).val())
          , name = $(this).text()
          , limit = quantityMap[value];
        names.push(limit ? name + "(大于" + limit + "排除)" : name)
    }),
    names
}
function syncExcludeMaterialLimitInputs(matSelect) {
    var $select = matSelect && matSelect.jquery ? matSelect : $(matSelect)
      , $container = $select.closest(".bootstrap-select");
    if (!$container.length)
        return;
    var quantityMap = getExcludeMaterialQuantityMap($select);
    var picker = $select.data("selectpicker")
      , currentData = picker && picker.selectpicker && picker.selectpicker.current ? picker.selectpicker.current.data : null;
    currentData && currentData.length || (currentData = []);
    $container.find(".inner > li").each(function(itemIndex) {
        var itemData = currentData[itemIndex];
        if (!itemData || "option" !== itemData.type || !itemData.option)
            return;
        var $li = $(this)
          , $option = $(itemData.option);
        if (!$option.length)
            return;
        $li.addClass("exclude-material-limit-item");
        var materialId = String($option.val())
          , materialName = $option.text()
          , currentValue = quantityMap[materialId] || ""
          , $text = $li.find("> a > .text");
        if (!$text.length)
            return;
        $text.text(materialName);
        var $overlay = $li.children(".exclude-material-limit-overlay");
        $overlay.length || ($overlay = $("<div class='exclude-material-limit-overlay'><span class='exclude-material-limit-box'><span class='exclude-material-limit-prefix'>大于</span><input type='number' min='0' step='1' class='exclude-material-limit-input' placeholder='数量'></span></div>"),
        $li.append($overlay));
        var $input = $overlay.find(".exclude-material-limit-input");
        $input.attr("data-material-id", materialId),
        document.activeElement !== $input[0] && $input.val(currentValue)
    })
}
function initExcludeMaterialLimitInputs(matSelect) {
    var $select = matSelect && matSelect.jquery ? matSelect : $(matSelect);
    if (!$select.length || $select.data("excludeMaterialLimitInited"))
        return;
    $select.data("excludeMaterialLimitInited", !0),
    ensureExcludeMaterialLimitStyles();
    var $container = $select.closest(".bootstrap-select")
      , syncInputs = function() {
        setTimeout(function() {
            syncExcludeMaterialLimitInputs($select)
        }, 0)
    };
    $container.addClass("exclude-material-select-picker"),
    syncInputs(),
    $select.on("loaded.bs.select rendered.bs.select shown.bs.select refreshed.bs.select changed.bs.select", syncInputs),
    $container.on("shown.bs.dropdown", syncInputs),
    $container.on("click mousedown mouseup dblclick", ".exclude-material-limit-overlay, .exclude-material-limit-box", function(e) {
        e.stopPropagation()
    }),
    $container.on("click mousedown mouseup dblclick", ".exclude-material-limit-input", function(e) {
        e.stopPropagation()
    }),
    $container.on("focus focusin", ".exclude-material-limit-input", function(e) {
        e.stopPropagation()
    }),
    $container.on("keydown", ".exclude-material-limit-input", function(e) {
        e.stopPropagation()
    }),
    $container.on("input", ".exclude-material-limit-input", function(e) {
        e.stopPropagation();
        var $input = $(this)
          , raw = $input.val().replace(/[^\d]/g, "");
        $input.val(raw);
        var materialId = String($input.attr("data-material-id"))
          , quantityMap = getExcludeMaterialQuantityMap($select);
        raw ? quantityMap[materialId] = raw : delete quantityMap[materialId],
        setExcludeMaterialQuantityMap($select, quantityMap)
    }),
    $container.on("change", ".exclude-material-limit-input", function(e) {
        e.stopPropagation();
        var materialId = String($(this).attr("data-material-id"))
          , selectedValues = $select.val() || [];
        if (selectedValues.indexOf(materialId) < 0) {
            selectedValues.push(materialId),
            $select.selectpicker("val", selectedValues)
        }
        $select.trigger("change"),
        $select.trigger("changed.bs.select")
    }),
    $container.on("click", ".bs-deselect-all", function() {
        clearExcludeMaterialQuantityMap($select)
    }),
    $select.on("changed.bs.select", function() {
        var selectedValues = $select.val() || [];
        0 === selectedValues.length && clearExcludeMaterialQuantityMap($select)
    })
}
window.getExcludeMaterialRules = getExcludeMaterialRules,
window.isRecipeExcludedByMaterialRules = isRecipeExcludedByMaterialRules;
...
function initTables(e, a) {
    ...
    $('.main-nav a[data-toggle="tab"]').on("show.bs.tab", function(e) {
        ...
        // 手机模式下，切换到非计算器页面时隐藏贵客查询/碰瓷查询开关容器
        if (isMobile) {
            if (".pane-cal" == $(e.target).attr("href")) {
                // 切换回计算器页面时，根据当前模式恢复显示
                if (calCustomRule && calCustomRule.isGuestRate) {
                    $("#mobile-mode-switch-container").show();
                }
            } else {
                $("#mobile-mode-switch-container").hide();
            }
        }
    }),
...
function initRecipeTable(e) {
    ...
    for (var t in $("#chk-recipe-show-material").selectpicker().on("changed.bs.select", function(a, t, i, l) {
        ...
        var s = $(this).val()
          , c = s && s.length ? getRecipeMaterialSortOrder(e, Number(s[0])) : null;
        c && $("#recipe-table").DataTable().order(c),
    ...
    // 初始化排除食材下拉框（按地区分组）
    (function() {
        var matSelect = $("#chk-recipe-material-exclude");
        var regionOrder = ["牧场", "鸡舍", "猪圈", "菜棚", "菜地", "森林", "作坊", "池塘"];
        var regionMats = {};
        for (var ri in regionOrder) regionMats[regionOrder[ri]] = [];
        for (var mi in e.materials) {
            var mat = e.materials[mi];
            if (regionMats[mat.origin]) regionMats[mat.origin].push(mat);
        }
        for (var ri in regionOrder) {
            var rn = regionOrder[ri], mats = regionMats[rn];
            if (mats.length > 0) {
                var grp = $("<optgroup label='" + rn + "'></optgroup>");
                for (var mi in mats) grp.append("<option value='" + mats[mi].materialId + "'>" + mats[mi].name + "</option>");
                matSelect.append(grp);
            }
        }
        matSelect.selectpicker(),
        initExcludeMaterialLimitInputs(matSelect);
        
        // 添加反选按钮和保存/方案按钮
        var bsContainer = matSelect.closest(".bootstrap-select");
        var $actionsBox = bsContainer.find(".bs-actionsbox .btn-group");
        if ($actionsBox.length && !$actionsBox.find(".bs-invert-all").length) {
            // 第一行：反选和清空按钮
            var invertBtn = '<button type="button" class="actions-btn bs-invert-all btn btn-default">反选</button>';
            $actionsBox.find(".bs-deselect-all").before(invertBtn);
            
            // 第二行：保存和方案按钮
            var secondRowHtml = '<div class="btn-group btn-group-sm btn-block" style="margin-top: 5px;">';
            secondRowHtml += '<button type="button" class="actions-btn bs-save-scheme btn btn-default">保存</button>';
            secondRowHtml += '<button type="button" class="actions-btn bs-view-schemes btn btn-default">方案</button>';
            secondRowHtml += '</div>';
            $actionsBox.parent().append(secondRowHtml);
            
            // 绑定反选按钮事件
            $actionsBox.find(".bs-invert-all").on("click", function(ev) {
                ev.stopPropagation();
                ev.preventDefault();
                
                var allOptions = matSelect.find("option");
                var selectedValues = matSelect.val() || [];
                var newSelectedValues = [];
                
                allOptions.each(function() {
                    var optVal = $(this).val();
                    if (selectedValues.indexOf(optVal) === -1) {
                        newSelectedValues.push(optVal);
                    }
                });
                
                matSelect.selectpicker("val", newSelectedValues);
                matSelect.trigger("changed.bs.select");
                
                return false;
            });
            
            // 绑定保存按钮事件
            bsContainer.find(".bs-save-scheme").on("click", function(ev) {
                ev.stopPropagation();
                ev.preventDefault();
                
                var selectedValues = matSelect.val() || [];
                if (selectedValues.length === 0) {
                    showAlert("请先选择要排除的食材", "提示");
                    return false;
                }
                
                // 收起下拉框
                matSelect.selectpicker('toggle');
                
                // 获取选中食材的名称
                var selectedNames = getExcludeMaterialSelectedNames(matSelect)
                  , selectedQuantities = getExcludeMaterialQuantityMap(matSelect);
                ...
                bootbox.prompt({
                    ...
                    callback: function(schemeName) {
                        if (!schemeName) return;
                        
                        // 保存到 localStorage（使用统一的键名）
                        var schemes = JSON.parse(localStorage.getItem("exclude_material_schemes") || "[]");
                        schemes.push({
                            name: schemeName,
                        ...
                        localStorage.setItem("exclude_material_schemes", JSON.stringify(schemes));
            ...
            // 绑定方案按钮事件
            bsContainer.find(".bs-view-schemes").on("click", function(ev) {
                ev.stopPropagation();
                ev.preventDefault();
                ...
                showExcludeSchemesModal("exclude_material_schemes", matSelect);
                return false;
            });
        }
        
        // 分组默认收起，点击展开/收起
        var _matGrpInited = false;
        bsContainer.on("shown.bs.dropdown", function() {
            if (!_matGrpInited) {
                _matGrpInited = true;
                var $menu = bsContainer.find(".dropdown-menu.inner");
                $menu.find(".dropdown-header").each(function() {
                    var $hdr = $(this);
                    $hdr.addClass("collapsible");
                    $hdr.append("<span class='collapse-icon'>❯</span>");
                    var $items = $hdr.nextUntil(".dropdown-header, .divider");
                    // 默认收起
                    $items.hide();
                    $hdr.find(".collapse-icon").css("transform", "translateY(-50%) rotate(0deg)");
                });
                $menu.on("click", ".dropdown-header.collapsible", function(ev) {
                    ev.stopPropagation();
                    ev.preventDefault();
                    var $hdr = $(this);
                    var $items = $hdr.nextUntil(".dropdown-header, .divider");
                    var $icon = $hdr.find(".collapse-icon");
    ...
    $.fn.dataTableExt.afnFiltering.push(function(a, t, i, l, s) {
        if (a.nTable != document.getElementById("recipe-table"))
            return !0;
        var excludeMaterialRules = getExcludeMaterialRules("#chk-recipe-material-exclude");
        if (!excludeMaterialRules || 0 == excludeMaterialRules.length)
            return !0;
        return !isRecipeExcludedByMaterialRules(l.materials, excludeMaterialRules)
    }),
    $("#chk-recipe-material-exclude").on("changed.bs.select", function() {
        $("#recipe-table").DataTable().draw()
    }),
    e.chefs)
    ...
    $("#btn-recipe-reset").click(function() {
        ...
        $("#chk-recipe-material-exclude").selectpicker("deselectAll"),
        clearExcludeMaterialQuantityMap("#chk-recipe-material-exclude"),
...
function getRecipeMaterialSortOrder(e, a) {
    if (!e.allSelectedMaterials || !e.allSelectedMaterials.length)
        return null;
    var t = e.allSelectedMaterials.indexOf(a);
    return t < 0 ? null : [e.recipeColNum + t, "desc"]
}
function getRecipeQuestSortOrder(e, a, t) {
    if (t)
        return [[e.recipeColNum + e.recipeAddColNum - 2, "asc"], [16, "asc"]];
    if (!a)
        return [16, "asc"];
    if ("questMaterialEff" == a.type) {
        var i = getRecipeMaterialSortOrder(e, a.materialId);
        return i || [23, "desc"]
    }
    return "materialEff" == a.type ? [23, "desc"] : "condimentEff" == a.type ? [24, "desc"] : "goldEff" == a.type ? [22, "desc"] : [16, "asc"]
}
function updateRecipeQuest(e, a) {
            ...
            for (var d in e.quests)
                if (r[p] == e.quests[d].questId) {
                    ...
                    e.quests[d].conditions) {
                        ...
                        "修炼任务" == e.quests[d].type && (4 == e.quests[d].conditions[u].rank ? c = !0 : s = {
                            type: "time"
                        }),
                        e.quests[d].conditions[u].materialId && e.quests[d].conditions[u].materialEff ? (i.push(e.quests[d].conditions[u].materialId),
                        s = {
                            type: "questMaterialEff",
                            materialId: e.quests[d].conditions[u].materialId
                        }) : s = e.quests[d].conditions[u].materialEff ? {
                            type: "materialEff"
                        } : e.quests[d].conditions[u].condimentEff ? {
                            type: "condimentEff"
                        } : e.quests[d].conditions[u].goldEff ? {
                            type: "goldEff"
                        } : {
                            type: "time"
                        }
    ...
    $("#recipe-table").DataTable().order(getRecipeQuestSortOrder(e, s, c)).draw()
}
    ...
    o.MakeCellsEditable({
        ...
        onUpdate: function(table, row, cell, oldValue) {
            if (20 == cell.index().column) {
                ...
                // 如果剩余份数有小数，向上取整后计算时间
                var countForTime = Math.ceil(remainingCount);
                
                // 获取最新的单时间（考虑了额外上场厨师的时间加成）
                var unitTime = rowData.oTime;
                
                // 计算剩余时间：向上取整后的份数 * 单时间
                var remainingTime = countForTime * unitTime;
                var remainingTimeDisp = secondsToTime(remainingTime);
                
                // 更新份数单元格显示
                cell.data(remainingCount);
                
                // 更新时间单元格显示（列21）
                o.cell(row.index(), 21).data(remainingTimeDisp);
...
function initChefTable(e) {
    ...
    $.fn.dataTableExt.afnFiltering.push(function(e, a, t, i, l) {
        if (e.nTable != document.getElementById("chef-table"))
            return !0;
        var s = $("#chk-chef-disk-level").val();
        if (0 == s.length)
            return !0;
        var c = i.disk && i.disk.level ? String(i.disk.level) : "";
    ...
    $("#chk-chef-disk-level").on("changed.bs.select", function() {
        $("#chef-table").DataTable().draw()
    }),
    ...
    $("#btn-chef-reset").click(function() {
        ...
        $("#chk-chef-disk-level").selectpicker("deselectAll"),
    ...
    o.MakeCellsEditable({
        ...
        onUpdate: function(a, t, i, l) {
            if (28 == i.index().column) {
                ...
                // 如果用户清空了厨具（equipId为空或null），则真正置空当前厨具
                ...
                s.equip = c,
                s.equipDisp = r,
                // 手动设置时同步更新当前厨具和保存态，避免刷新或重算后又恢复旧值
                s.originalEquip = c,
                s.originalEquipId = s.equipId,
                s.originalEquipDisp = r,
                s.initialEquipForRestore = c,
                s.initialEquipForRestoreId = s.equipId,
                s.initialEquipForRestoreDisp = r,
                // 重新计算采集期望值
                ...
                if (!isTimeChef && s.ultimateSkillEffect) {
                    for (var ue in s.ultimateSkillEffect) {
                        if ("OpenTime" == s.ultimateSkillEffect[ue].type || "CookbookTime" == s.ultimateSkillEffect[ue].type) {
                            isTimeChef = true;
                            break;
                        }
                    }
...
// 导入数据后刷新额外上场技能列表的公共函数
function _refreshPartialUltimateAfterImport(e) {
    var $dropdown = $('#chk-chef-partial-ultimate').parent().find('.dropdown-menu');
    var $activeTab = $dropdown.find('.chef-type-tabs li.active a');
    var isTimeTabActive = $activeTab.hasClass('chef-tab-time');
...
function initImportExport(e) {
    ...
    // 切换用户（value 现在是索引）
    $("#select-user-profile").selectpicker().on("changed.bs.select", function() {
        var selectedIdx = parseInt($(this).val());
        if (selectedIdx >= 0 && selectedIdx !== _currentUserIndex) {
            switchToUser(selectedIdx, e);
        }
    });

    // 修改用户名
    $("#btn-rename-user").click(function() {
        ...
        bootbox.prompt({
            ...
            callback: function(newName) {
                ...
                if (newName === curUser) return;
                renameUserProfile(newName);
                // 直接更新select的option文本和selectpicker显示
                var $sel = $("#select-user-profile");
                $sel.find("option:selected").text(newName);
                $sel.selectpicker("render");
                $.popupmsg({ message: "用户名已修改为 \"" + newName + "\"" });
            }
        });
    });
    ...
    $("#btn-import-data").click(function() {
        ...
        bootbox.prompt({
            title: "请输入用户名称",
            placeholder: "例如：大号、小号1",
            buttons: {
                confirm: { label: "确定" },
                cancel: { label: "取消" }
            },
            callback: function(userName) {
                if (!userName) return; // 取消
                ...
                // 创建用户配置
                var newIdx = createUserProfile(userName);
                // 开始导入
                $.popupmsg({ message: "导入中...", fade: !1 });
                $.ajax({
                    cache: !1,
                    success: function(a) {
                        var t = JSON.parse(a);
                        if ("S" == t.ret) {
                            ...
                            if (i) {
                                ...
                                $.popupmsg({ message: "用户 \"" + userName + "\" 导入成功 !" });
                                initUserProfileUI();
                                try {
                                    _refreshPartialUltimateAfterImport(e);
                                    updateRecipeChefTable(e, false);
                                } catch (err) {}
                            } else {
                                // 导入失败，回滚用户
                                removeUserProfile();
                                $.popupmsg({ message: "导入失败 !", type: "alert-danger" });
                                initUserProfileUI();
                            }
                        } else {
                            removeUserProfile();
                            $.popupmsg({ message: t.msg, type: "alert-danger" });
                            initUserProfileUI();
                        }
                    },
                    error: function() {
                        removeUserProfile();
                        $.popupmsg({ message: "获取失败 !", type: "alert-danger" });
                        initUserProfileUI();
                    },
        ...
        $.ajax({
            ...
            success: function(a) {
                ...
                if ("S" == t.ret) {
                    ...
                    if (i) {
                        $("#input-import-data").val("");
                        $.popupmsg({ message: "用户 \"" + getCurrentUserName() + "\" 数据更新成功 !" });
                        try {
                            _refreshPartialUltimateAfterImport(e);
                            updateRecipeChefTable(e, false);
                        } catch (err) {}
                    } else {
                        $.popupmsg({ message: "更新失败 !", type: "alert-danger" });
                    }
                } else {
                    $.popupmsg({ message: t.msg, type: "alert-danger" });
                }
    ...
    $("#btn-remove-user").click(function() {
        ...
        bootbox.confirm({
            message: "确定要删除用户 \"" + curUser + "\" 的所有数据吗？此操作不可恢复。",
            buttons: {
                confirm: { label: "确定删除", className: "btn-danger" },
                cancel: { label: "取消" }
            },
            callback: function(result) {
                if (result) {
                    ...
                    initUserProfileUI();
                    // 如果还有用户，加载新当前用户的数据
                    var newUser = getCurrentUserName();
                    if (newUser && _currentUserIndex >= 0) {
                        var userData = localStorage.getItem(_getStorageKeyByIndex(_currentUserIndex));
                        if (userData) {
                            importData(e, userData, true);
                            try {
                                _refreshPartialUltimateAfterImport(e);
                                updateRecipeChefTable(e, false);
                            } catch (err) {}
                    ...
                    $.popupmsg({ message: "用户 \"" + curUser + "\" 已删除" });
                }
            }
        });
    }),
    ...
    $("#btn-export-download").click(function() {
        ...
        var fileName = "food";
        var curUser = getCurrentUserName();
        if (curUser) fileName += "_" + curUser;
        saveAs(e, fileName + ".txt")
    }),
...
function importDataBcjh(e) {
    ...
    // 刷新排序（已勾选的优先显示）
    if (typeof refreshCalPartialUltimateSort === 'function') {
        refreshCalPartialUltimateSort();
    }
    var r = [];
...
function importData(e, a, skipUISettings) {
    var t;
    try {
        t = JSON.parse(a)
    } catch (e) {
    ...
    t.cal) {
            ...
            if (t.cal[i].id == o.cal[l].id) {
                for (var u in t.cal[i].data) {
                    ...
                    for (var h in o.cal[l].data)
                        if (getCalCustomSchemeValue(t.cal[i].data[u]) == getCalCustomSchemeValue(o.cal[l].data[h])) {
                            o.cal[l].data[h].menu = t.cal[i].data[u].menu,
                            t.cal[i].data[u].name && (o.cal[l].data[h].name = t.cal[i].data[u].name),
                            t.cal[i].data[u].schemeId && (o.cal[l].data[h].schemeId = t.cal[i].data[u].schemeId),
                            f = !0;
                            break
                        }
                    f || (o.cal[l].data.push(t.cal[i].data[u]),
                    t.cal[i].id == calCustomRule.id && (p += getCalCustomSchemeOption(t.cal[i].data[u])))
                }
                d = !0;
                break
            }
        ...
        t.cal[i].id == calCustomRule.id))
            for (var u in t.cal[i].data)
                p += getCalCustomSchemeOption(t.cal[i].data[u]);
        p && $("#select-cal-custom").append(p).selectpicker("destroy").selectpicker()
    }
    updateCalLocalData(o.cal),
    skipUISettings || updateMenu(t),
    skipUISettings || updateSetting(t),
    t.hasOwnProperty("decorationEffect") && (e.decorationEffect = t.decorationEffect,
    $("#input-cal-decoration").val(t.decorationEffect == null ? "" : t.decorationEffect));
    ...
    try {
        localStorage.setItem(getCurrentStorageKey(), generateExportData())
    } catch (e) {}
    ...
    // 更新calCustomRule.gameData引用，确保碰瓷查询使用最新数据
    if (typeof calCustomRule !== 'undefined') {
        calCustomRule.gameData = e;
    }
    
    // 延迟刷新碰瓷查询的贵客统计（确保数据已完全更新）
    setTimeout(function() {
        if (typeof GuestRateCalculator !== 'undefined' && GuestRateCalculator.populatePengciGuestOptions) {
            GuestRateCalculator.populatePengciGuestOptions();
        }
    }, 100);
...
// 计算器方案兼容工具：旧方案没有 schemeId/name 时继续使用 score。
function getCalCustomSchemeValue(scheme) {
    if (!scheme) return "";
    return String(scheme.schemeId || scheme.id || scheme.score || "");
}
function getCalCustomSchemeLabel(scheme) {
    if (!scheme) return "";
    return String(scheme.name || scheme.score || "");
}
function getCalCustomSchemeOption(scheme) {
    var value = getCalCustomSchemeValue(scheme);
    var label = getCalCustomSchemeLabel(scheme);
    return '<option value="' + escapeHtml(value) + '">' + escapeHtml(label) + '</option>';
}
function findCalCustomSchemeRule(calData, ruleId) {
    for (var i = 0; i < (calData || []).length; i++) {
        if (calData[i] && calData[i].id == ruleId) {
            return calData[i];
        }
    }
    return null;
}
function refreshCalCustomSchemeSelect(schemes, selectedValue) {
    var html = "";
    for (var i = 0; i < (schemes || []).length; i++) {
        html += getCalCustomSchemeOption(schemes[i]);
    }
    var $select = $("#select-cal-custom");
    $select.html(html).selectpicker("destroy").selectpicker();
    if (selectedValue) {
        $select.selectpicker("val", String(selectedValue));
    }
}
function createCalCustomSchemeId() {
    return "cal_scheme_" + Date.now() + "_" + Math.floor(Math.random() * 100000);
}
function copyCalCustomSchemeText(text) {
    var showCopied = function() {
        showAlert("已复制到剪切板", "成功");
    };
...
function exportCalCustomSchemes() {
    var localData = getLocalData();
    var ruleData = findCalCustomSchemeRule(localData.cal, calCustomRule && calCustomRule.id);
    var payload = {
        type: "foodgame-cal-custom-schemes",
        version: 2,
        ruleId: calCustomRule && calCustomRule.id,
        ruleTitle: calCustomRule && (calCustomRule.title || calCustomRule.Title) || "",
        exportedAt: new Date().toISOString(),
        data: ruleData && Array.isArray(ruleData.data) ? ruleData.data : []
    };
    copyCalCustomSchemeText(JSON.stringify(payload, null, 2));
}
function normalizeCalCustomImportedSchemes(value) {
    var source = Array.isArray(value) ? value : (value && Array.isArray(value.data) ? value.data : []);
    var result = [];
    for (var i = 0; i < source.length; i++) {
        ...
        var scheme = JSON.parse(JSON.stringify(item));
        scheme.score = score;
        scheme.name = String(item.name || item.score || "").trim();
        if (!scheme.name) {
            scheme.name = "方案" + (i + 1);
        }
        scheme.schemeId = String(item.schemeId || item.id || createCalCustomSchemeId());
        delete scheme.id;
        result.push(scheme);
    }
    return result;
}
function importCalCustomSchemes(text) {
    try {
            var parsed = JSON.parse(String(text || ""));
            var imported = normalizeCalCustomImportedSchemes(parsed);
            if (!imported.length) {
                showAlert("导入文件中没有有效的计算器方案", "提示");
                return;
            }
            var localData = getLocalData();
            localData.cal = localData.cal || [];
            var ruleData = findCalCustomSchemeRule(localData.cal, calCustomRule && calCustomRule.id);
            if (!ruleData) {
                ruleData = {id: calCustomRule.id, data: []};
                localData.cal.push(ruleData);
            }
            ruleData.data = ruleData.data || [];
            var existingByKey = {};
            for (var i = 0; i < ruleData.data.length; i++) {
                existingByKey[getCalCustomSchemeValue(ruleData.data[i])] = i;
            }
            var added = 0;
            for (var j = 0; j < imported.length; j++) {
                var importedScheme = imported[j];
                var key = getCalCustomSchemeValue(importedScheme);
                if (Object.prototype.hasOwnProperty.call(existingByKey, key)) {
                    ruleData.data[existingByKey[key]] = importedScheme;
                } else {
                    existingByKey[key] = ruleData.data.length;
                    ruleData.data.push(importedScheme);
                    added++;
                }
            }
            updateCalLocalData(localData.cal);
            refreshCalCustomSchemeSelect(ruleData.data, getCalCustomSchemeValue(imported[imported.length - 1]));
            $("#cal-custom-import-modal").modal("hide");
            $("#input-cal-custom-import").val("");
            showAlert("已导入 " + imported.length + " 个方案" + (added ? "，新增 " + added + " 个" : ""), "成功");
        } catch (error) {
            showAlert("方案导入失败：内容不是有效的 JSON 方案", "错误");
        }
}
...
function loadDecorationFromPersonalData(e) {
    var t = e && e.hasOwnProperty("decorationEffect") ? e.decorationEffect : getLocalData().decorationEffect
      , a = Number(t);
    $("#input-cal-decoration").val(isNaN(a) ? "" : a)
}
...
// ==================== 多用户配置系统 ====================
var _currentUserProfile = null; // 缓存当前用户名
var _currentUserIndex = -1;     // 缓存当前用户索引
var _profileMigrated = false;   // 防止重复迁移

function getUserProfiles() {
    try {
        var p = localStorage.getItem("userProfiles");
        if (p) {
            var parsed = JSON.parse(p);
            // 兼容旧数组格式: [{id, name, createdAt}, ...]
            if (Array.isArray(parsed)) {
                var users = [];
                for (var i = 0; i < parsed.length; i++) {
                    if (parsed[i] && parsed[i].name) users.push(parsed[i].name);
                }
                return { currentIndex: users.length > 0 ? 0 : -1, users: users };
            }
            if (!parsed.users) parsed.users = [];
            if (typeof parsed.currentIndex !== "number") parsed.currentIndex = -1;
            return parsed;
        }
        return { currentIndex: -1, users: [] };
    } catch (e) {
        return { currentIndex: -1, users: [] };
    }
}
...
function _getStorageKeyByIndex(idx) {
    return "data_user_" + idx;
}

// 自动迁移：如果旧 "data" key 有数据但还没有用户配置，自动创建"默认用户"
function migrateOldDataIfNeeded() {
    if (_profileMigrated) return;
    _profileMigrated = true;
    
    // 先检查是否是旧数组格式，如果是则转换并保存为新格式
    try {
        var raw = localStorage.getItem("userProfiles");
        if (raw) {
            var parsed = JSON.parse(raw);
            if (Array.isArray(parsed)) {
                // 旧数组格式 [{id, name, createdAt}, ...]，转换为新格式
                var users = [];
                for (var k = 0; k < parsed.length; k++) {
                    if (parsed[k] && parsed[k].name) users.push(parsed[k].name);
                }
                var newProfiles = { currentIndex: users.length > 0 ? 0 : -1, users: users };
                // 迁移旧数组格式用户的数据到索引key
                for (var m = 0; m < parsed.length; m++) {
                    if (parsed[m] && parsed[m].id) {
                        var oldData = localStorage.getItem("data_" + parsed[m].id);
                        if (oldData) {
                            localStorage.setItem(_getStorageKeyByIndex(m), oldData);
                            localStorage.removeItem("data_" + parsed[m].id);
                        }
                    }
                }
                // 如果索引0没数据，尝试从 "data" key 迁移
                if (users.length > 0 && !localStorage.getItem(_getStorageKeyByIndex(0))) {
                    var fallback = localStorage.getItem("data");
                    if (fallback && fallback.length > 2) {
                        localStorage.setItem(_getStorageKeyByIndex(0), fallback);
                    }
                }
                saveUserProfiles(newProfiles);
                _currentUserIndex = newProfiles.currentIndex;
                _currentUserProfile = users.length > 0 ? users[0] : null;
                return;
            }
        }
    } catch (e) {}
    ...
    if (profiles.users.length > 0) {
        if (profiles.currentIndex >= 0 && profiles.currentIndex < profiles.users.length) {
            _currentUserIndex = profiles.currentIndex;
            _currentUserProfile = profiles.users[profiles.currentIndex];
        }
        return;
    }
    try {
        ...
        if (oldData && oldData.length > 2) {
            ...
            profiles.users.push(defaultName);
            profiles.currentIndex = 0;
            localStorage.setItem(_getStorageKeyByIndex(0), oldData);
            _currentUserIndex = 0;
            _currentUserProfile = defaultName;
            saveUserProfiles(profiles);
        }
    } catch (e) {
        try { localStorage.removeItem("userProfiles"); } catch (e2) {}
        _profileMigrated = false;
    }
}

function getCurrentUserName() {
    if (_currentUserProfile) return _currentUserProfile;
    migrateOldDataIfNeeded();
    var p = getUserProfiles();
    if (p.currentIndex >= 0 && p.currentIndex < p.users.length) {
        _currentUserIndex = p.currentIndex;
        _currentUserProfile = p.users[p.currentIndex];
    }
    return _currentUserProfile;
}

function getCurrentStorageKey() {
    getCurrentUserName(); // ensure index is loaded
    if (_currentUserIndex < 0) return "data";
    return _getStorageKeyByIndex(_currentUserIndex);
}

function initUserProfileUI() {
    migrateOldDataIfNeeded();
    var profiles = getUserProfiles();
    var users = profiles.users || [];
    var $select = $("#select-user-profile");
    $select.empty();
    // 二次检查：如果迁移后仍无用户，但旧数据存在，强制再迁移一次
    if (users.length === 0) {
        try {
            var oldData = localStorage.getItem("data");
            if (oldData && oldData.length > 2) {
                var defaultName = "默认用户";
                profiles.users = [defaultName];
                profiles.currentIndex = 0;
                localStorage.setItem(_getStorageKeyByIndex(0), oldData);
                _currentUserIndex = 0;
                _currentUserProfile = defaultName;
    ...
    if (users.length === 0) {
        $select.append("<option value='' selected>无用户</option>");
    } else {
        for (var i = 0; i < users.length; i++) {
            var sel = (i === profiles.currentIndex) ? " selected" : "";
            $select.append("<option value='" + i + "'" + sel + ">" + users[i] + "</option>");
        }
    }
    $select.selectpicker("refresh");
}

function switchToUser(idx, gameData) {
    var profiles = getUserProfiles();
    idx = parseInt(idx);
    if (idx < 0 || idx >= profiles.users.length) return false;
    profiles.currentIndex = idx;
    _currentUserIndex = idx;
    _currentUserProfile = profiles.users[idx];
    saveUserProfiles(profiles);
    // 读取该用户的数据并应用，不刷新页面
    var storageKey = _getStorageKeyByIndex(idx);
    var userData = localStorage.getItem(storageKey);
    if (userData && gameData) {
        var ok = importData(gameData, userData, true);
        if (ok) {
            $.popupmsg({ message: "已切换到用户 \"" + profiles.users[idx] + "\"" });
            try {
                _refreshPartialUltimateAfterImport(gameData);
                updateRecipeChefTable(gameData, false);
            } catch (err) {}
        } else {
            $.popupmsg({ message: "切换用户数据加载失败", type: "alert-danger" });
        }
    } else if (!userData) {
        $.popupmsg({ message: "用户 \"" + profiles.users[idx] + "\" 暂无数据" });
    }
    return true;
}
...
function createUserProfile(userName) {
    var profiles = getUserProfiles();
    profiles.users.push(userName);
    var newIdx = profiles.users.length - 1;
    profiles.currentIndex = newIdx;
    _currentUserIndex = newIdx;
    _currentUserProfile = userName;
    saveUserProfiles(profiles);
    return newIdx;
}

function renameUserProfile(newName) {
    var profiles = getUserProfiles();
    var idx = profiles.currentIndex;
    if (idx < 0 || idx >= profiles.users.length) return false;
    profiles.users[idx] = newName;
    _currentUserProfile = newName;
    saveUserProfiles(profiles);
    // 验证保存成功
    var check = getUserProfiles();
    return true;
}
...
function removeUserProfile() {
    ...
    var idx = profiles.currentIndex;
    if (idx < 0 || idx >= profiles.users.length) return false;
    var userName = profiles.users[idx];
    profiles.users.splice(idx, 1);
    // 删除该用户的数据，并重新排列存储key
    try { localStorage.removeItem(_getStorageKeyByIndex(idx)); } catch (e) {}
    // 将后面的用户数据key前移
    for (var i = idx; i < profiles.users.length; i++) {
        var nextData = localStorage.getItem(_getStorageKeyByIndex(i + 1));
        if (nextData) {
            localStorage.setItem(_getStorageKeyByIndex(i), nextData);
            localStorage.removeItem(_getStorageKeyByIndex(i + 1));
        } else {
            localStorage.removeItem(_getStorageKeyByIndex(i));
        }
    }
    if (profiles.users.length > 0) {
        profiles.currentIndex = 0;
        _currentUserIndex = 0;
        _currentUserProfile = profiles.users[0];
    } else {
...
function updateLocalData(e, a) {
    ...
    t[e] = a;
    try {
        localStorage.setItem(getCurrentStorageKey(), JSON.stringify(t))
    } catch (e) {}
}
function getLocalData() {
    var e;
    try {
        var key = getCurrentStorageKey();
        var a = localStorage.getItem(key);
        e = JSON.parse(a);
        // 兼容：如果当前用户有数据但旧key "data" 也有，优先用用户key
        // 如果用户key没数据且是无用户状态，回退到旧key
        if (!e && !getCurrentUserName()) {
...
function isLocalChefOwnedAndUltimated(localChef) {
    return !!(localChef && ("是" == localChef.ult) && (localChef.got === "是" || localChef.got === true || localChef.got === 1 || localChef.got === "1"));
}

function isLocalChefOwned(localChef) {
    return !!(localChef && (localChef.got === "是" || localChef.got === true || localChef.got === 1 || localChef.got === "1"));
}
...
function isChefUltimated(chefId, localData, configIds) {
    ...
    if (localData && localData.chefs) {
        for (var i = 0; i < localData.chefs.length; i++) {
            if (localData.chefs[i].id == chefId && isLocalChefOwnedAndUltimated(localData.chefs[i])) {
                return true;
            }
        }
    }
...
function mergeHiddenUltimatedChefIdsForCalc(chefs, selectedIds, selectSelector, conditionChecker) {
    var mergedIds = [];
    var mergedSet = {};
    for (var i = 0; i < selectedIds.length; i++) {
        var selectedId = Number(selectedIds[i]);
        mergedIds.push(selectedId);
        mergedSet[String(selectedId)] = true;
    }
    
    var localData = getLocalData();
    var optionSet = {};
    $(selectSelector).find("option").each(function() {
        optionSet[String($(this).val())] = true;
    });
    
    if (!localData || !localData.chefs || !chefs) {
        return mergedIds;
    }
    
    for (var c = 0; c < chefs.length; c++) {
        var chef = chefs[c];
        if (!chef || !chef.chefId || !chef.ultimateSkillEffect || !conditionChecker(chef)) {
            continue;
        }
        
        var chefIdStr = String(chef.chefId);
        if (optionSet[chefIdStr] || mergedSet[chefIdStr]) {
            continue;
        }
        
        for (var lci = 0; lci < localData.chefs.length; lci++) {
            if (localData.chefs[lci].id == chef.chefId && isLocalChefOwnedAndUltimated(localData.chefs[lci])) {
                mergedIds.push(Number(chef.chefId));
                mergedSet[chefIdStr] = true;
                break;
            }
        }
    }
...
function buildLocalDataForUltimateCalc(selectedIds) {
    var localData = getLocalData() || {};
    var calcLocalData = JSON.parse(JSON.stringify(localData));
    calcLocalData.chefs || (calcLocalData.chefs = []);
    
    var localChefMap = {};
    for (var i = 0; i < calcLocalData.chefs.length; i++) {
        localChefMap[String(calcLocalData.chefs[i].id)] = calcLocalData.chefs[i];
    }
    
    for (var j = 0; j < selectedIds.length; j++) {
        var chefId = Number(selectedIds[j]);
        var chefIdStr = String(chefId);
        if (!localChefMap[chefIdStr]) {
            localChefMap[chefIdStr] = {
                id: chefId,
                got: "是",
                ult: "是"
            };
            calcLocalData.chefs.push(localChefMap[chefIdStr]);
        } else {
            localChefMap[chefIdStr].got = "是";
            localChefMap[chefIdStr].ult = "是";
        }
    }
    
    return calcLocalData;
}
...
function getConfigUltimatedChefIds() {
    ...
    var partialIdsSet = {};
    for (var p = 0; p < partialUltimatedChefIds.length; p++) {
        partialIdsSet[String(partialUltimatedChefIds[p])] = true;
    }
    
    var selfIdsSet = {};
    for (var s = 0; s < selfUltimatedChefIds.length; s++) {
        selfIdsSet[String(selfUltimatedChefIds[s])] = true;
    }
    
    var allIdsSet = {};
    for (var i = 0; i < allIds.length; i++) {
        allIdsSet[String(allIds[i])] = true;
    }
    
    return {
        partial: partialUltimatedChefIds,
        partialSet: partialIdsSet,
...
function initCalTables(e, a) {
    ...
    // 初始化排除食材下拉框（按地区分组）
    (function() {
        var matSelect = $("#chk-cal-recipe-material-exclude");
        var regionOrder = ["牧场", "鸡舍", "猪圈", "菜棚", "菜地", "森林", "作坊", "池塘"];
        var regionMats = {};
        for (var ri in regionOrder) regionMats[regionOrder[ri]] = [];
        for (var mi in e.materials) {
        ...
        // 分组默认收起，点击展开/收起（复用项目已有的 collapsible 样式）
        var _matGrpInited = false;
        bsContainer.on("shown.bs.dropdown", function() {
            if (!_matGrpInited) {
                _matGrpInited = true;
    ...
    // 初始化修炼查询模式的厨师分类标签（query.cultivate.js）
    if (typeof initCultivationCategoryTabs === 'function') {
        initCultivationCategoryTabs();
    }
    
    // 初始化修炼查询模式切换开关事件
    $("#chk-cultivation-mode").off("change").on("change", function() {
        var isGodMode = $(this).prop("checked");
        
        // 切换模式时清空场上已选
        if (typeof clearAllSelectedOnField === 'function') {
            clearAllSelectedOnField(e);
        }
        
        // 重置厨师分类状态，以便下次打开时使用正确的默认分类
        if (typeof globalCultivationCategory !== 'undefined') {
            globalCultivationCategory = null;
        }
        
        // 重置菜谱分类状态
        if (typeof globalCultivationRecipeCategory !== 'undefined') {
            globalCultivationRecipeCategory = null;
        }
        
        // 移除已有的分类标签，以便下次打开时重新创建
        $(".cultivation-category-tabs").remove();
        $(".cultivation-recipe-category-tabs").remove();
        
        if (isGodMode) {
            // 神级方案查询模式
            $("#unultimated-chef-wrapper").addClass("hidden");
            $("#unultimated-chef-action-wrapper").addClass("hidden");
            $("#recipe-god-query-wrapper").removeClass("hidden");
            $("#recipe-god-query-action-wrapper").removeClass("hidden");
            // 神级方案模式下隐藏菜谱时间
            $(".recipe-box-1").addClass("no-time");
            // 手机版时，把神级方案选择框和按钮移动到总分上方
            if (isMobile) {
                $("#recipe-god-query-wrapper").appendTo("#mobile-cultivate-container");
                $("#recipe-god-query-action-wrapper").appendTo("#mobile-cultivate-container");
            }
        } else {
            // 修炼查询模式
            $("#unultimated-chef-wrapper").removeClass("hidden");
            $("#unultimated-chef-action-wrapper").removeClass("hidden");
            $("#recipe-god-query-wrapper").addClass("hidden");
            $("#recipe-god-query-action-wrapper").addClass("hidden");
            // 修炼查询模式下恢复菜谱时间显示（根据配置）
            var ddShow = $("#chk-cal-dd-show").val() || [];
            if (ddShow.indexOf("recipe_time") >= 0) {
                $(".recipe-box-1").removeClass("no-time");
            } else {
                $(".recipe-box-1").addClass("no-time");
            }
            // 手机版时，把修炼查询选择框和按钮移动到总分上方
            if (isMobile) {
                $("#unultimated-chef-wrapper").appendTo("#mobile-cultivate-container");
                $("#unultimated-chef-action-wrapper").appendTo("#mobile-cultivate-container");
            }
        }
    ...
    function syncGuestRateSubModeUI(refreshPengciOptions) {
        var isPengciMode = $("#chk-guest-query-mode").prop("checked");
        var isJadeMode = $("#chk-guest-rate-submode").prop("checked");
        
        $("#pengci-mainline-wrapper").toggleClass("hidden", !isPengciMode);
        
        if (isPengciMode) {
            $("#guest-rate-result").addClass("hidden");
            $("#guest-jade-result").addClass("hidden");
            $("#guest-rate-submode-switch-wrapper").addClass("hidden");
            $("#pengci-query-area").removeClass("hidden");
            if (refreshPengciOptions && typeof GuestRateCalculator !== 'undefined' && GuestRateCalculator.populatePengciGuestOptions) {
                GuestRateCalculator.populatePengciGuestOptions();
            }
        } else {
            $("#pengci-query-area").addClass("hidden");
            $("#guest-rate-submode-switch-wrapper").removeClass("hidden");
            $("#guest-rate-result").toggleClass("hidden", isJadeMode);
            $("#guest-jade-result").toggleClass("hidden", !isJadeMode);
        }
    }
    
    // 初始化贵客查询/碰瓷查询模式切换开关事件
    $("#chk-guest-query-mode").off("change").on("change", function() {
        // 切换模式时清空场上已选
        if (typeof clearAllSelectedOnField === 'function') {
            clearAllSelectedOnField(e);
        }

        syncGuestRateSubModeUI(true);
    });
    
    // 初始化贵客查询子模式切换开关事件
    $("#chk-guest-rate-submode").off("change").on("change", function() {
        syncGuestRateSubModeUI(false);
    });
    
    // 初始化碰瓷查询标签页切换事件
    $(".pengci-tabs a[data-toggle='tab']").off("click").on("click", function(e) {
        e.preventDefault();
        var target = $(this).attr("href");
        // 切换标签激活状态
        $(".pengci-tabs li").removeClass("active");
        $(this).parent("li").addClass("active");
        // 切换内容区域
        $(".pengci-tab-content .tab-pane").removeClass("active");
        $(target).addClass("active");
    });
    
    // 初始化碰瓷查询区域收起/展开按钮事件
    $("#pengci-query-area .btn-collapse").off("click").on("click", function() {
        $("#pengci-query-area").toggleClass("collapsed");
    });
    
    // 确保游戏数据已设置到 calCustomRule.gameData，供碰瓷查询使用
    if (e && e.guests && !calCustomRule.gameData) {
        calCustomRule.gameData = e;
    }
    
    // 初始化碰瓷查询贵客选择框（调用 GuestRateCalculator 模块）
    if (typeof GuestRateCalculator !== 'undefined' && GuestRateCalculator.initPengciGuestSelect) {
        GuestRateCalculator.initPengciGuestSelect();
    }
    
    // 初始化符文碰瓷选择框
    if (typeof GuestRateCalculator !== 'undefined' && GuestRateCalculator.initPengciRuneSelect) {
        GuestRateCalculator.initPengciRuneSelect();
    }
}
            ...
            if (t && t.rules && t.rules.length > 0) {
                ...
                for (var l in t.rules) {
                    ...
                    if (s.Id && s.Title) {
                        ...
                        c.Id = s.Id;
                        c.Title = s.Title;
                        c.StartTime = s.start_time ? new Date(s.start_time + "+08:00").getTime() / 1e3 : null;
                        c.EndTime = s.end_time ? new Date(s.end_time + "+08:00").getTime() / 1e3 : null;
                        c.message = s.message || "";
...
function loadHistoryRules(e) {
    if (historyRulesData && historyRulesData.length > 0) {
        ...
        for (var i = 0; i < historyRulesData.length; i++) {
            ...
            // 带有进度标记（如“（10/10）”）的是限时活动，其余归入厨神大赛。
            var historyTag = historyRulesData[i].tag || "";
            var hasProgressMarker = /[（(]\s*\d+\s*\/\s*\d+\s*[）)]/.test(historyTag);
        ...
        $("#btn-cal-history").text("返回当前").removeClass("btn-primary").addClass("btn-primary");
...
function restoreCurrentRules(e) {
    if (originalRules) {
        ...
        $("#select-cal-rule").selectpicker("refresh");
        $("#btn-cal-history").text("历史计算器").removeClass("btn-primary").addClass("btn-primary");
...
function loadHistoricalCalRule(e, historicalRule) {
    ...
    calCustomRule.isCultivate = false;
    ...
    // 隐藏修炼查询模式特有UI
    $("#pane-cal-custom").removeClass("cultivate-mode");
    $("#unultimated-chef-wrapper").addClass("hidden");
    $("#unultimated-chef-action-wrapper").addClass("hidden");
    $("#recipe-god-query-wrapper").addClass("hidden");
    $("#recipe-god-query-action-wrapper").addClass("hidden");
    $("#cultivation-mode-switch-wrapper").addClass("hidden");
    
    // 恢复开关文字
    $("#chk-cal-max-disk").parent().contents().filter(function() {
        return this.nodeType === 3;
    }).last().replaceWith("默认满级心法盘");
    
    // 显示调料选择框
    $(".condiment-box").removeClass("hidden");
    $("#chk-cal-recipe-condiment").closest(".select-wrapper").removeClass("hidden");
    
    // 恢复菜谱时间显示（根据用户设置）
    var ddShow = $("#chk-cal-ui-show").val() || [];
    if (ddShow.indexOf("recipe_time") >= 0) {
        $(".recipe-box-1").removeClass("no-time");
    } else {
        $(".recipe-box-1").addClass("no-time");
    ...
    $(".btn-resize").hide();
    // 先隐藏整个容器，避免闪烁
    $("#pane-cal-custom").css("visibility", "hidden");
    ...
    for (var s = 0; s < calCustomRule.rules.length; s++) {
        ...
        if (c.Satiety) {
            ...
            if (!isMobile) {
                // 桌面版：移动到锁定界面按钮后面（同一行）
                $("#banquet-auto-calc").css({"display": "inline-block", "margin-left": "8px"}).removeClass("hidden");
                $("#chk-cal-lock").closest("label").after($("#banquet-auto-calc"));
            } else {
                $("#banquet-auto-calc").css({"display": "", "margin-left": ""}).removeClass("hidden");
            }
            // 隐藏厨神大赛自动计算按钮
            $("#competition-auto-calc").addClass("hidden").css({"display": "", "margin-left": ""});
            $("#pane-cal-custom").prepend($("#competition-auto-calc"));
        } else {
            $("#pane-cal-custom").removeClass("banquet");
            $(".cal-custom-item:eq(" + s + ") .selected-item:nth-child(-n+3)").show();
            $("#btn-cal-import-run").hide();
            // 隐藏风云宴自动计算按钮并归位
            $("#banquet-auto-calc").addClass("hidden").css({"display": "", "margin-left": ""});
            $("#pane-cal-custom").prepend($("#banquet-auto-calc"));
            // 厨神大赛自动计算按钮（网络规则且无饱食度）
            if (c.isNetworkRule) {
                // 初始化档位下拉框
                var $compTier = $("#select-competition-tier");
                $compTier.empty();
                $compTier.append('<option value="-1">不限制分数</option>');
                if (c.PassLine) {
                    var tierTips = [];
                    if (c.PassLine.length === 1) tierTips = ['过线'];
                    else if (c.PassLine.length === 3) tierTips = ['高保', '中保', '低保'];
                    else if (c.PassLine.length === 4) tierTips = ['高保', '中保', '低保', '达标'];
                    for (var pl = 0; pl < c.PassLine.length; pl++) {
                        var tipLabel = tierTips[pl] || ('档位' + (pl + 1));
                        $compTier.append('<option value="' + c.PassLine[pl] + '">' + tipLabel + ' (' + c.PassLine[pl] + ')</option>');
                    }
                }
                if ($compTier.hasClass('selectpicker')) {
                    $compTier.selectpicker('refresh');
                }
                if (c.PassLine && c.PassLine.length > 0) {
                    var isDefaultCompetitionMode = (new URLSearchParams(window.location.search)).get('chushen') !== '123';
                    if (isDefaultCompetitionMode) {
                        if ($compTier.hasClass('selectpicker')) {
                            $compTier.selectpicker('val', String(c.PassLine[0]));
                        } else {
                            $compTier.val(String(c.PassLine[0]));
                        }
                    }
                }
                // 初始化厨师选择下拉框
                var $compChefs = $("#select-competition-chefs");
                $compChefs.empty();
                if (c.chefs) {
                    for (var chi = 0; chi < c.chefs.length; chi++) {
                        $compChefs.append('<option value="' + c.chefs[chi].chefId + '">' + c.chefs[chi].name + '</option>');
                    }
                }
                if ($compChefs.hasClass('selectpicker')) {
                    $compChefs.selectpicker('refresh');
                }
                if (!isMobile) {
                    $("#competition-auto-calc").css({"display": "inline-block", "margin-left": "8px"}).removeClass("hidden");
                    $("#chk-cal-lock").closest("label").after($("#competition-auto-calc"));
                } else {
                    $("#competition-auto-calc").css({"display": "", "margin-left": ""}).removeClass("hidden");
                }
                if (!$("#chk-cal-got").prop("checked")) {
                    $("#chk-cal-got").prop("checked", true).trigger("change");
                    changeCheckStyle($("#chk-cal-got")[0]);
                }
            } else {
                $("#competition-auto-calc").addClass("hidden").css({"display": "", "margin-left": ""});
                $("#pane-cal-custom").prepend($("#competition-auto-calc"));
    ...
    // 在循环结束后初始化贵客选择下拉框（只调用一次）
    if ($("#pane-cal-custom").hasClass("banquet")) {
        initBanquetGuestSelector();
    }
    ...
    for (var t in p.cal) {
        if (p.cal[t].id == calCustomRule.id) {
            for (var i in p.cal[t].data) {
                d += getCalCustomSchemeOption(p.cal[t].data[i]);
            }
    ...
    setTimeout(function() {
        ...
        // loadRule后填充厨师选择下拉框（chefs在loadRule中才被设置）
        if (calCustomRule.rules[0] && calCustomRule.rules[0].chefs && !calCustomRule.rules[0].Satiety) {
            var $compChefsPost = $("#select-competition-chefs");
            $compChefsPost.empty();
            for (var _chi = 0; _chi < calCustomRule.rules[0].chefs.length; _chi++) {
                $compChefsPost.append('<option value="' + calCustomRule.rules[0].chefs[_chi].chefId + '">' + calCustomRule.rules[0].chefs[_chi].name + '</option>');
            }
            if ($compChefsPost.hasClass('selectpicker')) {
                $compChefsPost.selectpicker('refresh');
            }
        }
        _refreshChefComboSelect();
        
        // 初始化"查看前10名"按钮（在规则加载完成后）
        // 只在厨神大赛规则下显示（非风云宴且是网络规则）
        if (calCustomRule.rules[0] && !calCustomRule.rules[0].Satiety && calCustomRule.rules[0].isNetworkRule) {
            if (typeof CompetitionTop10UI !== 'undefined' && CompetitionTop10UI.showButton) {
                CompetitionTop10UI.showButton();
            }
        }
        ...
        initUnultimatedChefDropdown(e);
        initRecipeGodQueryDropdown(e);
        // 恢复容器可见性
        $("#pane-cal-custom").css("visibility", "visible");
        
        // 显示规则提示信息
        if (historicalRule.message) {
            showAlert(historicalRule.message);
        }
    }, 0);
}
function initCalRulesLocal(e, a) {
    ...
    $(document).on("shown.bs.select", "#select-cal-rule", function() {
        var $dropdown = $(this).parent().find(".dropdown-menu");
    ...
    $("#input-cal-decoration").val(e.hasOwnProperty("decorationEffect") ? e.decorationEffect : ""),
    loadUltimate(e, !0),
    ...
    $("#chk-cal-partial-ultimate").selectpicker().on("changed.bs.select", function() {
        if (isApplyingUltimateSelection) return;
        isAllUltimateMode = false;
        setCalConfigData(e),
        calCustomResults(e);
        // 刷新排序（已勾选的优先显示）
        if (typeof refreshCalPartialUltimateSort === 'function') {
    ...
    $("#select-cal-chef-order").change(function() {
        updateMenuLocalData()
    }),
    ...
    $("#btn-cal-reset-custom").click(function() {
        ...
        $("#chk-cal-recipe-material-exclude").selectpicker("deselectAll"),
        clearExcludeMaterialQuantityMap("#chk-cal-recipe-material-exclude"),
        checkMonitorStyle()
    }),
    // 清空场上已选按钮
    $("#btn-cal-clear-selected").click(function() {
        clearAllSelectedOnField(e);
    }),
    $("#chk-cal-got").change(function() {
        ...
        // 刷新未修炼厨师列表
        if (typeof refreshUnultimatedChefList === 'function' && calCustomRule.rules && calCustomRule.rules[0]) {
            refreshUnultimatedChefList(null);
        }
        // 刷新菜谱神级方案列表
        if (typeof refreshRecipeGodQueryList === 'function' && calCustomRule.rules && calCustomRule.rules[0]) {
            refreshRecipeGodQueryList(e);
        }
    }),
            ...
            for (var i in t.cal)
                if (t.cal[i].id == calCustomRule.id) {
                    for (var l in t.cal[i].data)
                        if (getCalCustomSchemeValue(t.cal[i].data[l]) == String(a)) {
            ...
            callback: function(a) {
                if (a) {
                    ...
                    for (var i in t.cal)
                        if (t.cal[i].id == calCustomRule.id) {
                    for (var l in t.cal[i].data)
                                if (getCalCustomSchemeValue(t.cal[i].data[l]) == String(e)) {
                                    t.cal[i].data.splice(l, 1),
                                    refreshCalCustomSchemeSelect(t.cal[i].data);
                                    break
                                }
                            break
                        }
    ...
    $("#btn-cal-custom-save").click(function() {
        if (calCustomRule.score) {
            bootbox.prompt({
                ...
                placeholder: "请输入方案名称",
                buttons: {
                    confirm: {label: "保存", className: "btn-primary"},
                    cancel: {label: "取消", className: "btn-default"}
                },
                callback: function(name) {
                    name = $.trim(String(name || ""));
                    if (!name) {
                        showAlert("方案名称不能为空", "提示");
                        return;
                    }

                    var saveScheme = function(overwriteIndex) {
                        var localData = getLocalData();
                        localData.cal = localData.cal || [];
                        var ruleData = findCalCustomSchemeRule(localData.cal, calCustomRule.id);
                        if (!ruleData) {
                            ruleData = {id: calCustomRule.id, data: []};
                            localData.cal.push(ruleData);
                        }
                        ...
                        var scheme = {
                            schemeId: overwriteIndex >= 0
                                ? getCalCustomSchemeValue(ruleData.data[overwriteIndex])
                                : createCalCustomSchemeId(),
                            name: name,
                            score: calCustomRule.score,
                            menu: []
                        };
                        for (var ruleIndex in calCustomRule.rules) {
                            var menuItems = [];
                            var customRules = calCustomRule.rules[ruleIndex].custom || {};
                            for (var customIndex in customRules) {
                                var custom = customRules[customIndex];
                                var item = {
                                    chef: custom.chef && custom.chef.chefId,
                                    equip: custom.equip && custom.equip.equipId,
                                    condiment: custom.condiment && custom.condiment.condimentId,
                                    recipes: []
                                };
                                if (custom.chef && custom.chef.chefId) {
                                    item.dlv = custom.chef.disk && custom.chef.disk.level;
                                    item.ambers = [];
                                    for (var amberIndex in (custom.chef.disk && custom.chef.disk.ambers || {})) {
                                        var amberId = 0;
                                        var amber = custom.chef.disk.ambers[amberIndex];
                                        if (amber && amber.data) amberId = amber.data.amberId;
                                        item.ambers.push(amberId);
                                    }
                                }
                                for (var recipeIndex in (custom.recipes || {})) {
                                    var recipe = custom.recipes[recipeIndex];
                                    var savedRecipe = {};
                                    if (recipe.data) {
                                        savedRecipe.id = recipe.data.recipeId;
                                        savedRecipe.qty = recipe.quantity;
                                        savedRecipe.cdt = recipe.useCondiment;
                                    }
                                    item.recipes.push(savedRecipe);
                                }
                                menuItems.push(item);
                            }
                            scheme.menu.push(menuItems);
                        }

                        if (overwriteIndex >= 0) {
                            ruleData.data[overwriteIndex] = scheme;
                        } else {
                            ruleData.data.push(scheme);
                        }
                        updateCalLocalData(localData.cal);
                        refreshCalCustomSchemeSelect(ruleData.data, scheme.schemeId);
                        showAlert("方案已保存", "成功");
                    };

                    var existingData = findCalCustomSchemeRule(getLocalData().cal, calCustomRule.id);
                    var existingIndex = -1;
                    if (existingData && Array.isArray(existingData.data)) {
                        for (var existingSchemeIndex = 0; existingSchemeIndex < existingData.data.length; existingSchemeIndex++) {
                            if ($.trim(getCalCustomSchemeLabel(existingData.data[existingSchemeIndex])) === name) {
                                existingIndex = existingSchemeIndex;
                                break;
                            }
                        }
                    }
                    if (existingIndex >= 0) {
                        bootbox.confirm({
                            size: "small",
                            message: "方案“" + escapeHtml(name) + "”已存在，是否覆盖？",
                            locale: "zh_CN",
                            callback: function(confirmed) {
                                if (confirmed) saveScheme(existingIndex);
                            }
                        });
                    } else {
                        saveScheme(-1);
    ...
    $("#btn-cal-custom-export").click(function() {
        exportCalCustomSchemes();
    }),
    $("#btn-cal-custom-import").click(function() {
        $("#input-cal-custom-import").val("");
        $("#cal-custom-import-modal").modal("show");
    }),
    $("#btn-cal-custom-import-confirm").click(function() {
        importCalCustomSchemes($("#input-cal-custom-import").val());
    }),
    ...
    if (!a || !a.menu || !(isMobile ? a.menu.calmui : a.menu.calui)) {
        var o = [];
        o.push("chef_icon"),
        o.push("amber"),
        o.push("amber_icon"),
...
function loadCalRule(e) {
    ...
    if (selectedValue === 'guest-rate') {
        ...
        calCustomRule.isCultivate = false;
    } else if (selectedValue === 'cultivate') {
        // 修炼查询使用与正常营业相同的逻辑
        selectedValue = 0;
        calCustomRule.isGuestRate = false;
        calCustomRule.isCultivate = true; // 标记为修炼查询模式
    } else {
        calCustomRule.isGuestRate = false;
        calCustomRule.isCultivate = false;
    }
    ...
    // 显示或隐藏贵客率计算结果区域（移到 setTimeout 中执行，避免闪烁）
    // 这里只设置模式标记，实际的显示/隐藏在 setTimeout 中执行
    var shouldShowGuestRateResult = false;
    var shouldShowGuestJadeResult = false;
    var shouldShowPengciArea = false;
    if (calCustomRule.isGuestRate) {
        var isPengciMode = $("#chk-guest-query-mode").prop("checked");
        var isJadeMode = $("#chk-guest-rate-submode").prop("checked");
        shouldShowGuestRateResult = !isPengciMode && !isJadeMode;
        shouldShowGuestJadeResult = !isPengciMode && isJadeMode;
        shouldShowPengciArea = isPengciMode;
        
        // 贵客率模式显示贵客查询/碰瓷查询切换开关
        $("#guest-query-mode-switch-wrapper").removeClass("hidden");
        // 手机版时，把开关移动到总分上方
        if (isMobile) {
            $("#guest-query-mode-switch-wrapper").appendTo("#mobile-mode-switch-container");
            $("#guest-rate-submode-switch-wrapper").appendTo("#mobile-mode-switch-container");
            $("#pengci-mainline-wrapper").appendTo("#mobile-mode-switch-container");
            $("#mobile-mode-switch-container").show();
        } else {
            // 桌面版时，确保开关在原位置
            $("#guest-query-mode-switch-wrapper").insertAfter("#cultivation-mode-switch-wrapper");
            $("#guest-rate-submode-switch-wrapper").insertAfter("#guest-query-mode-switch-wrapper");
            // 桌面版时，把绑定主线输入框移回原位置
            $("#pengci-mainline-wrapper").appendTo("#pengci-guest-tab .pengci-guest-controls");
            $("#mobile-mode-switch-container").hide();
        }
        syncGuestRateSubModeUI(false);
        ...
        $(".condiment-box").removeClass("hidden");
        // 贵客率模式隐藏菜谱调料选择框
        $("#chk-cal-recipe-condiment").closest(".select-wrapper").addClass("hidden");
        // 恢复菜谱时间显示（根据用户设置）
        var ddShow = $("#chk-cal-ui-show").val() || [];
        if (ddShow.indexOf("recipe_time") >= 0) {
            $(".recipe-box-1").removeClass("no-time");
    ...
    } else if (calCustomRule.isCultivate) {
        // 修炼查询模式：直接显示未修炼厨师选择、查询按钮、配置按钮
        $("#guest-rate-result").addClass("hidden");
        $("#guest-jade-result").addClass("hidden");
        $("#pengci-query-area").addClass("hidden");
        $("#pane-cal-custom").removeClass("guest-rate-mode");
        $("#pane-cal-custom").addClass("cultivate-mode");
        // 修炼查询模式隐藏贵客查询/碰瓷查询切换开关
        ...
        // 显示模式切换开关
        $("#cultivation-mode-switch-wrapper").removeClass("hidden");
        // 手机版时，把修炼/神级方案开关移动到总分上方
        if (isMobile) {
            $("#cultivation-mode-switch-wrapper").appendTo("#mobile-cultivate-container");
            $("#mobile-cultivate-container").removeClass("hidden");
        } else {
            // 桌面版时，确保开关在原位置
            $("#cultivation-mode-switch-wrapper").insertAfter("#btn-cal-clear-custom");
            $("#mobile-mode-switch-container").hide();
        }
        // 根据当前模式显示对应的查询UI
        ...
        $("#pane-cal-custom").removeClass("guest-rate-mode");
        $("#pane-cal-custom").removeClass("cultivate-mode");
        // 正常营业模式隐藏贵客查询/碰瓷查询切换开关，并隐藏顶部容器
        $("#guest-query-mode-switch-wrapper").addClass("hidden");
        $("#guest-rate-submode-switch-wrapper").addClass("hidden");
        $("#mobile-mode-switch-container").hide();
        $("#mobile-cultivate-container").addClass("hidden");
        // 非贵客率模式下，恢复开关文字
        $("#chk-cal-max-disk").parent().contents().filter(function() {
            return this.nodeType === 3;
        }).last().replaceWith("默认满级心法盘");
        // 正常营业模式：隐藏修炼查询相关控件，并移回原位置
        $("#unultimated-chef-wrapper").addClass("hidden").insertAfter("#guest-query-mode-switch-wrapper");
        $("#unultimated-chef-action-wrapper").addClass("hidden").insertAfter("#unultimated-chef-wrapper");
        // 隐藏菜谱神级方案查询UI，并移回原位置
        $("#recipe-god-query-wrapper").addClass("hidden").insertAfter("#unultimated-chef-action-wrapper");
        $("#recipe-god-query-action-wrapper").addClass("hidden").insertAfter("#recipe-god-query-wrapper");
        // 隐藏模式切换开关，并移回原位置
        $("#cultivation-mode-switch-wrapper").addClass("hidden").insertAfter("#btn-cal-clear-custom");
        $("#guest-rate-submode-switch-wrapper").addClass("hidden").insertAfter("#guest-query-mode-switch-wrapper");
        // 正常营业模式显示调料选择框
        $(".condiment-box").removeClass("hidden");
        // 正常营业模式显示菜谱调料选择框
        $("#chk-cal-recipe-condiment").closest(".select-wrapper").removeClass("hidden");
    ...
    for (var s = 0; s < calCustomRule.rules.length; s++) {
        ...
        if (c.Satiety) {
            ...
            // 厨神大赛自动计算按钮（网络规则且无饱食度）
            if (c.isNetworkRule) {
                var $compTier2 = $("#select-competition-tier");
                $compTier2.empty();
                $compTier2.append('<option value="-1">不限制分数</option>');
                if (c.PassLine) {
                    var tierTips2 = [];
                    if (c.PassLine.length === 1) tierTips2 = ['过线'];
                    else if (c.PassLine.length === 3) tierTips2 = ['高保', '中保', '低保'];
                    else if (c.PassLine.length === 4) tierTips2 = ['高保', '中保', '低保', '达标'];
                    for (var pl2 = 0; pl2 < c.PassLine.length; pl2++) {
                        var tipLabel2 = tierTips2[pl2] || ('档位' + (pl2 + 1));
                        $compTier2.append('<option value="' + c.PassLine[pl2] + '">' + tipLabel2 + ' (' + c.PassLine[pl2] + ')</option>');
                    }
                }
                if ($compTier2.hasClass('selectpicker')) {
                    $compTier2.selectpicker('refresh');
                }
                if (c.PassLine && c.PassLine.length > 0) {
                    var isDefaultCompetitionMode2 = (new URLSearchParams(window.location.search)).get('chushen') !== '123';
                    if (isDefaultCompetitionMode2) {
                        if ($compTier2.hasClass('selectpicker')) {
                            $compTier2.selectpicker('val', String(c.PassLine[0]));
                        } else {
                            $compTier2.val(String(c.PassLine[0]));
                        }
                    }
                }
                // 初始化厨师选择下拉框
                var $compChefs2 = $("#select-competition-chefs");
                $compChefs2.empty();
                if (c.chefs) {
                    for (var chi2 = 0; chi2 < c.chefs.length; chi2++) {
                        $compChefs2.append('<option value="' + c.chefs[chi2].chefId + '">' + c.chefs[chi2].name + '</option>');
                    }
                }
                if ($compChefs2.hasClass('selectpicker')) {
                    $compChefs2.selectpicker('refresh');
                }
                if (!isMobile) {
                    $("#competition-auto-calc").css({"display": "inline-block", "margin-left": "8px"}).removeClass("hidden");
                    $("#chk-cal-lock").closest("label").after($("#competition-auto-calc"));
    ...
    setTimeout(function() {
        ...
        // loadRule后填充厨师选择下拉框（chefs在loadRule中才被设置）
        if (calCustomRule.rules[0] && calCustomRule.rules[0].chefs && !calCustomRule.rules[0].Satiety) {
            var $compChefsPost2 = $("#select-competition-chefs");
            $compChefsPost2.empty();
            for (var _chi2 = 0; _chi2 < calCustomRule.rules[0].chefs.length; _chi2++) {
                $compChefsPost2.append('<option value="' + calCustomRule.rules[0].chefs[_chi2].chefId + '">' + calCustomRule.rules[0].chefs[_chi2].name + '</option>');
            }
            if ($compChefsPost2.hasClass('selectpicker')) {
                $compChefsPost2.selectpicker('refresh');
            }
        }
        _refreshChefComboSelect();
        // 重新计算ultimateData，确保厨师页面修改"已有"状态后七侠加成能及时生效
        var _localData = getLocalData();
        var _applyUltimate = !0;
        var _applyUltimatePerson = $("#chk-cal-self-ultimate").val() && $("#chk-cal-self-ultimate").val().length > 0;
        e.ultimateData = getUltimateData(e.chefs, _localData, _applyUltimate, _applyUltimatePerson, e.skills);
        setCalConfigData(e);
        $("#pane-cal-recipes").attr("data-cal", "false");
        $(".loading").addClass("hidden");
        $(".cal-menu").removeClass("hidden");
        ...
        // 在恢复可见性之前，设置贵客率计算结果区域的显示状态（避免闪烁）
        if (shouldShowGuestRateResult) {
            $("#guest-rate-result").removeClass("hidden");
            $("#guest-jade-result").addClass("hidden");
            $("#pengci-query-area").addClass("hidden");
            $("#guest-rate-submode-switch-wrapper").removeClass("hidden");
        } else if (shouldShowGuestJadeResult) {
            $("#guest-rate-result").addClass("hidden");
            $("#guest-jade-result").removeClass("hidden");
            $("#pengci-query-area").addClass("hidden");
            $("#guest-rate-submode-switch-wrapper").removeClass("hidden");
        } else if (shouldShowPengciArea) {
            $("#guest-rate-result").addClass("hidden");
            $("#guest-jade-result").addClass("hidden");
            $("#pengci-query-area").removeClass("hidden");
            $("#guest-rate-submode-switch-wrapper").addClass("hidden");
            // 刷新碰瓷查询的贵客统计（确保使用最新的菜谱数据）
            if (typeof GuestRateCalculator !== 'undefined' && GuestRateCalculator.populatePengciGuestOptions) {
                GuestRateCalculator.populatePengciGuestOptions();
            }
        } else {
        ...
        // 显示网络规则的提示信息
        if (calCustomRule.rules[0] && calCustomRule.rules[0].isNetworkRule && calCustomRule.rules[0].message) {
            showAlert(calCustomRule.rules[0].message);
        }
    }, 0)
}
...
function loadUltimate(e, a) {
    ...
    isAllUltimateMode = !a;
    isApplyingUltimateSelection = !0;
    a && loadDecorationFromPersonalData(e);
    ...
    isApplyingUltimateSelection = !1;
    // 保存ultimateData到e对象，以便计算器页面可以使用七侠技法加成
    e.ultimateData = i;
    
    // 刷新未修炼厨师列表（全修炼模式变化后需要更新）
    if (typeof refreshUnultimatedChefList === 'function' && calCustomRule.rules && calCustomRule.rules[0]) {
        refreshUnultimatedChefList(e);
    }
}
function setCalConfigData(e) {
    ...
    for (var a in calCustomRule.rules) {
        ...
        for (var R in I)
            I[R] = Number(I[R]);
        I = mergeHiddenUltimatedChefIdsForCalc(e.chefs, I, "#chk-cal-partial-ultimate", function(chef) {
            for (var i = 0; i < chef.ultimateSkillEffect.length; i++) {
                if ("Partial" == chef.ultimateSkillEffect[i].condition || "Next" == chef.ultimateSkillEffect[i].condition) {
                    return true;
                }
            }
            return false;
        ...
        for (var R in N)
            N[R] = Number(N[R]);
        N = mergeHiddenUltimatedChefIdsForCalc(e.chefs, N, "#chk-cal-self-ultimate", function(chef) {
            for (var i = 0; i < chef.ultimateSkillEffect.length; i++) {
                if ("Self" == chef.ultimateSkillEffect[i].condition) {
                    return true;
                }
            }
            return false;
        });
        var qixiaSelectedIds = I.concat(N);
        var qixiaLocalData = buildLocalDataForUltimateCalc(qixiaSelectedIds);
        var qixiaUltimateData = getUltimateData(e.chefs, qixiaLocalData, !0, !isAllUltimateMode, e.skills);
        var A = getSelfUltimateData(e.chefs, !0, N)
          , M = getActivityConfigData(t)
          , Q = qixiaUltimateData && qixiaUltimateData.qixia ? qixiaUltimateData.qixia : null;
...
function clearAllSelectedOnField(gameData) {
    for (var ri = 0; ri < calCustomRule.rules.length; ri++) {
        var rule = calCustomRule.rules[ri];
        if (!rule || !rule.custom) continue;
        for (var i = 0; i < rule.custom.length; i++) {
            setCustomChef(ri, i, null);
            setCustomEquip(ri, i, null);
            setCustomCondiment(ri, i, null, gameData);
            for (var j = 0; j < 3; j++) {
                setCustomRecipe(ri, i, j, null);
            }
        }
    }
                ...
                if (i.chefs[s].chefId == t) {
                    ...
                    if ($("#chk-cal-max-disk").prop("checked")) {
                        ...
                        if (isGuestRateModeFlag) {
                            ...
                            // 先清空所有遗玉
                            for (var c in l[a].chef.disk.ambers) {
                                setCustomAmber(e, a, c, null);
                            }
                            // 然后为蓝色槽位设置默认蓝色遗玉（amberId=75）
                            for (var c in l[a].chef.disk.ambers) {
                                if (l[a].chef.disk.ambers[c].type === 3) {
                                    setCustomAmber(e, a, c, 75);
                                }
                            }
                        } else if (!$("#chk-cal-use-amber").prop("checked")) {
                            // 非贵客率模式 + 不勾选"已配遗玉"：只清空遗玉，不设置默认蓝色遗玉
                            // 遗玉已经在上面清空了，这里不需要额外操作
                        }
                        // 非贵客率模式 + 勾选"已配遗玉"：保留已配遗玉，只设置满级（已在上面设置）
                    }
...
function setCustomEquip(e, a, t) {
    var i = calCustomRule.rules[e]
      , l = i.custom;
    if (l[a]) {
        var equipInfo = t ? getEquipInfo(t, i.equips) : {};
        l[a].equip = equipInfo;
    }
}
...
function setCustomRecipe(e, a, t, i) {
    ...
    if (s[a]) {
        s[a].recipes[t] = {};
        // 先更新所有厨师数据，确保 materialEffects 包含 Partial 效果（如冬花的食材减少）
        var partialAdds = getPartialChefAdds(s, l);
        for (var _c in s) {
            if (s[_c].chef.chefId) {
                setDataForChef(s[_c].chef, s[_c].equip, !0, l.calGlobalUltimateData, partialAdds[_c], l.calSelfUltimateData, l.calActivityUltimateData, !0, l, !0, l.calQixiaData || null);
            } else if (partialAdds[_c]) {
                // 没有厨师的位置，也需要收集 Partial 效果
                s[_c].chef.materialEffects = [];
                var _mr = [];
                for (var _p in partialAdds[_c]) {
                    var _eff = partialAdds[_c][_p];
                    if (_eff.type === "MaterialReduce") {
                        _mr.push(_eff);
                    }
                }
                if (_mr.length > 0) s[_c].chef.materialEffects = _mr;
            } else {
                s[_c].chef.materialEffects = [];
            }
        }
        ...
        if (i)
            for (var o in l.menus)
                if (l.menus[o].recipe.data.recipeId == i) {
                    ...
                    // 修炼模式下：根据修炼任务要求份数自动计算
                    var isUnultimatedMode = calCustomRule && calCustomRule.isCultivate === true;
                    if (isUnultimatedMode && typeof getQuestRemainingQuantity === 'function') {
                        var chef = s[a].chef;
                        if (chef && chef.chefId) {
                            var remainingQty = getQuestRemainingQuantity(e, a, t, chef.chefId);
                            if (remainingQty !== null && remainingQty >= 0) {
                                // 取剩余需要份数和可制作份数的较小值
                                p = Math.min(p, remainingQty);
                            }
                        }
                    }
                    ...
                    d.quantity = p;
                    // 查询必来模式按所选菜谱星级和当前场上贵客率反推必来份数。
                    var isQueryMustMode = calCustomRule && calCustomRule.isGuestRate === true &&
                        typeof OneClickQuery !== 'undefined' && OneClickQuery &&
                        typeof OneClickQuery.getSetting === 'function' &&
                        OneClickQuery.getSetting('queryMode') === false;
                    if (isQueryMustMode && typeof GuestRateCalculator !== 'undefined' &&
                        GuestRateCalculator.calculateFields &&
                        typeof OneClickQuery.getRequiredPortionsForStarLevel === 'function') {
                        var recipeStarLevel = Number(d.data.rarity) || 5;
                        var recipeMinQuantity = ({1: 20, 2: 15, 3: 12, 4: 10, 5: 7})[recipeStarLevel] || 7;
                        var currentQualityLevel = $("#quality-level").val() || "1";
                        var currentGuestFields = GuestRateCalculator.calculateFields(s, l, recipeStarLevel, recipeMinQuantity, currentQualityLevel);
                        var currentGuestRate = Number(currentGuestFields && currentGuestFields.guestRate) || 0;
                        var requiredQuantity = OneClickQuery.getRequiredPortionsForStarLevel(currentGuestRate, recipeStarLevel);
                        var maxRecipeQuantity = Number(p);
                        if (typeof OneClickQuery.getUserMaxQuantity === 'function') {
                            var maxStarQuantity = Number(OneClickQuery.getUserMaxQuantity(recipeStarLevel));
                            if (isFinite(maxStarQuantity)) {
                                maxRecipeQuantity = Math.min(maxRecipeQuantity, maxStarQuantity);
                            }
                        }
                        if (isFinite(maxRecipeQuantity)) {
                            d.quantity = Math.min(requiredQuantity, Math.max(0, maxRecipeQuantity));
                        } else {
                            d.quantity = requiredQuantity;
                        }
                    } else if (calCustomRule && calCustomRule.isGuestRate === true) {
                        // 其他贵客/刷玉模式沿用上方对应星级的份数设置。
                        var configuredStarLevel = parseInt($("#star-level").val(), 10);
                        var configuredQuantity = parseInt($("#quantity-value").val(), 10);
                        if (Number(d.data.rarity) === configuredStarLevel && isFinite(configuredQuantity) && configuredQuantity >= 1) {
                            d.quantity = configuredQuantity;
                        }
                    }
                    s[a].recipes[t] = d;
                    break
...
var normalBusinessMaterialDetailsMap = {};
var normalBusinessMaterialIntervalMap = {};
function formatDailyMaterialAmount(amount) {
    var truncated = Math.floor(amount * 100) / 100;
    if (Math.abs(truncated - Math.round(truncated)) < 1e-9) {
        return String(Math.round(truncated));
    }
    return truncated.toFixed(2).replace(/\.?0+$/, "");
}
function getDailyMaterialDetails(custom, rule, totalTimeSeconds, intervalMinutes) {
    if (!custom || !rule || !totalTimeSeconds || totalTimeSeconds <= 0)
        return {
            totalMaterials: [],
            recipeDetails: [],
            totalDailyAmount: 0
        };
    var intervalSeconds = Math.max(0, Math.min(60, parseInt(intervalMinutes, 10) || 0)) * 60
      , dailyCycles = 86400 / (totalTimeSeconds + intervalSeconds)
      , materialNames = {}
      , materialTotals = {}
      , materialOrder = []
      , recipeDetails = []
    ...
    if (rule.materials)
        for (var i = 0; i < rule.materials.length; i++)
            rule.materials[i] && rule.materials[i].materialId && (materialNames[String(rule.materials[i].materialId)] = rule.materials[i].name || ("食材" + rule.materials[i].materialId));
    for (var c in custom)
        if (custom.hasOwnProperty(c)) {
            var chef = custom[c].chef
              , recipes = custom[c].recipes || [];
            for (var r in recipes)
                if (recipes.hasOwnProperty(r)) {
                    var recipe = recipes[r]
                      , recipeData = recipe && recipe.data
                      , recipeQuantity = parseInt(recipe && recipe.quantity, 10) || 0;
                    if (recipeData && recipeData.materials && recipeData.materials.length && recipeQuantity > 0) {
                        var recipeDetail = {
                        ...
                        for (var m = 0; m < recipeData.materials.length; m++) {
                            var material = recipeData.materials[m];
                            if (material) {
                                var materialId = String(material.material)
                                  , perPortionAmount = Number(material.quantity) || 0
                                  , reducedAmount = "function" == typeof calMaterialReduce ? calMaterialReduce(chef, material.material, perPortionAmount) : perPortionAmount
                                  , perCycleAmount = reducedAmount * recipeQuantity
                                  , dailyAmount = dailyCycles * reducedAmount * recipeQuantity;
                                materialTotals[materialId] || (materialTotals[materialId] = 0,
                                materialOrder.push(materialId));
                                var materialName = materialNames[materialId] || ("食材" + materialId);
                                recipeDetail.materials.push({
                                    materialId: materialId,
                                    materialName: materialName,
                                    perPortionAmount: reducedAmount,
                                    perCycleAmount: perCycleAmount,
                                    dailyAmount: dailyAmount
                                }),
                                recipeDetail.totalDailyAmount += dailyAmount,
                                totalDailyAmount += dailyAmount,
                                materialTotals[materialId] += dailyAmount
                            }
                        }
    ...
    var totalMaterials = [];
    for (var index = 0; index < materialOrder.length; index++) {
        var summaryMaterialId = materialOrder[index]
          , summaryName = materialNames[summaryMaterialId] || ("食材" + summaryMaterialId);
        totalMaterials.push({
            materialId: summaryMaterialId,
            materialName: summaryName,
            dailyAmount: materialTotals[summaryMaterialId]
        });
    }
    return totalMaterials.sort(function(a, b) {
        return b.dailyAmount - a.dailyAmount;
    }),
    recipeDetails.sort(function(a, b) {
        if (b.totalDailyAmount !== a.totalDailyAmount)
...
function getNormalMaterialIntervalMinutes(ruleIndex) {
    if (normalBusinessMaterialIntervalMap.hasOwnProperty(ruleIndex))
        return normalBusinessMaterialIntervalMap[ruleIndex];
    var $dropdown = $("#normal-material-interval-select-" + ruleIndex)
      , value = parseInt($dropdown.attr("data-value"), 10);
    return (isNaN(value) || value < 0) && (value = 0),
    value > 60 && (value = 60),
    $dropdown.length && $dropdown.attr("data-value", value),
    isNaN(value) || value < 0 ? 0 : Math.min(60, value);
}
function getMaterialIntervalDropdownHtml(selectedValue) {
    var value = parseInt(selectedValue, 10);
    (isNaN(value) || value < 0) && (value = 0),
    value > 60 && (value = 60);
    for (var html = '<button class="btn btn-default btn-sm dropdown-toggle material-interval-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' + value + '分钟 <span class="caret"></span></button><ul class="dropdown-menu material-interval-menu">', i = 0; i <= 60; i++)
        html += '<li' + (i === value ? ' class="active"' : '') + '><a href="#" class="material-interval-option" data-value="' + i + '">' + i + "分钟</a></li>";
    html += "</ul>";
    return html;
}
function getDailyMaterialSummaryHtml(ruleIndex, materialDetails, intervalMinutes) {
    var selectId = "normal-daily-material-select-" + ruleIndex
      , intervalSelectId = "normal-material-interval-select-" + ruleIndex
      , buttonDisabled = !materialDetails || !materialDetails.recipeDetails || !materialDetails.recipeDetails.length
      , html = '<div class="selected-sum-inline-materials selected-sum-material-row"><span class="material-label">开业间隔：</span><div id="' + intervalSelectId + '" class="dropdown material-interval-dropdown normal-material-interval-select" data-value="' + (intervalMinutes || 0) + '">' + getMaterialIntervalDropdownHtml(intervalMinutes) + '</div><span class="material-label">日均耗材：</span><select id="' + selectId + '" class="selectpicker material-daily-select normal-material-select" data-width="auto" data-container="body" data-none-selected-text="无耗材">';
    if (!materialDetails || !materialDetails.totalMaterials || !materialDetails.totalMaterials.length)
        html += '<option value="">无耗材</option>';
    else
        for (var i = 0; i < materialDetails.totalMaterials.length; i++) {
            var item = materialDetails.totalMaterials[i];
            html += '<option value="' + escapeHtml(String(item.materialId)) + '">' + escapeHtml(item.materialName + " " + formatDailyMaterialAmount(item.dailyAmount) + "/天") + "</option>";
        }
    return html += '</select><button class="btn btn-default btn-sm btn-normal-material-detail" type="button" data-rule-index="' + ruleIndex + '"' + (buttonDisabled ? " disabled" : "") + '>耗材详情</button></div>';
}
function renderNormalBusinessMaterialDetailModal(ruleIndex) {
    var $body = $("#normal-material-detail-body")
      , materialDetails = normalBusinessMaterialDetailsMap[ruleIndex];
    if (!$body.length)
        return;
    if (!materialDetails || !materialDetails.recipeDetails || !materialDetails.recipeDetails.length) {
        $body.html('<div class="jade-material-detail-empty">暂无耗材数据</div>');
        return;
    }
    for (var html = '<div class="jade-material-recipe-grid">', i = 0; i < materialDetails.recipeDetails.length; i++) {
        ...
        for (var j = 0; j < recipeDetail.materials.length; j++) {
            ...
            html += '<li>' + escapeHtml(materialItem.materialName) + " x " + formatDailyMaterialAmount(materialItem.perPortionAmount) + "  " + formatDailyMaterialAmount(materialItem.perCycleAmount) + "/轮 " + formatDailyMaterialAmount(materialItem.dailyAmount) + "/天</li>";
        }
        html += "</ul></div>";
    }
    html += "</div>",
...
$(document).off("click.normalMaterialDetail").on("click.normalMaterialDetail", ".btn-normal-material-detail", function() {
    var ruleIndex = parseInt($(this).attr("data-rule-index"), 10);
    isNaN(ruleIndex) || (renderNormalBusinessMaterialDetailModal(ruleIndex),
    $("#normal-material-detail-modal").modal("show"))
});
$(document).off("click.normalMaterialInterval").on("click.normalMaterialInterval", ".normal-material-interval-select .material-interval-option", function(event) {
    event.preventDefault();
    var $dropdown = $(this).closest(".normal-material-interval-select")
      , value = parseInt($(this).attr("data-value"), 10)
      , ruleIndex = parseInt(($dropdown.attr("id") || "").replace("normal-material-interval-select-", ""), 10) || 0;
    (isNaN(value) || value < 0) && (value = 0),
    value > 60 && (value = 60),
    $dropdown.attr("data-value", value).removeClass("open"),
    normalBusinessMaterialIntervalMap[ruleIndex] = value,
    "function" == typeof calCustomResults && "undefined" != typeof calCustomRule && calCustomRule && calCustomResults(calCustomRule.gameData);
});
$(document).off("click.normalMaterialIntervalToggle").on("click.normalMaterialIntervalToggle", ".normal-material-interval-select .material-interval-toggle", function() {
    $(".bootstrap-select.open").each(function() {
        var $select = $(this).find("select");
        $select.length && $select.data("selectpicker") ? $select.selectpicker("toggle") : $(this).removeClass("open");
    });
});
$(document).off("shown.bs.select.normalMaterialIntervalMutex").on("shown.bs.select.normalMaterialIntervalMutex", "select", function() {
    $(".normal-material-interval-select").removeClass("open").find(".material-interval-toggle").attr("aria-expanded", "false");
});
function calCustomResults(e) {
    ...
    normalBusinessMaterialDetailsMap = {};
    ...
    for (var a = 0, t = 0; t < calCustomRule.rules.length; t++) {
        ...
        for (var c in l) {
            if (l[c].chef.chefId) {
                setDataForChef(l[c].chef, l[c].equip, !0, i.calGlobalUltimateData, s[c], i.calSelfUltimateData, i.calActivityUltimateData, !0, i, !0, i.calQixiaData || null);
            } else if (s[c]) {
                l[c].chef.materialEffects = [];
                var _mr3 = [];
                for (var _p3 in s[c]) {
                    var _eff3 = s[c][_p3];
                    if (_eff3.type === "MaterialReduce") {
                        _mr3.push(_eff3);
                    }
                }
                if (_mr3.length > 0) l[c].chef.materialEffects = _mr3;
            } else {
                l[c].chef.materialEffects = [];
            }
        }
        ...
        for (var c in l) {
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    ...
                    // 贵客率计算模式或正常营业分类（Id为0）时，不限制份数不能超过max
                    // 其他模式下，确保quantity不超过max
                    var isNormalBusiness = (calCustomRule.id === 0);
                    if (!isNormalBusiness && l[c].recipes[r].quantity > recipeMax) {
        ...
        l)
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    ...
                    // 修炼查询模式：显示差值；普通模式：显示可制作差值
                    var isUnultimatedMode = calCustomRule && calCustomRule.isCultivate === true;
                    var isGodMode = $("#chk-cultivation-mode").prop("checked");
                    if (isUnultimatedMode) {
                        // 神级方案模式：根据设置页查询品级显示差值；修炼查询模式：固定使用神级差值
                        var gradeMultiplier = isGodMode ? (parseInt($("#select-recipe-god-grade").val()) || 4) : 4;
                        var gradeNames = {5: '传', 4: '神', 3: '特', 2: '优', 1: '可'};
                        var gradeName = gradeNames[gradeMultiplier] || '神';
                        var k = getSkillDiff(l[c].chef, l[c].recipes[r].data, gradeMultiplier);
                        if (k.value > 0) {
                            l[c].recipes[r].skillDiff = gradeName + "差值：" + k.disp;
                        } else {
                            l[c].recipes[r].skillDiff = "";
                        }
                    } else if (0 == g.rankVal) {
        ...
        var guestArrivalBaseRate = null;
        if (calCustomRule && calCustomRule.isGuestRate === true && typeof GuestRateCalculator !== "undefined" && typeof GuestRateCalculator.calculateFields === "function" && typeof GuestRateCalculator.calculateActualGuestRate === "function") {
            var guestArrivalFields = GuestRateCalculator.calculateFields(l, i, 5, 7, 1);
            guestArrivalBaseRate = Number(guestArrivalFields && guestArrivalFields.guestRate) || 0;
        }
        $(".cal-custom-item:eq(" + t + ") .guest-arrival-flag").empty();
        ...
        for (var c in l)
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    ...
                    // 其他模式下，确保quantity不超过max
                    var isNormalBusiness = (calCustomRule.id === 0);
                    if (!isNormalBusiness && actualQuantity > D) {
                        actualQuantity = D;
                    ...
                    // 贵客率计算模式或正常营业分类下，max设置为999；其他模式下设置为D
                    var maxValue = (isNormalBusiness) ? 999 : D;
                    ...
                    var guestArrivalFlag = "";
                    if (guestArrivalBaseRate !== null) {
                        var recipeStarLevel = Number(l[c].recipes[r].data.rarity) || 5;
                        var actualGuestRate = GuestRateCalculator.calculateActualGuestRate(recipeStarLevel, actualQuantity, guestArrivalBaseRate);
                        guestArrivalFlag = actualGuestRate >= 100 ? "(必来)" : "";
                    }
                    w.find(".guest-arrival-flag").html(guestArrivalFlag)
                }
        ...
        if (i.showTime) {
            ...
            for (var c in l)
                if (l[c].chef.chefId) {
                    ...
                    // 统计当前实际生效的自我修炼技能时间加成
                    var activeSelfUltimateEffects = getActiveSelfUltimateEffectsForTime(l[c].chef);
                    if (activeSelfUltimateEffects.length > 0) {
                        P += getTimeAddition(activeSelfUltimateEffects);
                    }
                    // 统计厨具技能的时间加成
                    if ((!i || !i.hasOwnProperty("DisableEquipSkillEffect") || 0 == i.DisableEquipSkillEffect) && l[c].equip && l[c].equip.effect) {
                        var V = updateEquipmentEffect(l[c].equip.effect, activeSelfUltimateEffects);
                        P += getTimeAddition(V);
                    }
                }
            for (var c in o[0]) {
            ...
            var isNormalBusinessMaterial = 0 === calCustomRule.id && !calCustomRule.isGuestRate && !calCustomRule.isCultivate
              , materialIntervalMinutes = isNormalBusinessMaterial ? getNormalMaterialIntervalMinutes(t) : 0;
            0 != +P.toFixed(2) && (_ += " 原时间：" + (secondsToTime(f) || 0));
            var B = Math.ceil(+(f * (1 + P / 100)).toFixed(2));
            var materialIntervalSeconds = Math.max(0, Math.min(60, parseInt(materialIntervalMinutes, 10) || 0)) * 60
              , displayTotalTimeSeconds = B + materialIntervalSeconds;
            _ += (isNormalBusinessMaterial ? " 总时间（含开业间隔）：" : " 总时间：") + (secondsToTime(isNormalBusinessMaterial ? displayTotalTimeSeconds : B) || 0);
            var efficiencyTimeSeconds = displayTotalTimeSeconds
              , F = 0;
            efficiencyTimeSeconds > 0 && (F = Math.floor(3600 * u / efficiencyTimeSeconds)),
            _ += " 总效率：" + F + "金币/小时";
            if (isNormalBusinessMaterial) {
                var dailyMaterialDetails = getDailyMaterialDetails(l, i, B, materialIntervalMinutes);
                normalBusinessMaterialDetailsMap[t] = dailyMaterialDetails,
                _ += getDailyMaterialSummaryHtml(t, dailyMaterialDetails, materialIntervalMinutes)
            }
        }
        if (y.lackIds.length)
        ...
        $(".cal-custom-item:eq(" + t + ") .normal-material-select").selectpicker();
        // 贵客率模式：显示贵客冲突提示
...
function getCalRecipeDisp(e) {
    ...
    return "<div class='name'>" + a.name + "</div><div class='recipe-ric'><div class='rank" + rankClass + "'>" + (e.rankDisp ? e.rankDisp : "") + "</div><div class='icon-box'>" + a.icon + "</div><div class='condiment'>" + a.condimentDisp + "</div></div>"
}
...
function getCustomChefsOptions(e, a, t) {
    var i = $("#chk-cal-got").prop("checked")
      ...
      , n = r.custom
      , o = !1;
    var isNormalBusinessMode = !r.isNetworkRule && !(calCustomRule && calCustomRule.isGuestRate === true) && !(calCustomRule && calCustomRule.isCultivate === true);
    var chefOrderMode = $("#select-cal-chef-order").val() || "分数";
    
    if (isNormalBusinessMode) {
        // 正常营业模式：
        // 1. 当前位置已有菜谱时，按该位置菜谱算分排序；
        // 2. 场上其他位置已有“厨师+菜谱”组合时，按全场分数排序第二/第三个厨师。
    ...
    // 检查是否为修炼查询模式（提前检查，用于过滤厨师）
    var isUnultimatedModeForFilter = calCustomRule && calCustomRule.isCultivate === true;
    ...
    for (var h in f) {
        // 修炼查询模式下过滤掉id为285的厨师
        if (isUnultimatedModeForFilter && f[h].chefId == 285) continue;
        ...
        var isOwned = isLocalChefOwned(f[h]) || isAllUltimateMode || (configUltimatedIds.allSet && configUltimatedIds.allSet[chefIdStr]);
        ...
        if (!i || isOwned) {
            ...
            if (isGuestRateModeFlag && typeof GuestRateCalculator !== 'undefined' && GuestRateCalculator.analyzeChefGuestRateSkills) {
                var analysisChef = JSON.parse(JSON.stringify(f[h]));
                if ((isPengciMode || isGuestRateModeFlag) && l) {
                    analysisChef.equip = f[h].equip;
                } else if (n[a].chef.chefId == f[h].chefId) {
                    analysisChef.equip = n[a].equip;
                }
                chefAnalysis = GuestRateCalculator.analyzeChefGuestRateSkills(
                    analysisChef,
                    r.calQixiaData || null,
                    l || (n[a].chef.chefId == f[h].chefId && !!analysisChef.equip && !!analysisChef.equip.equipId),
                    s, // useAmber
                    localData,
                    configUltimatedIds
                );
            ...
            // 检查是否为修炼查询模式
            isUnultimatedMode = calCustomRule && calCustomRule.isCultivate === true,
            
            // 修炼查询模式下添加分类
            isUnultimatedMode && (m.category = getChefCultivationCategory(f[h], localData, configUltimatedIds)),
            
            o && !isUnultimatedMode) {
                var v = getPartialChefAdds(d, r);
                // 碰瓷查询模式和贵客率模式：如果勾选了"已配厨具"，使用厨师自己佩戴的厨具
                var isGuestRateModeForEquip = calCustomRule && calCustomRule.isGuestRate === true;
                if ((isPengciMode || isGuestRateModeForEquip) && l) {
                    // 碰瓷/贵客率模式且勾选了已配厨具：使用厨师自己的厨具
                    d[a].equip = f[h].equip;
                } else if (n[a].chef.chefId == f[h].chefId) {
                    // 当前厨师是已选中的厨师：使用场上的厨具
                    d[a].equip = n[a].equip;
                } else if (l) {
                    // 其他厨师且勾选了已配厨具：使用厨师自己的厨具
                    d[a].equip = f[h].equip;
                }
                if (!s && n[a].chef.chefId != f[h].chefId)
                ...
                // 保存原始厨师数据（在addCheffSkillDiff之前），用于贵客率模式下计算单个菜谱差值
                var originalChefForRank = JSON.parse(JSON.stringify(d[a].chef));
                ...
                // 保存差值，稍后在星级后显示
                var chefSkillDiff = y;
                var isGuestRateModeForDiff = calCustomRule && calCustomRule.isGuestRate === true;
                // 非贵客率模式：差值稍后在星级后显示，这里只设置class
                if (!isGuestRateModeForDiff && "" != y) {
                ...
                var x = getPartialRecipeAdds(d, r)
                  ...
                  , displayScore = 0
                  , chefTotalTimeRaw = 0;
                var ruleScoreMultiply = 1;
                if (r && r.hasOwnProperty("scoreMultiply")) ruleScoreMultiply = r.scoreMultiply;
                var ruleScorePow = 1;
                if (r && r.hasOwnProperty("scorePow")) ruleScorePow = r.scorePow;
                var ruleScoreAdd = 0;
                if (r && r.hasOwnProperty("scoreAdd")) ruleScoreAdd = r.scoreAdd;
                for (var g in d)
                    for (var w in d[g].recipes)
                        if (d[g].recipes[w].data) {
                            ...
                            var actAdd = T.data && T.data.activityAddition ? T.data.activityAddition : 0;
                            d[g].recipes[w] = T,
                            chefTotalTimeRaw += T.totalTime,
                            C += T.totalScore,
                            displayScore += Math.ceil(+(T.totalScore * (1 + actAdd / 100)).toFixed(2))
                        }
                displayScore = +(Math.pow(displayScore, ruleScorePow) * ruleScoreMultiply).toFixed(2);
                displayScore = r.IsActivity ? Math.ceil(displayScore) : Math.floor(displayScore);
                displayScore && (displayScore += ruleScoreAdd);
                var S = calSatiety(d, r);
                r.Satiety && (C = calSatietyAdd(C, S.add),
                displayScore = calSatietyAdd(displayScore, S.add));
                var chefTimeAddition = 0;
                if (isNormalBusinessMode && r.showTime) {
                    for (var timeIdx in d)
                        if (d[timeIdx].chef.chefId) {
                            if (!(r && r.hasOwnProperty("DisableChefSkillEffect") && 0 != r.DisableChefSkillEffect)) {
                                chefTimeAddition += getTimeAddition(d[timeIdx].chef.specialSkillEffect);
                            }
                            var activeSelfUltimateEffectsForTime = getActiveSelfUltimateEffectsForTime(d[timeIdx].chef);
                            if (activeSelfUltimateEffectsForTime.length > 0) {
                                chefTimeAddition += getTimeAddition(activeSelfUltimateEffectsForTime);
                            }
                            if ((!r || !r.hasOwnProperty("DisableEquipSkillEffect") || 0 == r.DisableEquipSkillEffect) && d[timeIdx].equip && d[timeIdx].equip.effect) {
                                var timeEquipEffect = updateEquipmentEffect(d[timeIdx].equip.effect, activeSelfUltimateEffectsForTime);
                                chefTimeAddition += getTimeAddition(timeEquipEffect);
                            }
                        }
                    if (x[0]) {
                        for (var openTimeIdx in x[0]) {
                            var openTimeEffect = x[0][openTimeIdx].effect;
                            openTimeEffect && "OpenTime" == openTimeEffect.type && (chefTimeAddition += openTimeEffect.value);
                        }
                    }
                }
                var chefTotalTime = Math.ceil(+(chefTotalTimeRaw * (1 + chefTimeAddition / 100)).toFixed(2));
                var chefEfficiency = 0;
                chefTotalTime > 0 && (chefEfficiency = Math.floor(3600 * displayScore / chefTotalTime));
                
                // 贵客率计算模式下：每个菜谱单独计算品级和差值
                var isGuestRateModeForRank = calCustomRule && calCustomRule.isGuestRate === true;
                if (isGuestRateModeForRank) {
                    // 品级颜色映射
                    var rankColorMap = {'可': '#000000', '优': '#28a745', '特': '#fd7e14', '神': '#dc3545', '传': '#6f42c1'};
                    var rankOrder = {'传': 5, '神': 4, '特': 3, '优': 2, '可': 1};
                    ...
                    for (var recipeIdx in d[a].recipes) {
                        if (d[a].recipes[recipeIdx].data) {
                            // 使用原始厨师数据计算单个菜谱的差值
                            var singleChef = JSON.parse(JSON.stringify(originalChefForRank));
                            var singleRecipe = d[a].recipes[recipeIdx].data;
                            addCheffSkillDiff(singleChef, singleRecipe);
                            var singleDiff = getChefSillDiff(singleChef, originalChefForRank);
                            
                            // 计算差值总和（提取数字）
                            if (singleDiff !== "") {
                                hasAnyDiff = true;
                                var diffMatches = singleDiff.match(/-?\d+/g);
                                if (diffMatches) {
                                    for (var dm = 0; dm < diffMatches.length; dm++) {
                                        totalDiffSum += Math.abs(parseInt(diffMatches[dm]));
                                    }
                                }
                            }
                            ...
                            if (singleDiff !== "") {
                                // 有差值：只显示差值
                                rankDisplayList.push("<span style='color:#d9534f'>" + singleDiff + "</span>");
                                rankListPlain.push('-');
                            } else {
                                // 无差值：显示品级
                                var rankDisp = d[a].recipes[recipeIdx].rankDisp || '';
                                if (rankDisp) {
                                    var rankColor = rankColorMap[rankDisp] || '#000000';
                                    rankDisplayList.push("<span style='color:" + rankColor + ";font-weight:bold'>" + rankDisp + "</span>");
                                    rankListPlain.push(rankDisp);
                                }
                            }
                        }
                    }
                    ...
                    // 保存品级信息，稍后在星级后显示
                    if (rankDisplayList.length > 0) {
                        m.rankDisplay = rankDisplayList.join('、');
                    }
                    
                    // 排序逻辑：
                    // 1. 按三道菜的rank总和降序
                    // 2. 总和相同时，按排序后的rank数组逐个比较（优先展示最高rank更大的）
                    // 3. rank数组也相同时，按差值总和升序
                    var rankOrderMap = {'传': 5, '神': 4, '特': 3, '优': 2, '可': 1, '-': 0};
                    var rankValues = [];
                    var totalRankSum = 0;
                    for (var ri = 0; ri < rankListPlain.length; ri++) {
                        var rv = rankOrderMap[rankListPlain[ri]] || 0;
                        rankValues.push(rv);
                        totalRankSum += rv;
                    }
                    // 对rank值降序排列
                    rankValues.sort(function(a, b) { return b - a; });
                    
                    // 计算排序分数：总和*1e9 + 第一个rank*1e6 + 第二个rank*1e3 + 第三个rank - 差值
                    var sortScore = totalRankSum * 1e9;
                    for (var si = 0; si < rankValues.length && si < 3; si++) {
                        sortScore += rankValues[si] * Math.pow(1000, 2 - si);
                    }
                    sortScore -= totalDiffSum * 0.001;
                    ...
                    if (isNormalBusinessMode && "效率" == chefOrderMode) {
                        m.content += "<span class='score'>" + chefEfficiency + "</span>";
                    } else if (isNormalBusinessMode && "时间" == chefOrderMode) {
                        m.content += "<span class='score'>" + (secondsToTime(chefTotalTime) || 0) + "</span>";
                    } else {
                        m.content += "<span class='score'>" + displayScore + "</span>";
                    }
                    m.order = displayScore;
                }
                if (isNormalBusinessMode) {
                    "时间" == chefOrderMode ? m.order = chefTotalTime : "效率" == chefOrderMode && (m.order = chefEfficiency);
                }
            } else
                m.order = f[h].rarity;
            // 贵客率模式：在星级后显示品级或差值
            var isGuestRateModeForDisplay = calCustomRule && calCustomRule.isGuestRate === true;
            // 先显示星级
            c.indexOf("chef_rarity") >= 0 && (m.content += "<span class='subtext'>" + f[h].rarityDisp + "</span>");
            // 贵客率模式：在星级后显示品级或差值
            isGuestRateModeForDisplay && m.rankDisplay && (m.content += "<span class='score'>" + m.rankDisplay + "</span>");
            // 非贵客率模式：在星级后显示差值
            !isGuestRateModeForDisplay && chefSkillDiff && "" != chefSkillDiff && (m.content += "<span class='skill'>" + chefSkillDiff + "</span>");
            // 修炼查询模式：在星级后显示第一个修炼任务描述（光环厨师分类不显示）
            isUnultimatedMode && f[h].ultimateGoal && f[h].ultimateGoal.length > 0 && m.category && m.category.indexOf("unultimated-chef-category") >= 0 && (m.content += "<span class='skilltext'>" + getFirstUltimateQuestDesc(f[h], t) + "</span>");
            c.indexOf("chef_specialSkill") >= 0 && (m.content += "<span class='skilltext'>" + f[h].specialSkillDisp.replace(/<br>/g, " ") + "</span>");
            // 修炼查询模式下显示完整修炼技能，非修炼模式使用过滤后的
            c.indexOf("chef_ultimate") >= 0 && (m.content += "<span class='skilltext'>" + (isUnultimatedMode ? (f[h].ultimateSkillDisp || "").replace(/<br>/g, " ") : f[h].ultimateCustomDisp) + "</span>");
            c.indexOf("chef_origin") >= 0 && (m.content += "<span class='subtext'>" + f[h].origin.replace(/<br>/g, " ") + "</span>");
            for (var q in calCustomRule.rules) {
...
// 获取厨师第一个修炼任务的描述
function getFirstUltimateQuestDesc(chef, gameData) {
    if (!chef.ultimateGoal || chef.ultimateGoal.length === 0) return "";
    var questId = chef.ultimateGoal[0];
    for (var i = 0; i < gameData.quests.length; i++) {
        if (gameData.quests[i].questId === questId) {
            return gameData.quests[i].goal || "";
        }
    }
    return "";
}
// 获取厨师在修炼查询模式下的分类
function getChefCultivationCategory(chef, localData, configUltimatedIds) {
    var categories = [];
    
    // 检查是否为光环厨师（在上场类已修炼中勾选的Next/Partial类厨师，且修炼技能为纯技法加成类）
    var enabledChefIds = $("#chk-cal-partial-ultimate").val() || [];
    var isEnabledAura = enabledChefIds.indexOf(String(chef.chefId)) >= 0 || 
                        enabledChefIds.indexOf(chef.chefId) >= 0;
    
    if (isEnabledAura && chef.ultimateSkillEffect) {
        // 检查是否有Next或Partial技能，且技能类型为纯技法加成类（炒/煮/切/炸/烤/蒸）
        for (var k = 0; k < chef.ultimateSkillEffect.length; k++) {
            var effect = chef.ultimateSkillEffect[k];
            // 只检查六种技法类型，不包括MaxEquipLimit
            ...
            if ((effect.condition === "Next" || effect.condition === "Partial") && isSkillType) {
                categories.push("aura-chef-category");
                break;
            }
        }
    }
    ...
    // 检查是否为未修炼厨师
    var isUltimated = isChefUltimated(chef.chefId, localData, configUltimatedIds);
    if (!isUltimated) {
        categories.push("unultimated-chef-category");
    }
...
function getCustomEquipsOptions(e, a, t, i) {
    ...
    for (var u in t) {
        ...
        o[a].chef.chefId && r) {
            ...
            // 修炼模式、贵客率模式、碰瓷查询模式下不显示分数
            var isUnultimatedMode = calCustomRule && calCustomRule.isCultivate === true;
            var isGuestRateMode = calCustomRule && calCustomRule.isGuestRate === true;
            var isPengciMode = $("#chk-guest-query-mode").prop("checked");
            // 厨具选择框：贵客率模式和碰瓷模式不显示分数
            if (!isUnultimatedMode && !isGuestRateMode && !isPengciMode) {
...
function getCustomRecipesOptions(e, a, t, i) {
    var l = $("#chk-cal-got").prop("checked")
      ...
      , _excludeMaterialRules = getExcludeMaterialRules("#chk-cal-recipe-material-exclude")
    ...
    // 先更新所有厨师数据，确保 materialEffects 包含 Partial 效果
    var _partialAdds = getPartialChefAdds(h, u);
    for (var _ci in h) {
        if (h[_ci].chef.chefId) {
            setDataForChef(h[_ci].chef, h[_ci].equip, !0, u.calGlobalUltimateData, _partialAdds[_ci], u.calSelfUltimateData, u.calActivityUltimateData, !0, u, !0, u.calQixiaData || null);
        } else if (_partialAdds[_ci]) {
            h[_ci].chef.materialEffects = [];
            var _mr2 = [];
            for (var _p2 in _partialAdds[_ci]) {
                var _eff2 = _partialAdds[_ci][_p2];
                if (_eff2.type === "MaterialReduce") {
                    _mr2.push(_eff2);
                }
            }
            if (_mr2.length > 0) h[_ci].chef.materialEffects = _mr2;
        } else {
            h[_ci].chef.materialEffects = [];
        }
    }
    ...
    for (var y in k) {
        var x = k[y].recipe;
        if ((!l || x.data.got || isAllUltimateMode) && !(s.length > 0 && s.indexOf(x.data.rarity.toString()) < 0)) {
            if (c.length > 0) {
                var D = !1;
                for (var C in c)
                    if (x.data["" + c[C]] > 0) {
            ...
            if (!(n.length > 0 && n.indexOf(x.data.condiment) < 0)) {
                if (isRecipeExcludedByMaterialRules(x.data.materials, _excludeMaterialRules)) continue;
                var w = {};
                        ...
                        if (h[b].recipes[v].data) {
                            ...
                            b == a && v == t) {
                                ...
                                if (isUnultimatedMode) {
                                    ...
                                    var gradeNames = {5: '传', 4: '神', 3: '特', 2: '优', 1: '可'};
                                    var gradeName = gradeNames[gradeMultiplier] || '神';
                                    var O = getSkillDiff(h[a].chef, x.data, gradeMultiplier);
                                    if (O.value > 0) {
                                        A = gradeName + "差值：" + O.disp;
                                    } else {
                                        A = "";
                                    }
                                } else if (0 == M.rankVal) {
                ...
                // 修炼查询模式：调整显示顺序为 菜谱名 → 星级 → 品级 → 差值 → 技法值
                var isUnultimatedModeForDisplay = calCustomRule && calCustomRule.isCultivate === true;
                // 碰瓷查询模式：调整显示顺序为 菜谱名 → 星级 → 品级 → 差值
                var isPengciModeForDisplay = $("#chk-guest-query-mode").prop("checked");
                // 贵客率计算模式：调整显示顺序为 菜谱名 → 星级 → 品级 → 差值 → 贵客名
                var isGuestRateModeForDisplay = calCustomRule && calCustomRule.isGuestRate === true;
                
                if (isUnultimatedModeForDisplay) {
                    // 修炼查询模式：先显示星级（根据菜单设置）
                    if (d.indexOf("recipe_rarity") >= 0) {
                        w.content += "<span class='subtext'>" + x.data.rarityDisp + "</span>";
                    }
                    
                    // 再显示品级
                    if (d.indexOf("recipe_rank") >= 0) {
                        var V = h[a].recipes[t].rankDisp;
                        if (V && "-" != V) {
                            var rankColor = '';
                            if (V === '可') rankColor = '#000000';
                            else if (V === '优') rankColor = '#28a745';
                            else if (V === '特') rankColor = '#fd7e14';
                            else if (V === '神') rankColor = '#dc3545';
                            else if (V === '传') rankColor = '#6f42c1';
                            w.content += "<span class='subtext' style='color:" + rankColor + ";font-weight:bold'>" + V + "</span>";
                        }
                    }
                    ...
                    w.class = "";
                    if ("" != A) {
                        w.content += "<span class='skill'>" + A + "</span>";
                        w.class += "warning-skill";
                    }
                } else if (isPengciModeForDisplay || isGuestRateModeForDisplay) {
                    // 碰瓷查询模式和贵客率模式：显示顺序为 星级 → 品级 → 差值
                    // 先显示星级（根据菜单设置）
                    if (d.indexOf("recipe_rarity") >= 0) {
                        w.content += "<span class='subtext'>" + x.data.rarityDisp + "</span>";
                    }
                ...
                if (u.hasOwnProperty("MaterialsNum")) {
                    ...
                    if (isGuestRateModeFlag) {
                        // 刷玉模式下，在品级和贵客名之间标记玉璧表中的单次玉璧数。
                        var isJadeModeForDisplay = $("#chk-guest-rate-submode").prop("checked");
                        if (isJadeModeForDisplay && typeof window.getJadeRecipeValueForDisplay === "function") {
                            var jadeValueForDisplay = window.getJadeRecipeValueForDisplay(x.data.name);
                            w.jadeValue = jadeValueForDisplay;
                            if (jadeValueForDisplay > 0) {
                                w.content += "<span class='subtext jade-recipe-value'>" + jadeValueForDisplay + "玉</span>";
                            }
                        }
                        // 贵客率计算模式：显示贵客信息
                        if (i && i.guests) {
                        ...
                        // 非贵客率计算模式
                        var isUnultimatedMode = calCustomRule && calCustomRule.isCultivate === true;
                        var isGodModeFlag = $("#chk-cultivation-mode").prop("checked");
                        var isQuestCategory = typeof globalCultivationRecipeCategory !== 'undefined' && 
                                              globalCultivationRecipeCategory && 
                                              globalCultivationRecipeCategory.category === 'quest-recipe-category';
                        
                        if (isUnultimatedMode && isGodModeFlag) {
                            // 神级方案模式：按品级降序，同品级按差值升序
                            var rankOrderMap = {'传': 5, '神': 4, '特': 3, '优': 2, '可': 1, '-': 0};
                            var rankValue = 0;
                            var diffValue = 0;
                            
                            // 如果有厨师，计算品级和差值
                            if (h[a].chef && h[a].chef.chefId) {
                                if (typeof calculateRecipeRank === 'function') {
                                    rankValue = calculateRecipeRank(h[a].chef, x.data);
                                } else if (h[a].recipes[t] && h[a].recipes[t].rankDisp) {
                                    rankValue = rankOrderMap[h[a].recipes[t].rankDisp] || 0;
                                }
                                // 获取差值用于排序
                                var gradeMultiplier = parseInt($("#select-recipe-god-grade").val()) || 4;
                                var skillDiffObj = getSkillDiff(h[a].chef, x.data, gradeMultiplier);
                                diffValue = skillDiffObj.value || 0;
                            }
                            // 品级*10000 - 差值（品级高的排前面，同品级差值小的排前面）
                            // 没有厨师时，rankValue=0，diffValue=0，按默认顺序
                            w.order = rankValue * 10000 - diffValue;
                        } else if (isUnultimatedMode && isQuestCategory) {
                            // 修炼模式的"修炼任务"分类：显示单份时间，不显示分数和总分
                            w.content += "<span class='score'>" + x.data.timeDisp + "</span>";
                            w.order = 0;
                        } else if ("分数" == o) {
                ...
                // 修炼模式、碰瓷查询模式和贵客率模式下星级已在前面显示，不重复显示
                !isUnultimatedModeForDisplay && !isPengciModeForDisplay && !isGuestRateModeForDisplay && d.indexOf("recipe_rarity") >= 0 && (w.content += "<span class='subtext'>" + x.data.rarityDisp + "</span>"),
    ...
    // 神级方案模式不过滤修炼任务菜谱，显示全部菜谱
    if (isUnultimatedMode && isQuestCategory && !isGodModeFlag) {
        // 修炼任务分类：过滤并排序
        var chef = h[a].chef;
        if (chef && chef.chefId && typeof getQuestMatchingRecipeIdsForOptions === 'function') {
            var questRecipeIds = getQuestMatchingRecipeIdsForOptions(e, a, chef);
            if (questRecipeIds && questRecipeIds.length > 0) {
                // 过滤：只保留满足修炼任务条件的菜谱
                var filteredG = [];
                var recipeMap = {};
                for (var gi = 0; gi < g.length; gi++) {
                    recipeMap[String(g[gi].value)] = g[gi];
                }
                // 按照questRecipeIds的顺序添加
                for (var qi = 0; qi < questRecipeIds.length; qi++) {
                    var rid = questRecipeIds[qi];
                    if (recipeMap[rid]) {
                        filteredG.push(recipeMap[rid]);
                    }
                }
                g = filteredG;
            } else if (questRecipeIds && questRecipeIds.length === 0) {
                // 没有满足条件的菜谱
                g = [];
            }
        }
        // 修炼任务分类不使用分数排序（已经在getQuestMatchingRecipeIdsForOptions中排序）
    } else if (isUnultimatedMode && isGodModeFlag) {
        // 神级方案模式：按品级降序，同品级按差值升序（使用w.order排序）
        g.sort(function(e, a) {
            return a.order - e.order;
        });
    } else if (!isUnultimatedMode || !isQuestCategory) {
...
function initCalCustomTable(e) {
    ...
    // 删除复制产生的重复ID元素（只保留第一个cal-custom-item中的）
    $(".cal-custom-item:gt(0) #guest-rate-result").remove(),
    $(".cal-custom-item:gt(0) #guest-jade-result").remove(),
    $(".cal-custom-item:gt(0) #guest-material-detail-modal").remove(),
    $(".cal-custom-item:gt(0) #jade-material-detail-modal").remove(),
    $(".cal-custom-item:gt(0) #pengci-query-area").remove(),
    ...
    $(".chef-box .chef-placeholder, .chef-box .chef-box-1").click(function() {
        ...
        var $chefSelect = $(this).closest(".chef-box").find("select.select-picker-chef");
        $chefSelect.html(getOptionsString(i)).selectpicker("refresh").selectpicker("toggle");
    }),
    ...
    $(".equip-box").click(function(evt) {
        // 如果下拉框已经打开（用户正在选择选项），不要重新执行打开逻辑
        ...
        // 修炼模式下的处理
        var isUnultimatedMode = calCustomRule && calCustomRule.isCultivate === true;
        var $equipSelect = $(this).find("select.select-picker-equip");
        var selectId = $equipSelect.attr('id') || ('cultivation-equip-select-' + Math.random().toString(36).substr(2, 9));
        $equipSelect.attr('id', selectId);
        
        // 保存原始选项（用于修炼模式的"全部"分类）
        var originalOptions = l;
        
        if (isUnultimatedMode && typeof sortEquipsForCultivation === 'function') {
            // 先保存原始的完整选项（用于"全部"分类）
            if (typeof cultivationEquipFullOptionsHtml === 'undefined') {
                window.cultivationEquipFullOptionsHtml = {};
            }
            cultivationEquipFullOptionsHtml[selectId] = getOptionsString(l);
            
            // 根据当前分类状态决定是否应用神级推荐过滤
            var currentCategory = (typeof globalCultivationEquipCategory !== 'undefined' && globalCultivationEquipCategory) 
                ? globalCultivationEquipCategory.category : 'god-recommend-category';
            
            if (currentCategory === 'god-recommend-category') {
                l = sortEquipsForCultivation(l, a, t, i.equips);
            }
        }
        
        $equipSelect.html(getOptionsString(l)).selectpicker("refresh").selectpicker("toggle");
        
    }),
    $(document).on("changed.bs.select", "select.select-picker-equip", function(a, t, i, l) {
        var s = $(".cal-custom-item").index($(this).closest(".cal-custom-item"))
          , c = $(this).closest(".cal-custom-item").find(".selected-item").index($(this).closest(".selected-item"))
          , r = $(this).val();
    ...
    $(".recipe-box .recipe-placeholder, .recipe-box .content, .recipe-box .recipe-result, .recipe-box .recipe-box-2").click(function() {
        ...
        // 修炼模式下的处理
        var isUnultimatedMode = calCustomRule && calCustomRule.isCultivate === true;
        var $recipeSelect = $(this).closest(".recipe-box").find("select.select-picker-recipe");
        var selectId = $recipeSelect.attr('id') || ('cultivation-recipe-select-' + Math.random().toString(36).substr(2, 9));
        $recipeSelect.attr('id', selectId);
        
        if (isUnultimatedMode) {
            // 确保修炼模式下默认为修炼任务分类
            if (typeof globalCultivationRecipeCategory === 'undefined' || !globalCultivationRecipeCategory) {
                globalCultivationRecipeCategory = { category: 'quest-recipe-category' };
            }
            
            // 初始化缓存对象
            if (typeof window.cultivationRecipeFullOptionsHtml === 'undefined') {
                window.cultivationRecipeFullOptionsHtml = {};
            }
            if (typeof window.cultivationRecipeQuestOptionsHtml === 'undefined') {
                window.cultivationRecipeQuestOptionsHtml = {};
            }
            
            // 1. 生成"全部"分类的选项（带分数排序）
            globalCultivationRecipeCategory = null;
            var fullOptions = getCustomRecipesOptions(a, t, i, e);
            window.cultivationRecipeFullOptionsHtml[selectId] = getOptionsString(fullOptions);
            
            // 2. 生成"修炼任务"分类的选项（带时间显示）
            globalCultivationRecipeCategory = { category: 'quest-recipe-category' };
            var questOptions = getCustomRecipesOptions(a, t, i, e);
            window.cultivationRecipeQuestOptionsHtml[selectId] = getOptionsString(questOptions);
            
            // 3. 根据当前分类状态显示对应选项
            var currentCategory = globalCultivationRecipeCategory ? globalCultivationRecipeCategory.category : null;
            if (currentCategory === 'quest-recipe-category') {
                $(this).closest(".recipe-box").find("select.select-picker-recipe").html(window.cultivationRecipeQuestOptionsHtml[selectId]).selectpicker("refresh").selectpicker("toggle");
            } else {
                $(this).closest(".recipe-box").find("select.select-picker-recipe").html(window.cultivationRecipeFullOptionsHtml[selectId]).selectpicker("refresh").selectpicker("toggle");
            }
        } else {
            var l = getCustomRecipesOptions(a, t, i, e);
    ...
    $("#chk-cal-max-disk").change(function() {
        ...
        for (var ruleIdx = 0; ruleIdx < calCustomRule.rules.length; ruleIdx++) {
            ...
            for (var pos in custom) {
                if (custom[pos].chef && custom[pos].chef.chefId && custom[pos].chef.disk) {
                    // 保存原始等级（如果还没保存）
                    ...
                    if (isChecked) {
                        ...
                        // 贵客率模式下，为蓝色槽位设置满棚陨遗玉
                        if (isGuestRateModeFlag && custom[pos].chef.disk.ambers) {
                            // 保存原始遗玉配置（如果还没保存）
                            if (!custom[pos].chef.disk.hasOwnProperty('originalAmbers')) {
                                custom[pos].chef.disk.originalAmbers = JSON.parse(JSON.stringify(custom[pos].chef.disk.ambers));
                            }
                            // 为蓝色槽位设置满棚陨遗玉（amberId=75）
                            for (var amberIdx in custom[pos].chef.disk.ambers) {
                                if (custom[pos].chef.disk.ambers[amberIdx].type === 3) {
                                    setCustomAmber(ruleIdx, pos, amberIdx, 75);
                                }
                            }
                        }
                    } else {
                        ...
                        // 贵客率模式下，恢复原始遗玉配置
                        if (isGuestRateModeFlag && custom[pos].chef.disk.originalAmbers) {
                            for (var amberIdx in custom[pos].chef.disk.ambers) {
                                if (custom[pos].chef.disk.ambers[amberIdx].type === 3) {
                                    var originalAmber = custom[pos].chef.disk.originalAmbers[amberIdx];
                                    var originalAmberId = originalAmber && originalAmber.data && originalAmber.data.amberId ? originalAmber.data.amberId : null;
                                    setCustomAmber(ruleIdx, pos, amberIdx, originalAmberId);
                                }
                            }
                        }
                    }
...
function generateData(e, a, t) {
    ...
    t && t.hasOwnProperty("decorationEffect") && (i.decorationEffect = t.decorationEffect),
            ...
            for (var i in partialChefIds) {
                for (var j in e.chefs) {
                    if (e.chefs[j].chefId == partialChefIds[i]) {
                        ...
                        // 时间类修炼词条只看 type/value，不使用 condition 判断
                        var activeSelfUltimateEffects = getActiveSelfUltimateEffectsForTime(e.chefs[j]);
                        var ultimateTimeEffects = [];
                        if (e.chefs[j].ultimateSkillEffect) {
                            for (var ue in e.chefs[j].ultimateSkillEffect) {
                                if ("OpenTime" == e.chefs[j].ultimateSkillEffect[ue].type || "CookbookTime" == e.chefs[j].ultimateSkillEffect[ue].type) {
                                    ultimateTimeEffects.push(e.chefs[j].ultimateSkillEffect[ue]);
                                }
                            }
                        }
                        if (ultimateTimeEffects.length > 0) {
                            var ultAdd = getTimeAddition(ultimateTimeEffects);
                            if (ultAdd != 0) {
                                chefTimeAdd += ultAdd;
                            }
                        }
                        ...
                        if (e.chefs[j].equip && e.chefs[j].equip.effect) {
                            ...
                            var equipEffect = updateEquipmentEffect(e.chefs[j].equip.effect, activeSelfUltimateEffects);
...
function getUpdateData(e) {
    var a = getLocalData()
      ...
      , c = getUltimateData(e.chefs, a, l, s, e.skills);
    e.ultimateData = c;
...
function getOriginVal(e) {
    var a = 0;
    return "菜棚" == e ? a = 1 : "菜地" == e ? a = 2 : "森林" == e ? a = 3 : "鸡舍" == e ? a = 4 : "猪圈" == e ? a = 5 : "牧场" == e ? a = 6 : "作坊" == e ? a = 7 : "池塘" == e ? a = 8 : a = 0,
    a
}
            ...
            if (i) {
                if (a)
                    for (var o in a.chefs)
                        if (e[r].chefId == a.chefs[o].id && isLocalChefOwnedAndUltimated(a.chefs[o])) {
                            n = !0;
                            break
                        }
            } else
                ...
                for (var u in e[r].ultimateSkillList) {
                    ...
                    if (skillObj && skillObj.effect && skillObj.effect.length > 0 && skillObj.effect[0].conditionType == "SwordsUnited") {
                        ...
                        if (i) {
                            ...
                            // 如果没有本地数据，但厨师没有hide属性，说明已拥有且已修炼
                            else if (!e[r].hide) {
                                hasUlt = !0;
                            }
                        } else
                                ...
                                for (var cv in effect.conditionValueList) {
                                    ...
                                    for (var ch in e) {
                                        if (e[ch].baseChefId == conditionValue) {
                                            var chefGot = !1;
                                            if (i) {
                                                // 七侠成员只需拥有即可（不需要修炼），技能持有者需已修炼
                                                if (a && a.chefs) {
                                                    for (var ac in a.chefs)
                                                        if (a.chefs[ac].id == e[ch].chefId && isLocalChefOwned(a.chefs[ac])) {
                                                            chefGot = !0;
                                                            break
                                                        }
                                                } else if (e[ch].got === "是" || e[ch].got === true || e[ch].got === 1 || e[ch].got === "1" || !e[ch].hide) {
                                                    // 如果厨师有got属性且为true，或者没有hide属性（表示已拥有）
                                                    chefGot = !0
                                                }
                                            } else {
...
function getActiveSelfUltimateEffectsForTime(chef) {
    if (!chef)
        return [];
    if (chef.selfUltimateEffect && chef.selfUltimateEffect.length > 0)
        return chef.selfUltimateEffect;
    if (typeof isAllUltimateMode !== 'undefined' && isAllUltimateMode && chef.ultimateSkillEffect) {
        var effects = [];
        for (var i in chef.ultimateSkillEffect)
            "Self" == chef.ultimateSkillEffect[i].condition && effects.push(chef.ultimateSkillEffect[i]);
        return effects
    }
    return []
}
function setPartialUltimateOptions(e) {
    ...
    function getUltimateTimeEffects(effect) {
        var timeEffects = [];
        if (!effect) {
            return timeEffects;
        }
        for (var i in effect) {
            if ("OpenTime" == effect[i].type || "CookbookTime" == effect[i].type) {
                timeEffects.push(effect[i]);
            }
        }
        return timeEffects;
    }
    ...
    function getTimeChefFullDisp(chef) {
        ...
        if (!disp && chef.ultimateSkillEffect) {
            for (var u in chef.ultimateSkillEffect) {
                if ("CookbookTime" == chef.ultimateSkillEffect[u].type || "OpenTime" == chef.ultimateSkillEffect[u].type) {
        ...
        if (hasPartialOrNext) {
            ...
            for (var r in l.effect) {
                if (("Partial" == l.effect[r].condition || "Next" == l.effect[r].condition) &&
                    ("Bake" == l.effect[r].type || "Steam" == l.effect[r].type || "Boil" == l.effect[r].type || 
                    "Fry" == l.effect[r].type || "Knife" == l.effect[r].type || "Stirfry" == l.effect[r].type)) {
                    isSkillType = true;
                }
                // 时间类只看 type/value，不使用 condition 判断
    ...
    timeTypeChefs.sort(function(e, a) {
        ...
        var eUltimateTimeEffects = getUltimateTimeEffects(e.chef.ultimateSkillEffect);
        if (eUltimateTimeEffects.length > 0) {
            eTimeAdd += getTimeAddition(eUltimateTimeEffects);
        }
        if (e.chef.equip && e.chef.equip.effect) {
            // 处理MutiEquipmentSkill类技能（使厨具技能效果翻倍）
            var eEquipEffect = updateEquipmentEffect(e.chef.equip.effect, getActiveSelfUltimateEffectsForTime(e.chef));
        ...
        var aUltimateTimeEffects = getUltimateTimeEffects(a.chef.ultimateSkillEffect);
        if (aUltimateTimeEffects.length > 0) {
            aTimeAdd += getTimeAddition(aUltimateTimeEffects);
        }
        if (a.chef.equip && a.chef.equip.effect) {
            // 处理MutiEquipmentSkill类技能（使厨具技能效果翻倍）
            var aEquipEffect = updateEquipmentEffect(a.chef.equip.effect, getActiveSelfUltimateEffectsForTime(a.chef));
...
function setCalPartialUltimateOptions(e) {
    ...
    for (var t in e) {
        ...
        if (e[t].ultimateSkillEffect) {
            ...
            if (hasValidSkill) {
                ...
                if (localData && localData.chefs) {
                    for (var lci = 0; lci < localData.chefs.length; lci++) {
                        if (localData.chefs[lci].id == e[t].chefId && isLocalChefOwnedAndUltimated(localData.chefs[lci])) {
                            l.isUltimated = true;
                            break;
                        }
                    }
    ...
    a.sort(function(e, a) {
        // 已勾选的优先
        if (e.isUltimated !== a.isUltimated) {
            return e.isUltimated ? -1 : 1;
        }
        // 同状态按技能描述排序
        return e.disp.localeCompare(a.disp);
    });
...
/**
 * 刷新"上场类已修炼"下拉框排序（已勾选的优先显示）
 * 在导入数据、读取已修炼、选择变化时调用
 */
var isRefreshingPartialUltimateSort = false;  // 防止递归调用的标志
function refreshCalPartialUltimateSort() {
    // 如果正在刷新中，直接返回，防止递归调用
    if (isRefreshingPartialUltimateSort) return;
    isRefreshingPartialUltimateSort = true;
    
    try {
        var $select = $("#chk-cal-partial-ultimate");
        var selectedValues = $select.val() || [];
        ...
        $select.find('option').each(function() {
            ...
            options.push({
                value: $opt.val(),
                text: $opt.text(),
                subtext: $opt.attr('data-subtext') || '',
                isSelected: selectedValues.indexOf($opt.val()) >= 0
            });
        });
        ...
        for (var i = 0; i < options.length; i++) {
            var opt = options[i];
            var html = "<option value='" + opt.value + "' data-subtext='" + opt.subtext + "'>" + opt.text + "</option>";
            $select.append(html);
        }
        ...
        isRefreshingPartialUltimateSort = false;
    }
}
    ...
    for (var a in e) {
        ...
        for (var t in e[a].ultimateSkillEffect) {
            ...
            if ("Self" == i.condition && "Material_Gain" != i.type) {
                ...
                if (localData && localData.chefs) {
                    for (var lci = 0; lci < localData.chefs.length; lci++) {
                        if (localData.chefs[lci].id == e[a].chefId && isLocalChefOwnedAndUltimated(localData.chefs[lci])) {
...
function getOptionsString(e) {
    ...
    return $.each(e, function(e, t) {
        ...
        t.jadeValue !== undefined && (a += " data-jade-value='" + t.jadeValue + "'"),
...
var calCustomRule, private = !1, isMobile = !1, expendBtn = !0, isAllUltimateMode = !1, isApplyingUltimateSelection = !1;
...
// ==================== 风云宴/江湖帖自动计算优化器事件处理 ====================
$(function() {
    // 点击"自动计算"按钮，根据规则类型选择对应的优化器
    $("#btn-banquet-auto").click(function() {
        
        // 从档位下拉框获取目标分数
        var tierVal = $("#select-banquet-tier").val();
        var targetScore = null;
        if (tierVal && tierVal !== '-1') {
            targetScore = parseInt(tierVal);
            if (isNaN(targetScore) || targetScore <= 0) targetScore = null;
        }
        ...
        $btn.prop("disabled", true).html('计算中...');
        
        // 显示进度条
        $("#banquet-progress-bar").css("width", "0%");
        $("#banquet-progress-text").text("0%");
        $("#banquet-progress-wrapper").removeClass("hidden");
        
        // 判断当前规则类型：根据规则标题判断是风云宴还是江湖帖/贤客楼
        var currentRule = calCustomRule && calCustomRule.rules && calCustomRule.rules[0];
        var ruleTitle = currentRule ? currentRule.Title : '';
        var isJianghuRule = ruleTitle.indexOf('江湖帖') >= 0 || ruleTitle.indexOf('贤客楼') >= 0;
        
        // 根据规则类型选择对应的优化器
        var Optimizer = isJianghuRule ? JianghuOptimizer : BanquetOptimizer;
        var optimizerName = isJianghuRule ? 'JianghuOptimizer' : 'BanquetOptimizer';
        
        // 初始化优化器
        var initSuccess = Optimizer.init(calCustomRule.gameData);
        if (!initSuccess) {
            $btn.prop("disabled", false).html(originalText);
            $("#banquet-progress-wrapper").addClass("hidden");
            showAlert("初始化失败，请检查是否已导入厨师和菜谱数据", "提示");
            return;
        }
        ...
        Optimizer.optimize(
            targetScore,
            // 进度回调 — 更新进度条
            function(percent, currentScore) {
                $("#banquet-progress-bar").css("width", percent + "%");
                var text = percent + '%';
                if (currentScore > 0) text += '  ' + currentScore;
                $("#banquet-progress-text").text(text);
            },
            // 完成回调
            function(result) {
                ...
                if (result.success) {
                    // 自动应用结果并获取实际分数
                    var applyResult = Optimizer.applyResult(result);
                    var actualScore = applyResult.actualScore || result.score;
                    
                    var msg = "自动计算完成！得分: " + actualScore + "，耗时: " + (result.timeMs / 1000).toFixed(1) + "秒";
                    if (targetScore && actualScore >= targetScore) {
                        msg += "（已达到目标分数 " + targetScore + "）";
                    } else if (targetScore) {
                        msg += "（未达到目标分数 " + targetScore + "）";
                    }
                    if (result.satietyOk === false) {
                        msg += "（饱食度未完全达标，累计偏差 " + (result.satietyDiff || 0) + "）";
                    }
                    showAlert(msg, "成功");
                } else {
                    showAlert(result.message || "未找到满足条件的配置", "提示");
                }
            }
        );
    });
...
// ==================== 厨师组合保存/加载 ====================
function _getChefComboStorageKey() {
    var userName = typeof getCurrentUserName === 'function' ? getCurrentUserName() : '';
    return (userName ? userName + '_' : '') + 'competition_chef_combos';
}

function _loadChefCombos() {
    try {
        var data = localStorage.getItem(_getChefComboStorageKey());
        return data ? JSON.parse(data) : [];
    } catch(e) { return []; }
}

function _saveChefCombos(combos) {
    try {
        localStorage.setItem(_getChefComboStorageKey(), JSON.stringify(combos));
    } catch(e) {}
}

function _refreshChefComboSelect() {
    var $sel = $("#select-chef-combo");
    $sel.empty();
    $sel.append('<option value="" selected disabled>厨师组合</option>');
    var combos = _loadChefCombos();
    for (var i = 0; i < combos.length; i++) {
        var c = combos[i];
        $sel.append('<option value="' + i + '" title="' + c.names.join(', ') + '">' + c.label + '</option>');
    }
    // 添加"清空所有组合"选项
    if (combos.length > 0) {
        $sel.append('<option data-divider="true"></option>');
        $sel.append('<option value="__clear_all__" class="text-danger">清空所有组合</option>');
    }
...
// 保存按钮
$(document).on("click", "#btn-save-chef-combo", function() {
    var selectedIds = $("#select-competition-chefs").val();
    if (!selectedIds || selectedIds.length === 0) {
        if (typeof showAlert === 'function') showAlert('请先选择厨师');
        return;
    }
    // 获取厨师名称
    var names = [];
    $("#select-competition-chefs option").each(function() {
        if (selectedIds.indexOf($(this).val()) >= 0) {
            names.push($(this).text());
        }
    });
    var label = names.length <= 3 ? names.join('+') : names.slice(0, 3).join('+') + '等' + names.length + '人';
    
    var combos = _loadChefCombos();
    // 检查是否已存在相同组合
    var sortedIds = selectedIds.slice().sort();
    var exists = false;
    for (var i = 0; i < combos.length; i++) {
        var existIds = combos[i].ids.slice().sort();
        if (existIds.length === sortedIds.length && existIds.join(',') === sortedIds.join(',')) {
            exists = true;
            break;
        }
    }
    if (exists) {
        if (typeof showAlert === 'function') showAlert('该组合已存在');
        return;
    }
    combos.push({ ids: selectedIds, names: names, label: label });
    _saveChefCombos(combos);
    _refreshChefComboSelect();
});

// 组合选择框变更
$(document).on("changed.bs.select", "#select-chef-combo", function() {
    var val = $(this).val();
    ...
    var combos = _loadChefCombos();
    if (combos[idx]) {
        var ids = combos[idx].ids;
        // 只选择当前下拉框中存在的厨师ID
        var validIds = [];
        $("#select-competition-chefs option").each(function() {
            if (ids.indexOf($(this).val()) >= 0) {
                validIds.push($(this).val());
            }
        });
        $("#select-competition-chefs").selectpicker('val', validIds);
    }
    // 重置选择框回默认
    $(this).selectpicker('val', '');
});
...
// ==================== 厨神大赛自动计算优化器事件处理 ====================
// 全局变量：用于控制是否使用追加模式（重新跑分时不清空前10名列表）
var _competitionAppendMode = false;

$(function() {
    $("#btn-competition-auto").click(function() {
        // 检查是否已导入算法
        if (typeof window.CompetitionOptimizer === 'undefined') {
            showAlert('请先导入算法文件！<br><br>点击右侧的设置按钮（齿轮图标）导入算法文件。', '提示');
            return;
        }
        
        var tierVal = $("#select-competition-tier").val();
        var targetScore = null;
        if (tierVal && tierVal !== '-1') {
            targetScore = parseInt(tierVal);
            if (isNaN(targetScore) || targetScore <= 0) targetScore = null;
        ...
        $("#competition-progress-bar").css("width", "0%");
        $("#competition-progress-text").text("0%");
        $("#competition-progress-wrapper").removeClass("hidden");

        var initSuccess = window.CompetitionOptimizer.init(calCustomRule.gameData);
        if (!initSuccess) {
            $btn.prop("disabled", false).html(originalText);
        ...
        // 读取追加模式标志，然后立即重置
        var appendMode = _competitionAppendMode;
        _competitionAppendMode = false;
        
        // 读取重跑索引
        var rerunIndex = window._competitionRerunIndex;
        window._competitionRerunIndex = undefined;

        window.CompetitionOptimizer.optimize(
            targetScore,
            function(info) {
                var percent = info.progress || 0;
                var currentScore = info.score || 0;
                $("#competition-progress-bar").css("width", percent + "%");
                var text = percent + '%';
                if (currentScore > 0) text += '  ' + currentScore;
                $("#competition-progress-text").text(text);
            },
            function(result) {
                $btn.prop("disabled", false).html(originalText);
                $("#competition-progress-wrapper").addClass("hidden");
                ...
                if (result.success) {
                    var msg = "自动计算完成！得分: " + result.score;
                    if (result.passLine) msg += " " + result.passLine;
                    msg += "，耗时: " + (result.timeMs / 1000).toFixed(1) + "秒";
                    if (targetScore && result.score >= targetScore) {
                        msg += "（已达到目标分数 " + targetScore + "）";
                    } else if (targetScore) {
                        msg += "（未达到目标分数 " + targetScore + "）";
                    }
                    ...
                    // 如果是重跑模式，更新组合的重跑状态并刷新Modal显示
                    if (appendMode && rerunIndex !== undefined) {
                        try {
                            var combo = CompetitionOptimizer.Top10Manager.getCombo(rerunIndex);
                            if (combo) {
                                combo.rerunStatus = 'rerun';
                                
                                // 如果Modal是打开的，刷新显示
                                if ($('#top10-modal').hasClass('in')) {
                                    var list = CompetitionOptimizer.Top10Manager.getList();
                                    renderModal(list);
                                }
                            }
                        } catch (e) {
            ...
            { appendMode: appendMode }  // 传递追加模式参数
        );
    });
});

/**
 * 初始化风云宴/江湖帖/贤客楼贵客选择下拉框
 * 根据当前规则动态生成选项：双贵客 + 单独的贵客1 + 单独的贵客2
 * 同时初始化级别和档位下拉框
 */
function initBanquetGuestSelector() {
    // 判断是否为江湖帖/贤客楼
    var currentRule = calCustomRule && calCustomRule.rules && calCustomRule.rules[0];
    var ruleTitle = currentRule ? currentRule.Title : '';
    var isJianghuRule = ruleTitle.indexOf('江湖帖') >= 0 || ruleTitle.indexOf('贤客楼') >= 0;
    var isXiankeRule = ruleTitle.indexOf('贤客楼') >= 0;
    var $settingsBtn = $("#btn-banquet-settings");
    
    var $select = $("#select-banquet-guest");
    var $level = $("#select-banquet-level");
    
    if (isJianghuRule) {
        if ($settingsBtn.length) {
            $settingsBtn.hide().prop("disabled", true);
        }
        $("#banquet-settings-modal").hide();
        // 江湖帖/贤客楼：隐藏省级和贵客组合下拉框
        if ($select.length) {
            $select.closest('.select-wrapper').hide();
        }
        if ($level.length) {
            $level.closest('.select-wrapper').hide();
        }
    } else {
        if ($settingsBtn.length) {
            $settingsBtn.show().prop("disabled", false);
        }
        // 风云宴：显示省级和贵客组合下拉框
        if ($select.length) {
            $select.closest('.select-wrapper').show();
            ...
            // 添加"双贵客"选项
            var allTitle = "";
            if (calCustomRule.rules.length >= 2) {
                allTitle = (calCustomRule.rules[0].Title || "贵客1") + " + " + (calCustomRule.rules[1].Title || "贵客2");
            } else if (calCustomRule.rules.length === 1) {
                allTitle = calCustomRule.rules[0].Title || "贵客1";
            } else {
                allTitle = "双贵客";
            }
            $select.append('<option value="all">' + allTitle + '</option>');
            
            // 添加单独贵客选项
            for (var i = 0; i < calCustomRule.rules.length; i++) {
                var title = calCustomRule.rules[i].Title || ("贵客" + (i + 1));
            ...
            if ($bsContainer.length) {
                var curW = $bsContainer.outerWidth();
                if (curW > 20) $bsContainer.css('width', (curW - 5) + 'px');
            }
            ...
            $select.off("changed.bs.select").on("changed.bs.select", function() {
                onBanquetGuestChange();
            });
        }
        
        if ($level.length) {
            $level.closest('.select-wrapper').show();
        }
    }
    
    // 重置优化器过滤器和UI显示状态
    if (typeof BanquetOptimizer !== 'undefined') {
        BanquetOptimizer.setGuestFilter(null);
    }
    for (var i = 0; i < calCustomRule.rules.length; i++) {
        $(".cal-custom-item:eq(" + i + ")").show();
    }
    
    // 初始化级别下拉框（风云宴才需要）
    if (!isJianghuRule) {
        _initBanquetLevelSelector();
    }
    
    // 先初始化档位selectpicker
    var $tier = $("#select-banquet-tier");
    if ($tier.length) {
        try { $tier.selectpicker("destroy"); } catch(e) {}
        $tier.selectpicker();
    }
    
    // 初始化档位下拉框内容（根据当前贵客和级别）
    _updateBanquetTierSelector();
    
}

/**
 * 根据当前规则类型获取对应的优化器
 * @returns {Object} BanquetOptimizer 或 JianghuOptimizer
 */
function _getCurrentOptimizer() {
    var currentRule = calCustomRule && calCustomRule.rules && calCustomRule.rules[0];
    var ruleTitle = currentRule ? currentRule.Title : '';
    var isJianghuRule = ruleTitle.indexOf('江湖帖') >= 0 || ruleTitle.indexOf('贤客楼') >= 0;
    var isXiankeRule = ruleTitle.indexOf('贤客楼') >= 0;
    ...
    return isJianghuRule ? JianghuOptimizer : BanquetOptimizer;
}

/**
 * 初始化风云宴级别下拉框
 */
function _initBanquetLevelSelector() {
    var $level = $("#select-banquet-level");
    if (!$level.length) return;
    
    try { $level.selectpicker("destroy"); } catch(e) {}
    $level.selectpicker();
    ...
    // 绑定切换事件
    $level.off("changed.bs.select").on("changed.bs.select", function() {
        _onBanquetLevelChange();
    });
}
 ...
 * 级别切换时的处理
 */
function _onBanquetLevelChange() {
    var levelIndex = parseInt($("#select-banquet-level").val());
    
    var Optimizer = _getCurrentOptimizer();
    if (typeof Optimizer === 'undefined') return;
    var levels = Optimizer.BANQUET_LEVELS;
    if (!levels || !levels[levelIndex]) return;
    
    var isDual = levels[levelIndex].dual;
    var $guest = $("#select-banquet-guest");
    
    // 单贵客模式（郡/县/村）：只显示第一个贵客，隐藏双贵客和第二个贵客
    // 双贵客模式（省/州）：显示全部选项（组合+单独贵客）
    $guest.find('option').each(function() {
        var val = $(this).val();
        if (isDual) {
            // 双贵客模式：所有选项都可用
    ...
    if (isDual) {
        // 省/州 = 双客人，选择"all"
        $guest.selectpicker("val", "all");
        onBanquetGuestChange();
    } else {
        // 郡/县/村 = 单客人，选择第一个贵客
        $guest.selectpicker("val", "0");
        onBanquetGuestChange();
    }
...
/**
 * 更新档位下拉框的选项（根据当前贵客组合和级别）
 */
function _updateBanquetTierSelector() {
    var $tier = $("#select-banquet-tier");
    ...
    var levelIndex = parseInt($("#select-banquet-level").val()) || 0;
    
    // 获取当前贵客组合的Title
    var title1 = null, title2 = null;
    if (calCustomRule.rules.length >= 1) {
        title1 = calCustomRule.rules[0].Title;
    }
    if (calCustomRule.rules.length >= 2) {
        title2 = calCustomRule.rules[1].Title;
    }
    
    var key = Optimizer.findTierScoreKey(title1, title2);
    
    // 清空并重建选项
    $tier.empty();
    $tier.append('<option value="-1">不限制分数</option>');
    
    if (key) {
        if (isXiankeRule) {
            var xiankeTierMatch = ruleTitle.match(/贤客楼\s+[^\s]+\s+([一二三四五])\s+/);
            var xiankeTierIndexMap = {
                '五': 0,
                '四': 1,
                '三': 2,
                '二': 3,
            ...
            var xiankeTierIndex = xiankeTierMatch ? xiankeTierIndexMap[xiankeTierMatch[1]] : undefined;
            if (xiankeTierIndex !== undefined) {
                var xiankeScore = Optimizer.getTierScore(key, 0, xiankeTierIndex);
                if (xiankeScore) {
                    var xiankeTierNum = 5 - xiankeTierIndex;
                    $tier.append('<option value="' + xiankeScore + '">' + xiankeTierNum + '档 ' + xiankeScore + '分</option>');
                }
            } else {
                // 异常标题兜底为完整档位列表，避免下拉为空
                for (var ti = 0; ti < 5; ti++) {
                    var fallbackXiankeScore = Optimizer.getTierScore(key, 0, ti);
                    if (fallbackXiankeScore) {
                        var fallbackXiankeTierNum = 5 - ti;
                        $tier.append('<option value="' + fallbackXiankeScore + '">' + fallbackXiankeTierNum + '档 ' + fallbackXiankeScore + '分</option>');
                    }
                }
            }
        } else if (isJianghuRule) {
            // 江湖帖：根据规则标题判断是普通还是特级
            // 标题格式："江湖帖 铁拐李" = 普通，"江湖帖 铁拐李 特" = 特级
            var isSpecial = ruleTitle.indexOf(' 特') >= 0 || ruleTitle.endsWith('特');
            
            if (isSpecial) {
                // 只显示特级分数（不显示"特级"文字）
                var specialScore = Optimizer.getTierScore(key, 0, 0);
                if (specialScore) {
                    $tier.append('<option value="' + specialScore + '">' + specialScore + '分</option>');
                }
            } else {
                // 只显示普通分数（不显示"普通"文字）
                var normalScore = Optimizer.getTierScore(key, 0, 1);
                if (normalScore) {
                    $tier.append('<option value="' + normalScore + '">' + normalScore + '分</option>');
                }
            }
        } else {
            // 风云宴：5个档位
            for (var ti = 0; ti < 5; ti++) {
                var score = Optimizer.getTierScore(key, levelIndex, ti);
                if (score) {
                    var tierNum = 5 - ti; // 5档, 4档, 3档, 2档, 1档
                    var scoreStr = '' + score;
                    $tier.append('<option value="' + score + '">' + tierNum + '档 ' + scoreStr + '分</option>');
                }
            }
        }
    }
    ...
    // 刷新selectpicker
    $tier.selectpicker("refresh");
    
    // 默认选中第一个分数档位
    var $firstScore = $tier.find('option[value!="-1"]').first();
    if ($firstScore.length) {
        $tier.selectpicker("val", $firstScore.val());
    }
}
 ...
 * 贵客选择切换时的处理函数
 */
function onBanquetGuestChange() {
    var selectedValue = $("#select-banquet-guest").val();
    
    // 切换贵客时清空场上已选
    var gd = calCustomRule.gameData;
    if (typeof clearAllSelectedOnField === 'function' && gd) {
        clearAllSelectedOnField(gd);
    }
    ...
    if (selectedValue === "all") {
        ...
        for (var i = 0; i < calCustomRule.rules.length; i++) {
            if (i === selectedIndex) {
                ...
                $(".cal-custom-item:eq(" + i + ")").hide();
            }
        }
        // 设置风云宴优化器只计算选中的贵客
        if (typeof BanquetOptimizer !== 'undefined') {
            BanquetOptimizer.setGuestFilter(selectedIndex);
        }
    }
...
/**
 * CompetitionTop10UI - 厨神大赛前10名组合的 UI 管理模块
 * 
 * 职责：
 * - 显示"查看前10名"按钮
 * - 渲染前10名 Modal
 ...
 * 
 * 依赖：
 * - CompetitionOptimizer.Top10Manager（数据层）
 * - CompetitionOptimizer._applySimStateToSystem（应用组合）
 */
var CompetitionTop10UI = (function() {
    'use strict';
    ...
    function showButton() {
        try {
            // 移除旧按钮（如果存在）
            $('#btn-view-top10').remove();

            var optimizer = window.CompetitionOptimizer;
            if (!optimizer || optimizer.supportsTop10 === false) {
                return;
            }

            // 创建按钮（始终显示，不显示数量统计）
            var button = $('<button>')
                .attr('id', 'btn-view-top10')
                .addClass('btn btn-success btn-sm chushen-only')
                .css({
                    'margin-left': '5px',
                    'vertical-align': 'top',
                    'margin-top': '1px'
            ...
            // 添加到算法设置按钮前面（在 #competition-auto-calc 容器内）
            // 这样按钮会跟随 #competition-auto-calc 容器的显示/隐藏逻辑
            var settingsBtn = $('#btn-competition-settings');
            if (settingsBtn.length > 0) {
                settingsBtn.before(button);
                
                // 检查 URL 参数，决定是否显示按钮
                var urlParams = new URLSearchParams(window.location.search);
                var isChushenMode = urlParams.get('chushen') === '123';
                if (!isChushenMode) {
                    button.hide();
                }
                return;
    ...
    function openModal() {
        try {
            ...
            // 尝试获取列表（如果优化器可用）
            if (typeof CompetitionOptimizer !== 'undefined' && CompetitionOptimizer.Top10Manager) {
                list = CompetitionOptimizer.Top10Manager.getList();
            }

            // 添加操作按钮到 Modal header
            addHeaderButtons();
    ...
    function addHeaderButtons() {
        // 移除旧按钮（如果存在）
        $('#top10-modal .modal-header .header-buttons').remove();

        // 创建按钮容器
        var buttonContainer = $('<span>')
            .addClass('header-buttons')
            .css({
                'float': 'right',
                'margin-right': '10px'
            });
        ...
        buttonContainer.append(importBtn).append(copyAllBtn);

        // 插入到标题后面（关闭按钮会自动在最右边）
        $('#top10-modal .modal-header .modal-title').after(buttonContainer);
    }
    ...
    function renderModal(list) {
        var container = $('#top10-modal-body');
        container.empty();

        if (!list || list.length === 0) {
            container.html('<p class="text-center text-muted">暂无数据</p>');
            return;
        }

        // 直接使用保存的分数，不重新计算（避免性能浪费）
        // 列表已经按分数降序排列
        for (var i = 0; i < list.length; i++) {
            var combo = list[i];
            var cardHtml = renderComboCard(combo, i, combo.score);
            container.append(cardHtml);
        }

        // 绑定应用按钮事件（使用事件委托）
        container.off('click', '.apply-combo-btn').on('click', '.apply-combo-btn', function() {
            var index = $(this).data('index');
            applyCombo(index);
        });

        // 绑定重新跑分按钮事件（使用事件委托）
        container.off('click', '.rerun-combo-btn').on('click', '.rerun-combo-btn', function() {
            var index = $(this).data('index');
            rerunCombo(index);
        });

        // 绑定复制按钮事件（使用事件委托）
        container.off('click', '.copy-combo-btn').on('click', '.copy-combo-btn', function() {
            var index = $(this).data('index');
            copyCombo(index);
        });
    }
    ...
    /**
     * 渲染单个组合卡片
     * @param {object} combo - 组合数据
     * @param {number} index - 显示的排名索引（0-based）
     * @param {number} displayScore - 显示的分数
     */
    function renderComboCard(combo, index, displayScore) {
        var card = $('<div>').addClass('combo-card').css({
        ...
        // 使用传入的分数
        var score = displayScore !== undefined ? displayScore : combo.score;

        // 卡片头部：排名、分数、来源、应用按钮
        var header = $('<div>').addClass('combo-header').css({
            'margin-bottom': '10px',
            'padding-bottom': '10px',
        ...
        var leftSection = $('<div>').css('flex', '1');

        var rank = $('<span>').addClass('combo-rank').css({
            'font-weight': 'bold',
            'font-size': '16px',
            'color': index === 0 ? '#d9534f' : '#333'
        }).text('第 ' + (index + 1) + ' 名');

        var scoreSpan = $('<span>').addClass('combo-score').css({
            'margin-left': '20px',
            'font-size': '16px',
            'color': '#5cb85c'
        }).text('分数: ' + displayScore.toLocaleString());

        var source = $('<span>').addClass('combo-source').css({
            'margin-left': '20px',
            'font-size': '14px',
            'color': '#777'
        }).text('来源: ' + combo.source);

        leftSection.append(rank).append(scoreSpan).append(source);

        // 按钮组：重新跑分按钮 + 复制按钮 + 应用按钮
        ...
        // 检查重跑状态
        var rerunStatus = combo.rerunStatus || 'not_run'; // 'not_run', 'rerun'
        var rerunBtnText = rerunStatus === 'rerun' ? '已重跑' : '重新跑分';
        var rerunBtnClass = rerunStatus === 'rerun' ? 'btn-default' : 'btn-warning';

        var rerunBtn = $('<button>')
            .addClass('btn btn-sm rerun-combo-btn ' + rerunBtnClass)
            .attr('data-index', index)
            .attr('data-rerun-status', rerunStatus)
            .text(rerunBtnText);

        var copyBtn = $('<button>')
            .addClass('btn btn-success btn-sm copy-combo-btn')
            .attr('data-index', index)
            .text('复制');

        var applyBtn = $('<button>')
            .addClass('btn btn-primary btn-sm apply-combo-btn')
            .attr('data-index', index)
            .text('应用此方案');

        buttonGroup.append(rerunBtn).append(copyBtn).append(applyBtn);

        header.append(leftSection).append(buttonGroup);
        card.append(header);

        // 卡片主体：厨师和菜谱信息
        var body = $('<div>').addClass('combo-body');

        if (!combo.state || !combo.state[0]) {
            body.append($('<p>').text('数据不完整'));
            card.append(body);
            return card;
        }
        ...
        // 获取 chefMap（从优化器获取）
        var chefMap = null;
        if (typeof CompetitionOptimizer !== 'undefined' && CompetitionOptimizer._chefMap) {
            chefMap = CompetitionOptimizer._chefMap();
        }

        // 遍历3个厨师位置
        for (var ci = 0; ci < combo.state[0].length; ci++) {
            var chef = combo.state[0][ci];
            if (!chef || !chef.chefId) continue;

            var chefDiv = $('<div>').addClass('chef-position').css({
                'margin-bottom': '10px',
                'padding': '10px',
            ...
            // 厨师名称（从chefObj获取，避免依赖_chefMap）
            var chefName = '未知厨师';
            if (chef.chefObj && chef.chefObj.name) {
                chefName = chef.chefObj.name;
            } else if (chefMap && chefMap[chef.chefId]) {
                chefName = chefMap[chef.chefId].name;
            }

            var chefTitle = $('<h5>').css({
                'margin': '0 0 8px 0',
                'color': '#337ab7'
            }).text('位置' + (ci + 1) + ': ' + chefName);
            chefDiv.append(chefTitle);

            // 菜谱列表
            if (chef.recipes && chef.recipes.length > 0) {
                var recipeList = $('<ul>').css({
                    'margin': '5px 0',
                    'padding-left': '20px'
                });

                for (var ri = 0; ri < chef.recipes.length; ri++) {
                    var recipe = chef.recipes[ri];
                    if (!recipe.data) continue;

                    var recipeName = recipe.data.name || '未知菜谱';
                    var quantity = recipe.quantity || 0;
                    
                    var recipeItem = $('<li>').text(recipeName + ' x ' + quantity);
                    recipeList.append(recipeItem);
                }

                chefDiv.append(recipeList);
            }

            // 装备、遗玉、调料信息
            var equipAmberDiv = $('<div>').css({
                'font-size': '12px',
                'color': '#666',
                'margin-top': '5px'
            });
            ...
            // 装备
            if (chef.equipObj && chef.equipObj.name) {
                var equipSpan = $('<span>').text('装备: ' + chef.equipObj.name);
                equipAmberDiv.append(equipSpan);
            }

            // 遗玉
            if (chef.chefObj && chef.chefObj.disk && chef.chefObj.disk.ambers) {
                var ambers = chef.chefObj.disk.ambers;
                var amberNames = [];
                for (var ai = 0; ai < ambers.length; ai++) {
                    if (ambers[ai].data && ambers[ai].data.name) {
                        amberNames.push(ambers[ai].data.name);
                    }
                }
                if (amberNames.length > 0) {
                    var amberSpan = $('<span>').css('margin-left', '15px')
                        .text('遗玉: ' + amberNames.join(', '));
                    equipAmberDiv.append(amberSpan);
                }
            }

            // 调料
            if (chef.condiment && chef.condiment.condimentId) {
                var condimentName = '未知调料';
                if (chef.condiment.name) {
                    condimentName = chef.condiment.name;
                }
                var condimentSpan = $('<span>').css('margin-left', '15px')
                    .text('调料: ' + condimentName);
                equipAmberDiv.append(condimentSpan);
            }

            chefDiv.append(equipAmberDiv);
            body.append(chefDiv);
        }
     ...
     * 应用选定的组合
     */
    function applyCombo(index) {
        try {
            if (typeof CompetitionOptimizer === 'undefined' || !CompetitionOptimizer.Top10Manager) {
                throw new Error('CompetitionOptimizer 不可用');
            }

            var combo = CompetitionOptimizer.Top10Manager.getCombo(index);
            
            if (!combo || !combo.state) {
                throw new Error('组合数据不完整');
            }

            var applied = false;

            // 直接使用通用写回逻辑，避免外部算法原生写回在部分状态下抛出空值异常
            applyComboStateDirect(combo.state);
            applied = isComboApplied(combo);

            if (!applied) {
                throw new Error('方案写回失败');
            }

            // 关闭Modal（不显示成功消息）
            $('#top10-modal').modal('hide');
            
        } catch (e) {
            if (typeof console !== 'undefined' && console.error) {
                console.error('[Top10] 应用方案失败:', e);
            }
            if (typeof showAlert === 'function') {
                showAlert('应用方案失败：' + e.message, '错误');
            } else {
                alert('应用方案失败：' + e.message);
            }
        }
    }

    /**
     * 重新跑分：使用该组合的三个厨师重新触发自动计算
     */
    function rerunCombo(index) {
        try {
            if (typeof CompetitionOptimizer === 'undefined' || !CompetitionOptimizer.Top10Manager) {
                throw new Error('CompetitionOptimizer 不可用');
            }
            ...
            if (!combo || !combo.state || !combo.state[0]) {
                throw new Error('组合数据不完整');
            }

            // 检查重跑状态，如果已重跑则弹窗确认
            if (combo.rerunStatus === 'rerun') {
                if (!confirm('此组合已经重跑过，是否再次重跑？')) {
                    return;
                }
            }
            ...
            for (var ci = 0; ci < combo.state[0].length; ci++) {
                var chef = combo.state[0][ci];
                if (chef && chef.chefId) {
                    chefIds.push(chef.chefId.toString());
                }
            }

            if (chefIds.length === 0) {
                throw new Error('没有找到厨师信息');
            }

            // 设置厨师选择下拉框的值
            var $chefSelect = $('#select-competition-chefs');
            if ($chefSelect.length === 0) {
                throw new Error('找不到厨师选择下拉框');
            }

            // 设置选中的厨师
            $chefSelect.val(chefIds);
            $chefSelect.selectpicker('refresh');

            // 关闭Modal
            $('#top10-modal').modal('hide');

            // 设置追加模式标志（不清空前10名列表）
            window._competitionAppendMode = true;
            
            // 保存当前重跑的组合索引，用于优化完成后更新状态
            window._competitionRerunIndex = index;

            // 触发自动计算按钮点击
            setTimeout(function() {
                $('#btn-competition-auto').click();
            }, 300);
            
        } catch (e) {
            alert('重新跑分失败: ' + e.message);
        }
    }
    ...
    function copyCombo(index) {
        try {
            ...
            // 生成导出数据（与页面导出格式一致）
            var exportData = generateExportData(combo);
    ...
    function copyAllCombos() {
        try {
            ...
            if (!list || list.length === 0) {
                throw new Error('没有可复制的组合');
            }

            // 生成所有组合的导出数据
            var allExportData = [];
            for (var i = 0; i < list.length; i++) {
                var exportData = generateExportData(list[i]);
                exportData._rank = i + 1; // 添加排名信息
                allExportData.push(exportData);
            }

            var jsonStr = JSON.stringify(allExportData, null, 2);
    ...
    function showImportModal() {
        try {
            // 清空输入框和消息
            $('#top10-import-textarea').val('');
            $('#top10-import-message').hide().removeClass('alert-success alert-danger');
            
            // 显示导入Modal
            $('#top10-import-modal').modal('show');
    ...
    function importCombo() {
        try {
            var jsonStr = $('#top10-import-textarea').val().trim();
            
            if (!jsonStr) {
                showImportMessage('请输入组合数据', 'danger');
                return;
            }
            ...
            // 检查是否是单个组合还是多个组合
            var combos = Array.isArray(importData) ? importData : [importData];
            
            if (combos.length === 0) {
                showImportMessage('没有有效的组合数据', 'danger');
                return;
            }

            // 检查 CompetitionOptimizer 是否可用
            if (typeof CompetitionOptimizer === 'undefined' || !CompetitionOptimizer.Top10Manager) {
                showImportMessage('优化器未加载，请先导入算法文件', 'danger');
                return;
            }

            // 将所有组合添加到 Top10Manager
            var successCount = 0;
            for (var i = 0; i < combos.length; i++) {
                var combo = combos[i];
                
                // 验证数据格式
                if (!combo.chefs || !combo.recipes || !combo.score) {
                    continue;
                }

                // 重建 simState 格式
                var simState = reconstructSimState(combo);
                if (!simState) {
                    continue;
                }

                // 添加到 Top10Manager
                var source = combo.source || '导入';
                var inserted = CompetitionOptimizer.Top10Manager.tryInsert(
                    combo.score,
                    simState,
                    source
                );
            ...
            // 关闭导入Modal
            $('#top10-import-modal').modal('hide');
            
            // 刷新主Modal显示（确保主Modal是打开的）
            var list = CompetitionOptimizer.Top10Manager.getList();
            renderModal(list);
            ...
            showImportMessage('导入失败：' + e.message, 'danger');
        }
    }
     ...
     * 从导出数据重建 simState
     */
    function reconstructSimState(importData) {
        try {
            var simState = [[]];
            
            for (var ci = 0; ci < 3; ci++) {
                var chefData = importData.chefs[ci];
                var recipeData = importData.recipes[ci];
                ...
                if (typeof calCustomRule !== 'undefined' && calCustomRule.rules && calCustomRule.rules[0]) {
                    ...
                    for (var i = 0; i < rule.chefs.length; i++) {
                        if (rule.chefs[i].chefId === chefData.chefId) {
                            chefObj = JSON.parse(JSON.stringify(rule.chefs[i]));
                            
                            // 应用导入的心法盘等级
                            if (chefData.diskLevel && chefObj.disk) {
                                chefObj.disk.level = chefData.diskLevel;
                            }
                            
                            // 应用导入的遗玉配置（覆盖厨师当前的遗玉）
                            if (chefData.ambers && chefObj.disk && chefObj.disk.ambers) {
                                for (var ai = 0; ai < chefObj.disk.ambers.length && ai < chefData.ambers.length; ai++) {
                                    var amberId = chefData.ambers[ai];
                                    if (amberId) {
                                        // 从 rule.ambers 中查找遗玉对象
                                        var amberObj = null;
                                        if (rule.ambers) {
                                            for (var aj = 0; aj < rule.ambers.length; aj++) {
                                                if (rule.ambers[aj].amberId === amberId) {
                                                    amberObj = rule.ambers[aj];
                                                    break;
                                                }
                                            }
                                        }
                                        chefObj.disk.ambers[ai].data = amberObj;
                                    } else {
                                        // 如果导入数据中遗玉为 null，清空该槽位
                                        chefObj.disk.ambers[ai].data = null;
                                    }
                                }
                            }
                ...
                // 获取装备对象（使用导入数据中的厨具ID）
                var equipObj = {};
                if (chefData.equipId && typeof getEquipInfo === 'function') {
                    if (typeof calCustomRule !== 'undefined' && calCustomRule.rules && calCustomRule.rules[0]) {
                        equipObj = getEquipInfo(chefData.equipId, calCustomRule.rules[0].equips) || {};
                    }
                }

                // 获取调料对象
                var condiment = {};
                if (chefData.condimentId) {
                    // 调料信息需要从 gameData 获取
                    if (typeof calCustomRule !== 'undefined' && calCustomRule.gameData) {
                        var gameData = calCustomRule.gameData;
                        for (var j = 0; j < gameData.condiments.length; j++) {
                            if (gameData.condiments[j].condimentId === chefData.condimentId) {
                                condiment = gameData.condiments[j];
                                break;
                            }
                        }
                    }
                ...
                for (var ri = 0; ri < 3; ri++) {
                    if (recipeData && recipeData[ri]) {
                        var recipeId = recipeData[ri].recipeId;
                        var quantity = recipeData[ri].quantity || 0;
                        
                        // 获取菜谱对象
                        var recipeObj = null;
                        if (typeof calCustomRule !== 'undefined' && calCustomRule.gameData) {
                            var gameData = calCustomRule.gameData;
                            for (var j = 0; j < gameData.recipes.length; j++) {
                                if (gameData.recipes[j].recipeId === recipeId) {
                                    recipeObj = gameData.recipes[j];
                                    break;
                                }
                            }
                        }
                        ...
                        recipes.push({data: null, quantity: 0, max: 0});
                    }
                }
     ...
     * 生成导出数据（与页面导出格式一致）
     */
    function generateExportData(combo) {
        var exportData = {
            score: combo.score,
            source: combo.source,
        ...
        for (var ci = 0; ci < combo.state[0].length; ci++) {
            ...
            var chefData = {
                chefId: chef.chefId,
                equipId: chef.equipObj && chef.equipObj.equipId ? chef.equipObj.equipId : null,
                condimentId: chef.condiment && chef.condiment.condimentId ? chef.condiment.condimentId : null,
                diskLevel: chef.chefObj && chef.chefObj.disk && chef.chefObj.disk.level ? chef.chefObj.disk.level : 1,
                ambers: []
            };

            // 遗玉信息
            if (chef.chefObj && chef.chefObj.disk && chef.chefObj.disk.ambers) {
                for (var ai = 0; ai < chef.chefObj.disk.ambers.length; ai++) {
                    var amber = chef.chefObj.disk.ambers[ai];
                    chefData.ambers.push(amber.data && amber.data.amberId ? amber.data.amberId : null);
                }
            }

            exportData.chefs.push(chefData);
            ...
            for (var ri = 0; ri < chef.recipes.length; ri++) {
                ...
                if (recipe.data) {
                    ...
                    recipeData.push(null);
                }
            }
            exportData.recipes.push(recipeData);
        }
     ...
     * 应用导入的组合数据
     */
    function applyImportedCombo(importData) {
        if (!importData.chefs || !importData.recipes) {
        ...
        // 先清空已有配置，避免旧的厨师/厨具/菜谱残留
        for (var clearCi = 0; clearCi < 3; clearCi++) {
            ...
            if (typeof setCustomCondiment === 'function' && gameData) {
                setCustomCondiment(0, clearCi, null, gameData);
            }
        ...
        // 设置厨师和菜谱
        for (var ci = 0; ci < importData.chefs.length && ci < 3; ci++) {
            var chefData = importData.chefs[ci];
            if (!chefData || !chefData.chefId) {
                continue;
            }
            
            // 设置厨师
            if (typeof setCustomChef === 'function') {
                setCustomChef(0, ci, chefData.chefId);
            }

            // 设置心法盘等级
            if (chefData.diskLevel && typeof setCustomDiskLevel === 'function') {
                setCustomDiskLevel(0, ci, chefData.diskLevel);
            }

            // 设置厨具
            if (chefData.equipId && typeof setCustomEquip === 'function') {
                setCustomEquip(0, ci, chefData.equipId);
            }

            // 设置遗玉（未配置的槽位也要清空）
            if (chefData.ambers && typeof setCustomAmber === 'function') {
                var amberSlots = [];
                if (typeof calCustomRule !== 'undefined' &&
                    calCustomRule.rules && calCustomRule.rules[0] &&
                    calCustomRule.rules[0].custom &&
                    calCustomRule.rules[0].custom[ci] &&
                    calCustomRule.rules[0].custom[ci].chef &&
                    calCustomRule.rules[0].custom[ci].chef.disk &&
                    calCustomRule.rules[0].custom[ci].chef.disk.ambers) {
                    amberSlots = calCustomRule.rules[0].custom[ci].chef.disk.ambers;
                }
                for (var ai = 0; ai < amberSlots.length; ai++) {
                    setCustomAmber(0, ci, ai, chefData.ambers[ai] || null);
                }
            }

            // 设置菜谱
            if (importData.recipes[ci]) {
                for (var ri = 0; ri < importData.recipes[ci].length && ri < 3; ri++) {
                    var recipeData = importData.recipes[ci][ri];
                    if (recipeData && recipeData.recipeId && typeof setCustomRecipe === 'function') {
                        setCustomRecipe(0, ci, ri, recipeData.recipeId);
                        
                        // 设置份数
                        if (recipeData.quantity > 0 && typeof setCustomRecipeQuantity === 'function') {
                            setCustomRecipeQuantity(0, ci, ri, recipeData.quantity);
                        }
                    }
                }
            }
            ...
            // 设置调料
            if (chefData.condimentId && typeof setCustomCondiment === 'function') {
                if (gameData) {
                    setCustomCondiment(0, ci, chefData.condimentId, gameData);
                }
            }
        }
    ...
    /**
     * 按原始 simState 结构直接应用方案
     */
    function applyComboStateDirect(simState) {
        if (!simState || !simState[0]) {
            throw new Error('方案状态无效');
        }

        var gameData = (typeof calCustomRule !== 'undefined' && calCustomRule && calCustomRule.gameData)
            ? calCustomRule.gameData : null;

        if (typeof clearAllSelectedOnField === 'function' && gameData) {
        ...
        // 第二步：设置菜谱
        for (var ci2 = 0; ci2 < simState[0].length; ci2++) {
            var slot2 = simState[0][ci2] || {};
            var recipes = slot2.recipes || [];
            for (var ri = 0; ri < recipes.length; ri++) {
                if (recipes[ri] && recipes[ri].data && typeof setCustomRecipe === 'function') {
                    setCustomRecipe(0, ci2, ri, recipes[ri].data.recipeId);
                }
            }
        }

        // 第三步：回写份数、厨具、心法盘、遗玉、调料
        for (var ci3 = 0; ci3 < simState[0].length; ci3++) {
            var slot3 = simState[0][ci3] || {};
            var recipes3 = slot3.recipes || [];

            for (var ri3 = 0; ri3 < recipes3.length; ri3++) {
                if (recipes3[ri3] && recipes3[ri3].data && typeof setCustomRecipeQuantity === 'function') {
                    setCustomRecipeQuantity(0, ci3, ri3, recipes3[ri3].quantity || 0);
                }
            }

            if (slot3.equipObj && slot3.equipObj.equipId && typeof setCustomEquip === 'function') {
                setCustomEquip(0, ci3, slot3.equipObj.equipId);
            }

            if (slot3.chefObj && slot3.chefObj.disk && slot3.chefObj.disk.level && typeof setCustomDiskLevel === 'function') {
                setCustomDiskLevel(0, ci3, slot3.chefObj.disk.level);
            }

            if (slot3.chefObj && slot3.chefObj.disk && slot3.chefObj.disk.ambers && typeof setCustomAmber === 'function') {
                for (var ai = 0; ai < slot3.chefObj.disk.ambers.length; ai++) {
                    var amberData = slot3.chefObj.disk.ambers[ai] && slot3.chefObj.disk.ambers[ai].data
                        ? slot3.chefObj.disk.ambers[ai].data : null;
                    setCustomAmber(0, ci3, ai, amberData ? amberData.amberId : null);
                }
            }

            if (slot3.condiment && slot3.condiment.condimentId && typeof setCustomCondiment === 'function' && gameData) {
                setCustomCondiment(0, ci3, slot3.condiment.condimentId, gameData);
            }
        }
     ...
     * 检查组合是否已正确写回页面
     */
    function isComboApplied(combo) {
        try {
            if (!combo || !combo.state || !combo.state[0] ||
                typeof calCustomRule === 'undefined' || !calCustomRule.rules || !calCustomRule.rules[0]) {
                return false;
            }

            var custom = calCustomRule.rules[0].custom || [];
            for (var ci = 0; ci < 3; ci++) {
                var slot = combo.state[0][ci] || {};
                var pageSlot = custom[ci] || {};
                var pageChefId = pageSlot.chef && pageSlot.chef.chefId ? pageSlot.chef.chefId : null;
                var targetChefId = slot.chefId || null;
                if (String(pageChefId || '') !== String(targetChefId || '')) {
                    return false;
                }

                var targetEquipId = slot.equipObj && slot.equipObj.equipId ? slot.equipObj.equipId : null;
                var pageEquipId = pageSlot.equip && pageSlot.equip.equipId ? pageSlot.equip.equipId : null;
                if (String(pageEquipId || '') !== String(targetEquipId || '')) {
                    return false;
                }

                for (var ri = 0; ri < 3; ri++) {
                    var targetRecipe = slot.recipes && slot.recipes[ri] ? slot.recipes[ri] : null;
                    var pageRecipe = pageSlot.recipes && pageSlot.recipes[ri] ? pageSlot.recipes[ri] : null;
                    var targetRecipeId = targetRecipe && targetRecipe.data ? targetRecipe.data.recipeId : null;
                    var pageRecipeId = pageRecipe && pageRecipe.data ? pageRecipe.data.recipeId : null;
                    if (String(pageRecipeId || '') !== String(targetRecipeId || '')) {
                        return false;
                    }
                }
            }
    ...
    function copyToClipboardFallback(text) {
        ...
        if (!successful) {
            throw new Error('复制到剪贴板失败，请手动复制');
        }
    }
    ...
    function initEventListeners() {
        // 导入确认按钮
        $(document).off('click', '#btn-top10-import-confirm').on('click', '#btn-top10-import-confirm', function() {
            importCombo();
        });
        
        // 注意：showButton() 现在在规则加载完成后调用（在 initCalTables 的 setTimeout 中）
        // 这样可以确保 #competition-auto-calc 容器已经创建并设置了正确的显示/隐藏状态
    }
    ...
    return {
        ...
        openModal: openModal,
        rerunCombo: rerunCombo,
        copyCombo: copyCombo,
        copyAllCombos: copyAllCombos,
        importCombo: importCombo,
