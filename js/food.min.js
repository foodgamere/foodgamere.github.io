...
function updateRecipeChefTable(e) {
    ...
    setTimeout(function() {
        ...
        if (selectedGap != null && selectedGap.length > 0 && selectedGap[0] != "") {
            ...
            table.rows().every(function(rowIdx, tableLoop, rowLoop) {
                var data = this.data();
                table.cell(rowIdx, 18).data(data['exCount' + gapLevel]);
                table.cell(rowIdx, 19).data(data['exTimeDisp' + gapLevel]);
            });
            table.column(18).header().innerHTML = '熟练份数(' + gapNames[gapLevel] + ')';
            table.column(19).header().innerHTML = '熟练时间(' + gapNames[gapLevel] + ')';
            table.draw(false);
        }
        $(".loading").addClass("hidden");
    }, 0);
