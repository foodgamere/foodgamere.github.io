...
function getDailyMaterialDetails(custom, rule, totalTimeSeconds) {
    ...
    var dailyCycles = 86400 / totalTimeSeconds
      , materialNames = {}
      , materialTotals = {}
      , materialOrder = []
      , recipeDetails = []
...
function getDailyMaterialSummaryHtml(ruleIndex, materialDetails) {
    var selectId = "normal-daily-material-select-" + ruleIndex
      ...
      , html = '<span class="selected-sum-inline-materials"><span class="material-label">日均耗材：</span><select id="' + selectId + '" class="selectpicker material-daily-select normal-material-select" data-width="auto" data-container="body" data-none-selected-text="无耗材">';
    ...
    return html += '</select><button class="btn btn-default btn-sm btn-normal-material-detail" type="button" data-rule-index="' + ruleIndex + '"' + (buttonDisabled ? " disabled" : "") + '>耗材详情</button></span>';
}
...
function calCustomResults(e) {
    ...
    for (var a = 0, t = 0; t < calCustomRule.rules.length; t++) {
        ...
        if (i.showTime) {
            ...
            _ += " 总效率：" + F + "金币/小时";
            if (0 === calCustomRule.id && !calCustomRule.isGuestRate && !calCustomRule.isCultivate) {
                var dailyMaterialDetails = getDailyMaterialDetails(l, i, B);
                normalBusinessMaterialDetailsMap[t] = dailyMaterialDetails,
                _ += " " + getDailyMaterialSummaryHtml(t, dailyMaterialDetails)
            }
        }
        if (y.lackIds.length)
