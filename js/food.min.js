...
var CompetitionTop10UI = (function() {
    ...
    function applyCombo(index) {
        try {
            ...
            // 应用状态到系统（调用优化器的函数）
            if (typeof CompetitionOptimizer._applySimStateToSystem === 'function') {
                CompetitionOptimizer._applySimStateToSystem(combo.state);
            } else {
                throw new Error('_applySimStateToSystem 函数不可用');
            }
    ...
    function applyImportedCombo(importData) {
        ...
        for (var ci = 0; ci < importData.chefs.length && ci < 3; ci++) {
            ...
            // 设置厨师
            if (chefData.chefId && typeof setCustomChef === 'function') {
            ...
            if (chefData.ambers && typeof setCustomAmber === 'function') {
                for (var ai = 0; ai < chefData.ambers.length; ai++) {
                    if (chefData.ambers[ai]) {
                        setCustomAmber(0, ci, ai, chefData.ambers[ai]);
                    }
                }
            }
