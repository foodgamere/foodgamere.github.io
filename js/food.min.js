...
$(function() {
    ...
    $("#btn-banquet-auto").click(function() {
        ...
        // 初始化优化器
        var initSuccess = BanquetOptimizer.init(calCustomRule.gameData);
        if (!initSuccess) {
        ...
        BanquetOptimizer.optimize(
            ...
            function(result) {
                ...
                if (result.success) {
                    // 自动应用结果并获取实际分数
                    var applyResult = BanquetOptimizer.applyResult(result);
                    ...
                    if (appendMode && rerunIndex !== undefined) {
                        try {
                            ...
                            if (combo) {
                                ...
                                console.log('[Top10UI] 更新组合' + rerunIndex + '的重跑状态为已重跑');
                                
                                // 如果Modal是打开的，刷新显示
                                if ($('#top10-modal').hasClass('in')) {
                                    var list = CompetitionOptimizer.Top10Manager.getList();
                                    renderModal(list);
                                    console.log('[Top10UI] 刷新Modal显示');
                                }
                            }
                        } catch (e) {
                            console.error('[Top10UI] 更新重跑状态失败:', e);
                        }
                    }
                } else {
                    showAlert(result.message || "未找到满足条件的配置", "提示");
...
function _onBanquetLevelChange() {
    ...
    if (typeof BanquetOptimizer === 'undefined') return;
    var levels = BanquetOptimizer.BANQUET_LEVELS;
...
function _updateBanquetTierSelector() {
    ...
    // 单客人模式也需要用双贵客组合来查找档位分数
    var levels = BanquetOptimizer.BANQUET_LEVELS;
    var key = BanquetOptimizer.findTierScoreKey(title1, title2);
    ...
    if (key) {
        ...
        for (var ti = 0; ti < 5; ti++) {
            var score = BanquetOptimizer.getTierScore(key, levelIndex, ti);
            if (score) {
...
var CompetitionTop10UI = (function() {
    ...
    function showButton() {
        try {
            ...
            console.warn('[Top10UI] 未找到算法设置按钮，无法添加查看前10名按钮');
        } catch (e) {
            console.error('[Top10UI] 显示按钮失败:', e);
        }
    }
    ...
    function openModal() {
        try {
            ...
            console.error('[Top10UI] 打开Modal失败:', e);
        }
    }
    ...
    function applyCombo(index) {
        try {
            ...
            console.error('[Top10UI] 应用方案失败:', e);
        }
    }
    ...
    function rerunCombo(index) {
        try {
            ...
            console.log('[Top10UI] 重新跑分: 厨师=' + chefIds.join(',') + ', 追加模式=true, 索引=' + index);
            
        } catch (e) {
            console.error('[Top10UI] 重新跑分失败:', e);
    ...
    function copyCombo(index) {
        try {
            ...
            console.error('[Top10UI] 复制组合失败:', e);
        }
    }
    ...
    function copyAllCombos() {
        try {
            ...
            console.error('[Top10UI] 复制全部组合失败:', e);
        }
    }
    ...
    function showImportModal() {
        try {
            ...
            console.error('[Top10UI] 打开导入Modal失败:', e);
        }
    }
        ...
        try {
            ...
            for (var i = 0; i < combos.length; i++) {
                ...
                if (!combo.chefs || !combo.recipes || !combo.score) {
                    console.warn('[Top10UI] 跳过无效组合 #' + (i + 1));
                    continue;
                }
                ...
                if (!simState) {
                    console.warn('[Top10UI] 无法重建组合 #' + (i + 1));
                    continue;
                }
            ...
            console.error('[Top10UI] 导入组合失败:', e);
    ...
    function reconstructSimState(importData) {
        try {
            ...
            console.error('[Top10UI] 重建 simState 失败:', e);
            return null;
        }
    }
    ...
    function copyToClipboard(text) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text).then(function() {
                ...
                console.warn('[Top10UI] Clipboard API 失败，尝试备用方法:', err);
    ...
    function copyToClipboardFallback(text) {
        ...
        try {
            ...
            console.warn('[Top10UI] setSelectionRange 失败:', e);
        }
        ...
        try {
            ...
            if (successful) {
                ...
                console.error('[Top10UI] execCommand 返回 false');
            }
        } catch (e) {
            console.error('[Top10UI] execCommand 异常:', e);
        }
