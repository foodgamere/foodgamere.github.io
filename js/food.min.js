...
function initCalTables(e, a) {
    ...
    $("#chk-guest-query-mode").off("change").on("change", function() {
        var isPengciMode = $(this).prop("checked");
        ...
        if (isPengciMode) {
            // 碰瓷查询模式：隐藏计算结果区域，显示碰瓷查询区域
            $("#guest-rate-result").addClass("hidden");
            $("#pengci-query-area").removeClass("hidden");
            // 手机模式下显示绑定主线输入框
            if (isMobile) {
                $("#pengci-mainline-wrapper").removeClass("hidden");
            }
        } else {
            // 贵客查询模式：显示计算结果区域，隐藏碰瓷查询区域
...
function loadCalRule(e) {
    ...
    if (calCustomRule.isGuestRate) {
        ...
        shouldShowGuestRateResult = !isPengciMode;
        ...
        if (isMobile) {
            // 先把绑定主线输入框移动到总分上方容器
            $("#pengci-mainline-wrapper").prependTo("#mobile-mode-switch-container");
            // 再把碰瓷查询开关移动到顶部容器（在绑定主线输入框后面）
            ...
            for (var i in partialChefIds) {
                for (var j in e.chefs) {
                    if (e.chefs[j].chefId == partialChefIds[i]) {
                        ...
                        if (activeSelfUltimateEffects.length > 0) {
                            var ultAdd = getTimeAddition(activeSelfUltimateEffects);
                            if (ultAdd != 0) {
                                chefTimeAdd += ultAdd;
                            }
                        }
...
function setPartialUltimateOptions(e) {
    ...
    timeTypeChefs.sort(function(e, a) {
        ...
        var eActiveSelfUltimateEffects = getActiveSelfUltimateEffectsForTime(e.chef);
        if (eActiveSelfUltimateEffects.length > 0) {
            eTimeAdd += getTimeAddition(eActiveSelfUltimateEffects);
        }
        if (e.chef.equip && e.chef.equip.effect) {
            // 处理MutiEquipmentSkill类技能（使厨具技能效果翻倍）
            var eEquipEffect = updateEquipmentEffect(e.chef.equip.effect, eActiveSelfUltimateEffects);
        ...
        var aActiveSelfUltimateEffects = getActiveSelfUltimateEffectsForTime(a.chef);
        if (aActiveSelfUltimateEffects.length > 0) {
            aTimeAdd += getTimeAddition(aActiveSelfUltimateEffects);
        }
        if (a.chef.equip && a.chef.equip.effect) {
            // 处理MutiEquipmentSkill类技能（使厨具技能效果翻倍）
            var aEquipEffect = updateEquipmentEffect(a.chef.equip.effect, aActiveSelfUltimateEffects);
