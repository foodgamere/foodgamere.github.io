...
var GuestRateCalculator = (function($) {
    ...
    function normalizeRecipeRankValue(rankValue, recipe) {
        var normalizedRank = parseInt(rankValue, 10);
        if (!isNaN(normalizedRank) && normalizedRank > 0) {
            return normalizedRank;
        }

        var rankText = '';
        if (recipe) {
            rankText = recipe.rankDisp || (recipe.data && recipe.data.rank) || '';
        }
    ...
    function getRankTextByValue(rankValue) {
        switch (Number(rankValue) || 0) {
            case 1:
                return '可';
            case 2:
                return '优';
    ...
    function getAverageQualityLevelFromCurrentCustom(custom) {
        if (!custom) {
            return null;
        }
        ...
        for (var c in custom) {
            if (!custom.hasOwnProperty(c) || !custom[c] || !custom[c].recipes) {
                continue;
            }
        ...
        var averageRank = Math.floor(rankSum / rankCount);
        if (averageRank < 1) {
            averageRank = 1;
        } else if (averageRank > 5) {
            averageRank = 5;
        }
    ...
    function getActiveSelfUltimateEffects(chef, activeSelfUltimateMap) {
        ...
        var chefIdStr = String(chef.chefId || '');
        var effects = (activeSelfUltimateMap && activeSelfUltimateMap[chefIdStr]) || chef.selfUltimateEffect || [];
        if (effects.length > 0) {
            return effects;
        }
        
        if (typeof isAllUltimateMode !== 'undefined' && isAllUltimateMode && chef.ultimateSkillEffect) {
            var allUltimateSelfEffects = [];
            for (var i = 0; i < chef.ultimateSkillEffect.length; i++) {
                if (chef.ultimateSkillEffect[i].condition === 'Self') {
                    allUltimateSelfEffects.push(chef.ultimateSkillEffect[i]);
                }
            }
            return allUltimateSelfEffects;
        }
        
        return [];
    }

    function isOwnedValueTrue(value) {
        return value === "是" || value === true || value === 1 || value === "1";
    }

    function isLocalChefOwnedAndUltimatedForCalc(localData, chefId) {
        if (!localData || !localData.chefs) {
            return false;
        }
        for (var i = 0; i < localData.chefs.length; i++) {
            var localChef = localData.chefs[i];
            if (localChef && localChef.id == chefId && localChef.ult === "是" && isOwnedValueTrue(localChef.got)) {
                return true;
            }
        }
        return false;
    ...
    function hasUltimateSelectOption(selector, chefId) {
        if (typeof $ !== 'function') {
            return false;
        }
        return $(selector).find('option[value="' + String(chefId) + '"]').length > 0;
    }

    function isChefInUltimateConfigList(configUltimatedIds, listName, chefId) {
        if (!configUltimatedIds) {
            return false;
        }
        var chefIdStr = String(chefId);
        var setName = listName + 'Set';
        if (configUltimatedIds[setName] && configUltimatedIds[setName][chefIdStr]) {
            return true;
        }
        var list = configUltimatedIds[listName] || [];
        for (var i = 0; i < list.length; i++) {
            if (String(list[i]) === chefIdStr) {
                return true;
            }
        }
        return false;
    ...
    function isUltimateConditionActiveForChef(chef, condition, localData, configUltimatedIds) {
        if (!chef || !chef.chefId) {
            return false;
        }
        ...
        if (condition === 'Self') {
            listName = 'self';
            selector = '#chk-cal-self-ultimate';
        } else if (condition === 'Partial' || condition === 'Next') {
            listName = 'partial';
            selector = '#chk-cal-partial-ultimate';
        }
        ...
        if (isChefInUltimateConfigList(configUltimatedIds, listName, chef.chefId)) {
            return true;
        }
        
        if (hasUltimateSelectOption(selector, chef.chefId)) {
            return false;
        }
        
        return isLocalChefOwnedAndUltimatedForCalc(localData, chef.chefId);
    }
    
    /**
     * 使用与下方总时间一致的口径计算时间百分比
     * @private
     * @param {Object} custom - 当前场上配置
     * @param {Object} rule - 当前规则
     * @returns {number} 时间百分比
     */
    function calculateCurrentTimePercentage(custom, rule) {
        var timeAddition = 0;
        var disableChefSkillEffect = rule && rule.hasOwnProperty("DisableChefSkillEffect") && rule.DisableChefSkillEffect != 0;
        var disableEquipSkillEffect = rule && rule.hasOwnProperty("DisableEquipSkillEffect") && rule.DisableEquipSkillEffect != 0;
        var partialRecipeAdds = typeof getPartialRecipeAdds === 'function' ? getPartialRecipeAdds(custom, rule) : [];
        ...
        for (var c in custom) {
            ...
            if (!disableChefSkillEffect && chef.specialSkillEffect && typeof getTimeAddition === 'function') {
                timeAddition += getTimeAddition(chef.specialSkillEffect);
            }
            
            var activeSelfUltimateEffects = getActiveSelfUltimateEffects(chef);
            if (activeSelfUltimateEffects.length > 0 && typeof getTimeAddition === 'function') {
                timeAddition += getTimeAddition(activeSelfUltimateEffects);
            }
            
            if (!disableEquipSkillEffect && equip && equip.effect && typeof getTimeAddition === 'function') {
                var equipEffect = equip.effect;
                if (typeof updateEquipmentEffect === 'function') {
                    equipEffect = updateEquipmentEffect(equip.effect, activeSelfUltimateEffects);
                }
                timeAddition += getTimeAddition(equipEffect);
            }
        }
        
        if (partialRecipeAdds && partialRecipeAdds[0]) {
            for (var i = 0; i < partialRecipeAdds[0].length; i++) {
                var partialAdd = partialRecipeAdds[0][i];
                if (partialAdd && partialAdd.effect && partialAdd.effect.type === "OpenTime") {
                    timeAddition += partialAdd.effect.value || 0;
                }
            }
        }
        
        return 100 + +(timeAddition.toFixed(2));
    }

    function setSelectpickerValueSilently(selector, value) {
        var $select = $(selector);
        if (!$select.length) {
            return;
        }
    ...
    function parseTimeTextToSeconds(timeText) {
        ...
        var hoursMatch = timeText.match(/(\d+)\s*小时/);
        var minutesMatch = timeText.match(/(\d+)\s*分/);
        var secondsMatch = timeText.match(/(\d+)\s*秒/);
    ...
    function loadJadeRecipeValueMap() {
        $.ajax({
            url: 'data/jade-values.json?v=' + Date.now(),
            dataType: 'json',
            cache: false
        }).done(function(data) {
            jadeRecipeValueMap = normalizeJadeRecipeValueMap(data);
            syncJadeModeFields();
        }).fail(function() {
            jadeRecipeValueMap = {};
            syncJadeModeFields();
    ...
    function getCurrentJadeRecipeValueTotal() {
        ...
        for (var c in custom) {
            ...
            for (var r in recipes) {
                ...
                totalValue += Number(jadeRecipeValueMap[recipeData.name]) || 0;
            }
        }
    ...
    function calculateRuneModeOutputs() {
        ...
        return {
            ...
            dailyRune: runeDetails.totalDailyRune,
        ...
        for (var c in custom) {
            ...
            for (var r in recipes) {
                ...
                recipeEntries.push({
                    ...
                    totalTime: Number(recipeItem && recipeItem.totalTime) || 0,
    ...
    function getRuneModeRecipeDetails(actualGuestRate, critRate) {
        ...
        for (var i = 0; i < recipeEntries.length; i++) {
            ...
            recipeDetails.push({
                ...
                positionIndex: recipeEntry.positionIndex,
                slotIndex: recipeEntry.slotIndex,
                totalTime: totalTime,
                dailyRune: dailyRune,
                rankText: rankText,
        ...
        recipeDetails.sort(function(a, b) {
            if (b.dailyRune !== a.dailyRune) {
                return b.dailyRune - a.dailyRune;
            }
        ...
        return {
            ...
            totalDailyRune: totalDailyRune
        };
    }

    function getJadeMaterialDetails(dailyCycles) {
        var rule = calCustomRule && calCustomRule.rules && calCustomRule.rules[0];
        var custom = rule && rule.custom;
        var materialMap = {};
        var totalByMaterial = {};
        ...
        if (rule && rule.materials) {
            for (var mi = 0; mi < rule.materials.length; mi++) {
                if (rule.materials[mi] && rule.materials[mi].materialId) {
                    materialMap[String(rule.materials[mi].materialId)] = rule.materials[mi];
                }
            }
        }
        ...
        for (var c in custom) {
            ...
            for (var r in recipes) {
                ...
                var recipeItem = recipes[r];
                var recipeData = recipeItem && recipeItem.data;
                if (!recipeData || !recipeData.materials || !recipeData.materials.length) {
                    continue;
                }

                var recipeQuantity = parseInt(recipeItem.quantity, 10) || 0;
                if (recipeQuantity <= 0) {
                    continue;
                }

                var recipeDetail = {
                    recipeName: recipeData.name || ("菜谱" + (Number(r) + 1)),
                    quantity: recipeQuantity,
                    positionIndex: Number(c) + 1,
                    slotIndex: Number(r) + 1,
                    materials: [],
                ...
                for (var rm = 0; rm < recipeData.materials.length; rm++) {
                    var materialItem = recipeData.materials[rm];
                    if (!materialItem) {
                        continue;
                    }
                    var materialId = String(materialItem.material);
                    var perPortionAmount = Number(materialItem.quantity) || 0;
                    var perCycleAmount = recipeQuantity * perPortionAmount;
                    var dailyAmount = dailyCycles * perCycleAmount;
                    var materialInfo = materialMap[materialId] || {};
                    var materialName = materialInfo.name || ("食材" + materialId);

                    recipeDetail.materials.push({
                        materialId: materialId,
                        materialName: materialName,
                        perPortionAmount: perPortionAmount,
                        perCycleAmount: perCycleAmount,
                        dailyAmount: dailyAmount
                    });
                    recipeDetail.totalDailyAmount += dailyAmount;
                    totalDailyAmount += dailyAmount;

                    if (!totalByMaterial[materialId]) {
                        totalByMaterial[materialId] = {
                            materialId: materialId,
                            materialName: materialName,
                            dailyAmount: 0
                        };
                    }
                    totalByMaterial[materialId].dailyAmount += dailyAmount;
                }

                recipeDetail.materials.sort(function(a, b) {
        ...
        var totalMaterials = [];
        for (var materialKey in totalByMaterial) {
            if (totalByMaterial.hasOwnProperty(materialKey)) {
                totalMaterials.push(totalByMaterial[materialKey]);
            }
        }
        ...
        recipeDetails.sort(function(a, b) {
            return b.totalDailyAmount - a.totalDailyAmount;
        });
    ...
    function applyDailySelectLayout(selectSelector, buttonSelector) {
        ...
        var $button = buttonSelector ? $(buttonSelector) : $();
        ...
        if (!$wrapper.length) {
            $wrapper = $select.siblings(".bootstrap-select");
        }
        if (!$wrapper.length) {
            $wrapper = $select.closest(".bootstrap-select");
        }
        if (!$wrapper.length) {
            return;
        }
        ...
        var longestOptionWidth = textWidth;
        $select.find("option").each(function() {
            var optionText = $.trim($(this).text());
            if (!optionText) {
                return;
            }
            var optionWidth = Math.ceil(measureJadeMaterialTextWidth(optionText, font));
            if (optionWidth > longestOptionWidth) {
                longestOptionWidth = optionWidth;
            }
        });
        var isRuneSelect = selectSelector === "#daily-rune-select";
        var basePadding = isRuneSelect ? 50 : 42;
        var minWidth = isRuneSelect ? 150 : 100;
        var mobileMinClosedWidth = isRuneSelect ? 110 : 92;
        var mobileClosedPadding = isRuneSelect ? 6 : 10;
        var currentTextWidth = Math.max(textWidth + basePadding + mobileClosedPadding, mobileMinClosedWidth);
        var desiredWidth = Math.max(minWidth, longestOptionWidth + basePadding);
        var rowWidth = $row.innerWidth() || 0;
        var groupWidth = $row.closest(".calc-material-group").innerWidth() || 0;
        ...
        if (isMobileViewport) {
            referenceWidth = Math.max(referenceWidth, groupWidth, panelWidth);
            if (!referenceWidth) {
        ...
        $select.attr("data-width", targetWidth + "px");
        $select.css({
        ...
        var pickerInstance = $select.data("selectpicker") || $select.data("this");
        var $menu = $wrapper.find("> .dropdown-menu");
        var $menuContainer = $();

        if (pickerInstance) {
            if (pickerInstance.$menu && pickerInstance.$menu.length) {
                $menu = pickerInstance.$menu;
            }
            if (pickerInstance.$bsContainer && pickerInstance.$bsContainer.length) {
                $menuContainer = pickerInstance.$bsContainer;
            }
        }
        ...
        $menu.css({
            ...
            maxWidth: isMobileViewport ? Math.max(targetWidth, viewportWidth - 24) + "px" : "none"
        });
        $menu.find(".inner").css({
            minWidth: dropdownWidth + "px"
        });
    ...
    function scheduleDailySelectLayout(selectSelector, buttonSelector) {
        if (typeof window.requestAnimationFrame === 'function') {
            window.requestAnimationFrame(function() {
                window.requestAnimationFrame(function() {
                    applyDailySelectLayout(selectSelector, buttonSelector);
                });
            });
            return;
        }
    ...
    function applyJadeMaterialSelectLayout() {
        applyDailySelectLayout("#daily-material-select-jade", "#btn-material-detail-jade");
    }

    function scheduleJadeMaterialSelectLayout() {
        scheduleDailySelectLayout("#daily-material-select-jade", "#btn-material-detail-jade");
    }
    ...
    function renderJadeMaterialSelect(materialDetails) {
        ...
        for (var i = 0; i < materialDetails.totalMaterials.length; i++) {
            var item = materialDetails.totalMaterials[i];
            var value = formatNumber(Math.floor(item.dailyAmount * 100) / 100);
            $select.append(
                $('<option>')
                    .val(item.materialId)
                    .text(item.materialName + ' ' + value + '/天')
            );
        }
    ...
    function renderJadeMaterialDetailModal(materialDetails) {
        var $body = $("#jade-material-detail-body");
        if (!$body.length) {
            return;
        }

        if (!materialDetails || !materialDetails.recipeDetails.length) {
            $body.html('<div class="jade-material-detail-empty">暂无耗材数据</div>');
            $("#btn-material-detail-jade").prop('disabled', true);
            return;
        }

        $("#btn-material-detail-jade").prop('disabled', false);

        var html = '<div class="jade-material-recipe-grid">';
        for (var i = 0; i < materialDetails.recipeDetails.length; i++) {
            var recipeDetail = materialDetails.recipeDetails[i];
            html += '<div class="jade-material-recipe-card">';
            html += '<div class="jade-material-recipe-title">' + escapeHtml(recipeDetail.recipeName) + ' <span class="jade-material-recipe-subtitle">单轮份数：' + recipeDetail.quantity + '</span></div>';
            html += '<ul class="jade-material-detail-list">';
            for (var j = 0; j < recipeDetail.materials.length; j++) {
                var materialItem = recipeDetail.materials[j];
                html += '<li>' + escapeHtml(materialItem.materialName) +
                    ' x ' + formatNumber(materialItem.perPortionAmount) +
                    '  ' + formatNumber(materialItem.perCycleAmount) + '/轮' +
                    ' ' + formatNumber(Math.floor(materialItem.dailyAmount * 100) / 100) + '/天' +
                    '</li>';
            }
            html += '</ul>';
            html += '</div>';
    ...
    function renderGuestRuneSelect(runeDetails) {
        var $select = $("#daily-rune-select");
        if (!$select.length) {
            return;
        }
        ...
        if (!runeDetails || !runeDetails.recipeDetails.length) {
            $select.append('<option value="">无菜谱</option>');
            $select.prop('selectedIndex', 0);
            $select.prop('disabled', false);
            if ($select.data('selectpicker')) {
                $select.selectpicker('refresh');
        ...
        var averageDailyRune = runeDetails.recipeDetails.length > 0
            ? runeDetails.totalDailyRune / runeDetails.recipeDetails.length
            : 0;
        $select.append(
            $('<option>')
                .val("__average__")
                .text('综合产出 ' + formatNumber(Math.floor(averageDailyRune * 100) / 100) + '/天')
        );

        for (var i = 0; i < runeDetails.recipeDetails.length; i++) {
            var item = runeDetails.recipeDetails[i];
            var value = formatNumber(Math.floor(item.dailyRune * 100) / 100);
            var optionValue = item.positionIndex + "-" + item.slotIndex;
            var recipeLabel = item.recipeName + (item.rune ? '（' + item.rune + '）' : '');
            $select.append(
                $('<option>')
                    .val(optionValue)
                    .text(recipeLabel + ' ' + value + '/天')
            );
        }
    ...
    function renderGuestMaterialSelect(materialDetails) {
        var $select = $("#daily-material-select");
        if (!$select.length) {
            return;
        }
    ...
    function renderGuestMaterialDetailModal(materialDetails) {
        var $body = $("#guest-material-detail-body");
        if (!$body.length) {
            return;
        }
        ...
        if (!materialDetails || !materialDetails.recipeDetails.length) {
            $body.html('<div class="jade-material-detail-empty">暂无耗材数据</div>');
            $("#btn-material-detail").prop('disabled', true);
            return;
        }

        $("#btn-material-detail").prop('disabled', false);
    ...
    function syncGuestRateModeSupplementOutputs() {
        var runeOutputs = calculateRuneModeOutputs();
        renderGuestRuneSelect(runeOutputs.runeDetails);
        renderGuestMaterialSelect(runeOutputs.materialDetails);
    ...
    function ensureGuestSupplementSyncOnCalCustomResults() {
        ...
        hasWrappedCalCustomResults = true;
    }

    function syncJadeModeFields() {
        isSyncingJadeMode = true;
        $("#guest-rate-value-jade").text($("#guest-rate-value").text());
        $("#total-crit-rate-jade").text($("#total-crit-rate").text());
        $("#time-percentage-jade").text($("#time-percentage").text());
        $("#actual-guest-rate-jade").text($("#actual-guest-rate").text());
        var jadeOutputs = calculateJadeModeOutputs();
        $("#rune-rate-jade").text(formatNumber(Math.floor(jadeOutputs.scatterRate * 100) / 100) + "%");
        $("#hundred-pot-output-jade").text((Math.floor(jadeOutputs.dailyJade * 100) / 100).toFixed(2));
        renderJadeMaterialSelect(jadeOutputs.materialDetails);
        renderJadeMaterialDetailModal(jadeOutputs.materialDetails);

        setSelectpickerValueSilently("#star-level-jade", $("#star-level").val());
        setSelectpickerValueSilently("#quality-level-jade", $("#quality-level").val());
        $("#quantity-value-jade").val($("#quantity-value").val());

        $("#guest-rate-input-jade").val($("#guest-rate-input").val());
        setSelectpickerValueSilently("#star-level-2-jade", $("#star-level-2").val());
        setSelectpickerValueSilently("#star-level-3-jade", $("#star-level-3").val());
        $("#quantity-value-2-jade").val($("#quantity-value-2").val());
        $("#quantity-value-3-jade").val($("#quantity-value-3").val());

        $("#actual-guest-rate-2-jade").text($("#actual-guest-rate-2").text());
        $("#actual-guest-rate-3-jade").text($("#actual-guest-rate-3").text());
        $("#rune-a-probability-jade").text($("#rune-a-probability").text());
        $("#rune-b-probability-jade").text($("#rune-b-probability").text());
        $("#scatter-probability-jade").text($("#scatter-probability").text());
        $("#total-guest-rate-jade").text($("#total-guest-rate").text());

        $("#sync-guest-double-jade").closest(".calc-result-row").hide();
        $("#guest-jade-result .double-recipe-section").hide();
        isSyncingJadeMode = false;
    }

    function syncJadeControlToSource() {
        isSyncingJadeMode = true;
        setSelectpickerValueSilently("#star-level", $("#star-level-jade").val());
        setSelectpickerValueSilently("#quality-level", $("#quality-level-jade").val());
        $("#quantity-value").val($("#quantity-value-jade").val());
        isSyncingJadeMode = false;
    }
    ...
    function syncJadeDualControlToSource() {
        ...
        setSelectpickerValueSilently("#star-level-2", $("#star-level-2-jade").val());
        setSelectpickerValueSilently("#star-level-3", $("#star-level-3-jade").val());
        $("#quantity-value-2").val($("#quantity-value-2-jade").val());
        $("#quantity-value-3").val($("#quantity-value-3-jade").val());
        isSyncingJadeMode = false;
    }
    ...
    function init() {
        ...
        $("#daily-rune-select").selectpicker();
        $("#daily-material-select").selectpicker();
        $("#star-level-jade").selectpicker();
        $("#quality-level-jade").selectpicker();
        $("#star-level-2-jade").selectpicker();
        $("#star-level-3-jade").selectpicker();
        $("#daily-material-select-jade").selectpicker();
        $("#daily-rune-select, #daily-material-select").on("changed.bs.select change", function() {
            var selectId = "#" + this.id;
            var buttonSelector = selectId === "#daily-material-select" ? "#btn-material-detail" : null;
        ...
        $("#daily-rune-select, #daily-material-select, #daily-material-select-jade").on("shown.bs.select", function() {
            ...
            var buttonSelector = selectId === "#daily-material-select"
                ? "#btn-material-detail"
                : (selectId === "#daily-material-select-jade" ? "#btn-material-detail-jade" : null);
            scheduleDailySelectLayout(selectId, buttonSelector);
        });
        ...
        $("#quantity-value-jade").on("input change", function() {
            if (isSyncingJadeMode) {
                return;
            }
            validateQuantity($(this));
        ...
        $("#quantity-value-2-jade, #quantity-value-3-jade").on("input change", function() {
            if (isSyncingJadeMode) {
                return;
            }
            var val = parseInt($(this).val());
        ...
        $("#guest-rate-input-jade").on("input change", function() {
            if (isSyncingJadeMode) {
                return;
            }
            validateGuestRate($(this));
        ...
        $("#star-level-jade, #quality-level-jade").on("changed.bs.select", function() {
            if (isSyncingJadeMode) {
                return;
            }
            syncJadeControlToSource();
        ...
        $("#quantity-plus-jade").off("click").on("click", function() {
            var input = $("#quantity-value-jade");
            var val = parseInt(input.val()) || 0;
            input.val(val + 1).trigger("change");
        });

        $("#quantity-minus-jade").off("click").on("click", function() {
            var input = $("#quantity-value-jade");
            var val = parseInt(input.val()) || 0;
            if (val > 1) {
                input.val(val - 1).trigger("change");
        ...
        $("#quantity-plus-2-jade").off("click").on("click", function() {
            var input = $("#quantity-value-2-jade");
            var val = parseInt(input.val()) || 0;
            input.val(val + 1).trigger("change");
        });

        $("#quantity-minus-2-jade").off("click").on("click", function() {
            var input = $("#quantity-value-2-jade");
            var val = parseInt(input.val()) || 0;
            if (val > 0) {
                input.val(val - 1).trigger("change");
        ...
        $("#quantity-plus-3-jade").off("click").on("click", function() {
            var input = $("#quantity-value-3-jade");
            var val = parseInt(input.val()) || 0;
            input.val(val + 1).trigger("change");
        });

        $("#quantity-minus-3-jade").off("click").on("click", function() {
            var input = $("#quantity-value-3-jade");
            var val = parseInt(input.val()) || 0;
            if (val > 0) {
                input.val(val - 1).trigger("change");
        ...
        $("#sync-guest-double-jade").off("click").on("click", function() {
            syncJadeControlToSource();
            $("#sync-guest-double").trigger("click");
            syncJadeModeFields();
        });

        $("#btn-material-detail-jade").off("click").on("click", function() {
            $("#jade-material-detail-modal").modal("show");
        });
        $("#btn-material-detail").off("click").on("click", function() {
            $("#guest-material-detail-modal").modal("show");
        });
        ...
        $("#star-level-2-jade, #star-level-3-jade").on("changed.bs.select", function() {
            if (isSyncingJadeMode) {
                return;
            }
            syncJadeDualControlToSource();
        ...
        if (typeof window.getCalRecipeDisp === 'function') {
            ...
            window.getCalRecipeDisp = function(e) {
                ...
                // 在碰瓷查询模式下，显示当前品级对应的贵客
                if (isGuestRateMode() && isPengciMode && a.guests && a.guests.length > 0) {
                    // 使用 e.rankDisp（显示的品级）而不是 a.rank（个人品级）
                    // e.rankDisp: 厨师做出来的品级（可/优/特/神/传）
                    // guests[0]=优级贵客, guests[1]=特级贵客, guests[2]=神级贵客
                    var currentRank = 0;
                    var displayRank = e.rankDisp || '可';
                    if (displayRank === '可') currentRank = 1;
                    else if (displayRank === '优') currentRank = 2;
                    else if (displayRank === '特') currentRank = 3;
                    else if (displayRank === '神') currentRank = 4;
                    ...
                    if (currentGuestIndex >= 0 && currentGuestIndex < Math.min(a.guests.length, 3)) {
                        ...
                        if (currentGuest && currentGuest.guest) {
                            recipeName = a.name + '-' + currentGuest.guest;
                        }
                    }
                }
    ...
    function calculateFields(custom, rule, starLevel, quantity, qualityLevel) {
        ...
        var activeSelfUltimateMap = {};
        if (rule && rule.calSelfUltimateData) {
            for (var su = 0; su < rule.calSelfUltimateData.length; su++) {
                var selfUltimateItem = rule.calSelfUltimateData[su];
                if (selfUltimateItem && selfUltimateItem.chefId) {
                    activeSelfUltimateMap[String(selfUltimateItem.chefId)] = selfUltimateItem.effect || [];
                }
            }
        }
        
        var activePartialChefSet = {};
        if (rule && rule.calPartialChefIds) {
            for (var pi = 0; pi < rule.calPartialChefIds.length; pi++) {
                activePartialChefSet[String(rule.calPartialChefIds[pi])] = true;
            }
        }
        
        var globalUltimateSkills = rule && rule.calGlobalUltimateData ? rule.calGlobalUltimateData : [];
        ...
        for (var c in custom) {
            ...
            var chefIdStr = String(chef.chefId);
            var activeSelfUltimateEffects = getActiveSelfUltimateEffects(chef, activeSelfUltimateMap);
            var isPartialUltimateActive = !!activePartialChefSet[chefIdStr] ||
                (typeof isAllUltimateMode !== 'undefined' && isAllUltimateMode);
            ...
            // 2. 当前生效的自我修炼技能
            if (activeSelfUltimateEffects && activeSelfUltimateEffects.length > 0) {
                for (var i = 0; i < activeSelfUltimateEffects.length; i++) {
                    allSkills.push(activeSelfUltimateEffects[i]);
                    skillSources.push({
                        type: 'ultimate',
            ...
            // 3. 当前生效的上场类修炼技能
            if (isPartialUltimateActive && chef.ultimateSkillEffect) {
                for (var i = 0; i < chef.ultimateSkillEffect.length; i++) {
                    var ultimateSkill = chef.ultimateSkillEffect[i];
                    if (ultimateSkill.condition === 'Partial' || ultimateSkill.condition === 'Next') {
                        allSkills.push(ultimateSkill);
                        skillSources.push({
                            type: 'ultimate',
                            desc: chef.ultimateSkillDisp || ''
                        });
            ...
            // 4. 厨具技能（使用当前生效的自我修炼技能增强厨具效果）
            if (equip && equip.effect) {
                var equipEffect = equip.effect;
                if (activeSelfUltimateEffects.length > 0 && typeof updateEquipmentEffect === 'function') {
                    equipEffect = updateEquipmentEffect(equip.effect, activeSelfUltimateEffects);
                }
        ...
        if (globalUltimateSkills.length > 0) {
            for (var g = 0; g < globalUltimateSkills.length; g++) {
                var globalSkill = globalUltimateSkills[g];
                
                if (globalSkill.type === "GuestApearRate") {
                    result.guestRate += globalSkill.value || 0;
                }
                
                if (globalSkill.type === "OpenTime") {
                    var globalTimeValue = globalSkill.value || 0;
                    globalTimeValue = Math.trunc(globalTimeValue * 10) / 10;
                    result.timePercentage += globalTimeValue;
                }
                
                if (globalSkill.type === "GuestAntiqueDropRate") {
                    guestAntiqueDropRateSkills.push({
                        value: globalSkill.value,
                        conditionType: globalSkill.conditionType,
                        conditionValue: globalSkill.conditionValue
                    });
                }
            }
        }
        ...
        // 时间字段直接复用下方总时间的 OpenTime 汇总口径，保持一致
        result.timePercentage = calculateCurrentTimePercentage(custom, rule);
    ...
    function updateSummaryDisplay(calCustomRule) {
        ...
        if (calCustomRule.isGuestRate && calCustomRule.rules.length > 0) {
            ...
            var fields = calculateFields(currentCustom, calCustomRule.rules[0], starLevel, quantity, qualityLevel);
    ...
    function analyzeChefGuestRateSkills(chefOrEquip, qixiaData, useEquip, useAmber, localData, configUltimatedIds, type) {
        ...
        var activeSelfUltimateEffects = [];
        var activeUltimateTimeEffects = [];
        var isPartialUltimateActive = false;
        var isChefUltimatedFlag = false;

        if (chef.ultimateSkillEffect && chef.ultimateSkillEffect.length > 0) {
            var selfUltimateActive = isUltimateConditionActiveForChef(chef, 'Self', localData, configUltimatedIds);
            isPartialUltimateActive = isUltimateConditionActiveForChef(chef, 'Partial', localData, configUltimatedIds);
            isChefUltimatedFlag = isChefUltimated(chef.chefId, localData, configUltimatedIds);
            
            if (selfUltimateActive) {
                for (var se = 0; se < chef.ultimateSkillEffect.length; se++) {
                    if (chef.ultimateSkillEffect[se].condition === 'Self') {
                        activeSelfUltimateEffects.push(chef.ultimateSkillEffect[se]);
                    }
                }
            }
        }
        ...
        function markCategoryBySkill(skill) {
            if (!skill) {
                return;
            }
        ...
        if (chef.ultimateSkillEffect) {
            for (var i = 0; i < chef.ultimateSkillEffect.length; i++) {
                ...
                var isActiveUltimateSkill = false;

                // 时间类修炼词条只看词条本身的 type/value，已修炼即可参与分类排序
                if (isChefUltimatedFlag && (skill.type === 'OpenTime' || skill.type === 'CookbookTime')) {
                    isActiveUltimateSkill = true;
                } else if (skill.condition === 'Self') {
                    isActiveUltimateSkill = activeSelfUltimateEffects.length > 0;
                } else if (skill.condition === 'Partial' || skill.condition === 'Next') {
                    isActiveUltimateSkill = isPartialUltimateActive;
                }
                if (isActiveUltimateSkill && targetSkillTypes.indexOf(skill.type) >= 0) {
                    result.hasSkills = true;
                    markCategoryBySkill(skill);
                    if (skill.type === 'OpenTime' || skill.type === 'CookbookTime') {
                        activeUltimateTimeEffects.push(skill);
                    }
                    allSkillSources.push({
                        skill: skill,
                        type: 'ultimate',
        ...
        if (categoryFlags.time) {
            ...
            if (activeUltimateTimeEffects.length > 0 && typeof getTimeAddition === 'function') {
                totalTimeAddition += getTimeAddition(activeUltimateTimeEffects);
            }
            
            if (useEquip && chef.equip && chef.equip.effect && typeof getTimeAddition === 'function') {
                var equipEffect = chef.equip.effect;
                if (typeof updateEquipmentEffect === 'function') {
                    equipEffect = updateEquipmentEffect(chef.equip.effect, activeSelfUltimateEffects);
                }
    ...
    function detectAndMarkConflicts($select, sp, runeList, gameData) {
        ...
        $innerMenu.find('li').each(function() {
            ...
            var guestHtml = guestArray.map(function(guestName) {
                ...
                if (isFieldConflict) {
                    return '<span style="color: #ff5b5b; font-weight: bold;">' + guestName + '</span>';
                }
                // 检查是否在同一符文内有冲突（红色）
                if (isSameRuneConflict) {
                    return '<span style="color: #ff5b5b;">' + guestName + '</span>';
                }
                // 检查是否跨符文冲突
    ...
    function checkGuestConflictsForSelectedRecipes(ruleIndex, gameData) {
        ...
        // 先清除所有菜谱的冲突标记
        $(".cal-custom-item:eq(" + ruleIndex + ") .selected-item .recipe-box .recipe-box-1 .name")
            .removeClass("guest-conflict-highlight")
            .css({
                "color": "",
                "font-weight": ""
            });
        ...
        conflictRecipes.forEach(function(recipeInfo) {
            ...
            if ($recipeName.length > 0) {
                $recipeName.addClass("guest-conflict-highlight");
                $recipeName.css({
                    "color": "red",
                    "font-weight": "bold"
                });
    ...
    function displayGuestConflictWarning(ruleIndex, gameData) {
        ...
        if (conflictInfo && conflictInfo.length > 0) {
            var conflictHtml = "<div style='color:#ff5b5b; margin-top: 5px;'>";
    ...
    function getFilteredAndSortedChefs(chefs, categoryType, onlyShowOwned, options) {
        ...
        for (var i = 0; i < chefs.length; i++) {
            ...
            // 是否只显示已拥有的厨师
            if (onlyShowOwned && !isOwnedValueTrue(chef.got)) continue;
     ...
     * @param {Object} chef - 已计算技法值的厨师
     * @param {Object} recipe - 菜谱对象
     * @returns {number} 品级（0=无法制作, 1=可, 2=优, 3=特, 4=神, 5=传）
     */
    ...
    function autoQueryPengciRecipesForCurrentChefs(overwriteExisting) {
        ...
        if (currentChefs.length === 0) {
            return { success: false, message: '场上没有厨师，请先选择厨师或手动选择碰瓷菜谱' };
        }
        ...
        if (!gameData || !gameData.recipes || !gameData.chefs) {
        ...
        for (var i = 0; i < currentChefs.length; i++) {
            ...
            if (chefRecipes.length > 0) {
                assignment.push({
                    ...
                    chef: chefInfo.chef,
                    chefName: chefInfo.chef.name,
                    recipes: chefRecipes,
                    slotIndex: chefInfo.slotIndex
                });
            }
        }

        // 计算实际找到的菜谱数量（场上已有 + 本次新增）
        var totalFoundRecipes = existingRecipeCount;
        for (var i = 0; i < assignment.length; i++) {
            totalFoundRecipes += assignment[i].recipes.length;
        }
        
        // 如果菜谱数量不够，先显示已查询到的结果，再弹窗询问是否补全
        if (totalFoundRecipes < totalExpectedRecipes) {
            // 先显示已查询到的结果
            displayPengciQueryResult({
                success: true,
            ...
            // 然后显示补全选项
            showCompletionOptions(assignment, totalFoundRecipes, totalExpectedRecipes, currentChefs, normalPengciRecipes, rule);
    ...
    /**
     * 获取场上当前的厨师
     * @returns {Array} 厨师信息数组 [{chefId, chef, slotIndex}, ...]
     */
    function getCurrentChefs() {
        var currentChefs = [];
        ...
        // 遍历3个厨师位
        for (var i = 0; i < 3 && i < rule.custom.length; i++) {
            var customSlot = rule.custom[i];
            if (customSlot && customSlot.chef && customSlot.chef.chefId) {
                currentChefs.push({
                    chefId: customSlot.chef.chefId,
                    chef: customSlot.chef,
                    slotIndex: i
                });
            }
        }
    ...
    /**
     * 获取场上当前已选择的菜谱数量
     * @returns {number} 已选择的菜谱数量
     */
    function getCurrentSelectedRecipeCount() {
        if (typeof calCustomRule === 'undefined' || !calCustomRule.rules || !calCustomRule.rules[0]) {
            return 0;
        }
        ...
        for (var i = 0; i < 3 && i < rule.custom.length; i++) {
            var customSlot = rule.custom[i];
            if (!customSlot || !customSlot.recipes) continue;

            for (var j = 0; j < customSlot.recipes.length; j++) {
                if (customSlot.recipes[j] && customSlot.recipes[j].data && customSlot.recipes[j].data.recipeId) {
                    recipeCount++;
                }
            }
        }
     ...
     */
    function hasEnoughCurrentPengciConfig() {
        var currentChefs = getCurrentChefs();
        if (currentChefs.length === 0) {
            return false;
        }

        var currentRecipeCount = getCurrentSelectedRecipeCount();
        return currentRecipeCount >= currentChefs.length * 3;
    }

    /**
     * 获取场上当前碰瓷分配结果
     * @returns {Array} 当前分配结果
     */
    function getCurrentPengciAssignment() {
        var assignment = [];
        ...
        for (var i = 0; i < 3 && i < rule.custom.length; i++) {
            var customSlot = rule.custom[i];
            if (!customSlot || !customSlot.chef || !customSlot.chef.chefId) continue;

            var recipes = [];
            if (customSlot.recipes) {
                for (var j = 0; j < customSlot.recipes.length; j++) {
                    var recipeData = customSlot.recipes[j] && customSlot.recipes[j].data;
    ...
    /**
     * 获取场上当前已分配的碰瓷菜谱名称集合
     * @returns {Set} 已分配菜谱名称集合
     */
    function getCurrentAssignedRecipeNames() {
        var assignedRecipeNames = new Set();

        if (typeof calCustomRule === 'undefined' || !calCustomRule.rules || !calCustomRule.rules[0]) {
            return assignedRecipeNames;
        }
        ...
        for (var i = 0; i < 3 && i < rule.custom.length; i++) {
            ...
            for (var j = 0; j < customSlot.recipes.length; j++) {
                if (customSlot.recipes[j] && customSlot.recipes[j].data && customSlot.recipes[j].data.name) {
                    assignedRecipeNames.add(customSlot.recipes[j].data.name);
                }
            }
        }
     ...
     */
    function getNormalPengciRecipes(gameData) {
     ...
     * 获取所有碰瓷菜谱（普通 + 主线）
     * @param {Object} gameData - 游戏数据
     * @returns {Array} 全部可碰瓷菜谱数组
     */
    function getAllPengciRecipes(gameData) {
        var recipes = gameData.recipes || [];
        var allRecipes = [];
    ...
    function findPengciRecipesForChef(chef, pengciRecipes, rule, maxCount, chefIndex) {
        ...
        for (var i = 0; i < pengciRecipes.length; i++) {
            var pengciRecipe = pengciRecipes[i];
            var actualRank = getChefRecipeRank(chefWithSkills, pengciRecipe.recipe);
            
            // 检查是否能精确匹配目标品级
            var isMatch = false;
            if (pengciRecipe.targetRank === 4) {
                // 目标是神级，神级或传级都可以
                isMatch = (actualRank >= 4);
            } else {
                // 其他品级必须精确匹配
                isMatch = (actualRank === pengciRecipe.targetRank);
            }
            
            if (isMatch) {
                matchedRecipes.push({
                    recipe: pengciRecipe.recipe,
                    guest: pengciRecipe.guest,
                    targetRank: pengciRecipe.targetRank,
                    currentRank: pengciRecipe.currentRank,
                    recipeName: pengciRecipe.recipeName,
        ...
        // 按目标品级升序排序（优先最低品级）
        matchedRecipes.sort(function(a, b) {
            if (a.targetRank !== b.targetRank) {
        ...
        // 返回前maxCount个菜谱
        return matchedRecipes.slice(0, maxCount);
    }
    
    /**
     * 显示补全选项弹窗
     * @param {Array} assignment - 当前分配结果
     * @param {number} foundCount - 已找到的菜谱数量
     * @param {number} expectedCount - 期望的菜谱数量
     * @param {Array} currentChefs - 当前厨师数组
     * @param {Array} normalPengciRecipes - 普通碰瓷菜谱数组
     * @param {Object} rule - 规则对象
     */
    function showCompletionOptions(assignment, foundCount, expectedCount, currentChefs, normalPengciRecipes, rule) {
        var missingCount = expectedCount - foundCount;
        
        // 构建弹窗HTML
        var html = '<div class="modal fade" id="pengci-completion-modal" tabindex="-1">';
        html += '<div class="modal-dialog">';
        html += '<div class="modal-content">';
        html += '<div class="modal-header">';
        html += '<button type="button" class="close" data-dismiss="modal">&times;</button>';
        html += '<h4 class="modal-title">菜谱数量不足</h4>';
        html += '</div>';
        html += '<div class="modal-body">';
        html += '<p>查询到 <strong>' + foundCount + '</strong> 道菜谱，还差 <strong>' + missingCount + '</strong> 道菜谱未能分配。</p>';
        html += '<p>请选择补全方式：</p>';
        ...
        html += '<button type="button" class="btn btn-warning" id="btn-ignore-rank-complete" style="min-width: 150px;">无视品级补全</button>';
        html += '<span style="margin-left: 15px; color: #666;">使用普通碰瓷菜谱补全，无视额外次数消耗</span>';
        html += '</div>';
        
        // 全菜谱补全按钮
        html += '<div style="margin-bottom: 15px;">';
        html += '<button type="button" class="btn btn-success" id="btn-all-recipes-complete" style="min-width: 150px;">使用全菜谱补全</button>';
        html += '<span style="margin-left: 15px; color: #666;">使用普通、主线两个分类的全部菜谱补全</span>';
        html += '</div>';

        // 使用其他厨师补全按钮（只在场上厨师数量 < 3 时显示）
        if (currentChefs.length < 3) {
            html += '<div style="margin-bottom: 15px;">';
            html += '<button type="button" class="btn btn-primary" id="btn-add-chef-complete" style="min-width: 150px;">使用其他厨师补全</button>';
            html += '<span style="margin-left: 15px; color: #666;">添加新厨师到空位，补全剩余菜谱</span>';
            html += '</div>';
        }
        ...
        // 移除旧弹窗并添加新弹窗
        $('#pengci-completion-modal').remove();
        $('body').append(html);
        
        // 绑定按钮事件
        $('#btn-ignore-rank-complete').off('click').on('click', function() {
            $('#pengci-completion-modal').modal('hide');
            var currentAssignment = getCurrentPengciAssignment();
            var currentChefsOnField = getCurrentChefs();
            var gameData = (typeof calCustomRule !== 'undefined' && calCustomRule.gameData)
                ? calCustomRule.gameData
                : null;
            var currentNormalPengciRecipes = gameData ? getNormalPengciRecipes(gameData) : [];
            var currentAssignedRecipeNames = getCurrentAssignedRecipeNames();
            if (currentAssignedRecipeNames.size > 0) {
                currentNormalPengciRecipes = currentNormalPengciRecipes.filter(function(item) {
                    return !currentAssignedRecipeNames.has(item.recipeName);
                });
            }
            var result = completeWithIgnoreRank(currentAssignment, currentChefsOnField, currentNormalPengciRecipes, rule);
            displayPengciQueryResult(result);
        });

        $('#btn-all-recipes-complete').off('click').on('click', function() {
            $('#pengci-completion-modal').modal('hide');
            var currentAssignment = getCurrentPengciAssignment();
            var currentChefsOnField = getCurrentChefs();
            var result = completeWithAllRecipes(currentAssignment, currentChefsOnField, rule);
            displayPengciQueryResult(result);
        });

        $('#btn-add-chef-complete').off('click').on('click', function() {
            $('#pengci-completion-modal').modal('hide');
            var currentAssignment = getCurrentPengciAssignment();
            var currentChefsOnField = getCurrentChefs();
            var result = completeWithOtherChefs(currentAssignment, currentChefsOnField, normalPengciRecipes, rule, expectedCount);
            displayPengciQueryResult(result);
        });
        
        // 显示弹窗
        $('#pengci-completion-modal').modal('show');
    }
     ...
     * @param {Array} normalPengciRecipes - 普通碰瓷菜谱数组
     * @param {Object} rule - 规则对象
     * @returns {Object} 补全后的查询结果
     */
    function completeWithIgnoreRank(assignment, currentChefs, normalPengciRecipes, rule) {
        // 获取全场已分配的菜谱名称，补全过程中动态更新，避免重复分配
        var globalAssignedRecipeNames = new Set();
        for (var i = 0; i < assignment.length; i++) {
            for (var j = 0; j < assignment[i].recipes.length; j++) {
                globalAssignedRecipeNames.add(assignment[i].recipes[j].recipeName);
            }
        }
        ...
        for (var i = 0; i < assignment.length; i++) {
            ...
            for (var j = 0; j < normalPengciRecipes.length; j++) {
                var pengciRecipe = normalPengciRecipes[j];

                // 跳过全场已分配的菜谱
                if (globalAssignedRecipeNames.has(pengciRecipe.recipeName)) continue;

                var actualRank = getChefRecipeRank(chefWithSkills, pengciRecipe.recipe);

                // 至少要能做到优级，且高于当前品级，才可用于碰瓷补全
                if (actualRank >= 2 && actualRank > pengciRecipe.currentRank) {
                    additionalRecipes.push({
                        ...
                        recipeName: pengciRecipe.recipeName,
                        actualRank: actualRank,
                        rankDiff: Math.abs(actualRank - pengciRecipe.targetRank) // 品级差距
                    });
                }
            }

            // 排序规则：
            // 1. 优先选择当前品级更高的菜谱（优级>特级>神级>可级）
            // 2. 同品级内，按品级差距升序（越接近目标品级越好）
            // 3. 同品级差距内，按目标品级升序
            additionalRecipes.sort(function(a, b) {
                // 优先级：当前品级更高的优先（可级=1最低，神级=4最高）
                if (a.currentRank !== b.currentRank) {
                    return b.currentRank - a.currentRank; // 降序，品级高的在前
                }
                // 同品级内，按品级差距升序
            ...
            // 补全到3道菜
            var needCount = 3 - currentCount;
            for (var j = 0; j < additionalRecipes.length && j < needCount; j++) {
                item.recipes.push(additionalRecipes[j]);
                globalAssignedRecipeNames.add(additionalRecipes[j].recipeName);
            }
        }
     ...
     * @returns {Object} 补全后的查询结果
     */
    function completeWithAllRecipes(assignment, currentChefs, rule) {
        var gameData = (typeof calCustomRule !== 'undefined' && calCustomRule.gameData)
            ? calCustomRule.gameData
            : null;
        ...
        var allPengciRecipes = getAllPengciRecipes(gameData);
        var globalAssignedRecipeNames = getCurrentAssignedRecipeNames();
        if (globalAssignedRecipeNames.size > 0) {
            allPengciRecipes = allPengciRecipes.filter(function(item) {
                return !globalAssignedRecipeNames.has(item.recipeName);
            });
        }

        // 第一阶段：优先精确匹配目标品级，不浪费碰瓷次数
        for (var i = 0; i < assignment.length; i++) {
            var item = assignment[i];
            var currentCount = item.recipes.length;
            if (currentCount >= 3) continue;
            ...
            if (preciseRecipes.length > 0) {
                allPengciRecipes = allPengciRecipes.filter(function(recipe) {
                    return !globalAssignedRecipeNames.has(recipe.recipeName);
                });
            }
        }

        // 第二阶段：如果还有空位，再无视品级补全
        return completeWithIgnoreRank(assignment, currentChefs, allPengciRecipes, rule);
    }
     ...
     * @param {Array} normalPengciRecipes - 普通碰瓷菜谱数组
     * @param {Object} rule - 规则对象
     * @param {number} expectedCount - 期望的菜谱总数
     * @returns {Object} 补全后的查询结果
     */
    function completeWithOtherChefs(assignment, currentChefs, normalPengciRecipes, rule, expectedCount) {
        // 获取已分配的菜谱名称
        var assignedRecipeNames = new Set();
        for (var i = 0; i < assignment.length; i++) {
            for (var j = 0; j < assignment[i].recipes.length; j++) {
                assignedRecipeNames.add(assignment[i].recipes[j].recipeName);
            }
        }
        
        // 获取已使用的厨师ID
        var usedChefIds = new Set();
        for (var i = 0; i < currentChefs.length; i++) {
            usedChefIds.add(currentChefs[i].chefId);
        }
        
        // 过滤出未分配的菜谱
        var remainingRecipes = [];
        for (var i = 0; i < normalPengciRecipes.length; i++) {
            if (!assignedRecipeNames.has(normalPengciRecipes[i].recipeName)) {
                remainingRecipes.push(normalPengciRecipes[i]);
            }
        }
        ...
        var currentCount = 0;
        for (var i = 0; i < assignment.length; i++) {
            currentCount += assignment[i].recipes.length;
        }
        var needCount = expectedCount - currentCount;
        ...
        for (var i = 0; i < 3; i++) {
            var isUsed = false;
            for (var j = 0; j < currentChefs.length; j++) {
                if (currentChefs[j].slotIndex === i) {
                    isUsed = true;
                    break;
                }
            }
        ...
        for (var i = 0; i < otherChefs.length && emptySlots.length > 0 && addedCount < needCount; i++) {
            ...
            assignment.push({
                chefId: otherChef.chefId,
                chef: otherChef.chef,
                chefName: otherChef.chef.name,
                recipes: recipesToAdd,
                slotIndex: slotIndex
            });
            
            addedCount += recipesToAdd.length;
            
            // 更新已分配的菜谱名称
            for (var j = 0; j < recipesToAdd.length; j++) {
                assignedRecipeNames.add(recipesToAdd[j].recipeName);
            }
            
            // 更新剩余菜谱
            var newRemainingRecipes = [];
            for (var j = 0; j < remainingRecipes.length; j++) {
                if (!assignedRecipeNames.has(remainingRecipes[j].recipeName)) {
                    newRemainingRecipes.push(remainingRecipes[j]);
                }
            }
        ...
        if (addedCount < needCount) {
            var stillMissing = needCount - addedCount;
            setTimeout(function() {
                if (typeof showAlert === 'function') {
                    showAlert('已补全 ' + addedCount + ' 道菜谱，还有 ' + stillMissing + ' 道菜谱无法分配' + (emptySlots.length === 0 ? '（厨师位已满）' : ''));
                }
            }, 500);
        }
        ...
        return {
            ...
            assignment: assignment,
            unassignedRecipes: [],
            isPartial: addedCount < needCount,
            isAutoQuery: true
        };
    }
        ...
        $("#btn-pengci-guest-query").off("click").on("click", function() {
            ...
            if (selectedPengciRecipes.length === 0) {
                ...
                if (autoQueryResult.success) {
                    // 自动查询成功，显示结果
                    displayPengciQueryResult(autoQueryResult);
                    return;
                } else {
                    // 自动查询失败，显示提示
                    if (typeof showAlert === 'function') {
                        showAlert(autoQueryResult.message || '请先选择要碰瓷的菜谱');
                    } else {
                        alert(autoQueryResult.message || '请先选择要碰瓷的菜谱');
                    }
                    return;
                }
            }
