...
function initRecipeTable(e) {
    ...
    (function() {
        ...
        if ($actionsBox.length && !$actionsBox.find(".bs-invert-all").length) {
            ...
            bsContainer.find(".bs-save-scheme").on("click", function(ev) {
                ...
                // 获取选中食材的名称
                var selectedNames = [];
                matSelect.find("option:selected").each(function() {
    ...
    $.fn.dataTableExt.afnFiltering.push(function(a, t, i, l, s) {
        if (a.nTable != document.getElementById("recipe-table"))
            return !0;
        var excludeMats = $("#chk-recipe-material-exclude").val();
        if (!excludeMats || excludeMats.length == 0)
            return !0;
        // 检查菜谱是否包含排除的食材
        for (var ei in excludeMats) {
            for (var mi in l.materials) {
                if (l.materials[mi].material == excludeMats[ei]) {
                    return !1; // 包含排除的食材，不显示
                }
            }
        }
        return !0;
...
function getCustomRecipesOptions(e, a, t, i) {
    var l = $("#chk-cal-got").prop("checked")
      ...
      , _excludeMats = $("#chk-cal-recipe-material-exclude").val()
    ...
    for (var y in k) {
        ...
        if ((!l || x.data.got || isAllUltimateMode) && !(s.length > 0 && s.indexOf(x.data.rarity.toString()) < 0)) {
            ...
            if (!(n.length > 0 && n.indexOf(x.data.condiment) < 0)) {
                if (_excludeMats && _excludeMats.length > 0) {
                    var _hasExcluded = !1;
                    for (var _ei in x.data.materials)
                        if (_excludeMats.indexOf(x.data.materials[_ei].material.toString()) >= 0) {
                            _hasExcluded = !0;
                            break
                        }
                    if (_hasExcluded) continue
                }
                var w = {};
